...
class Job_card extends CI_Controller
{
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->new == 1) {
                  ...
                  foreach ($spring_extrusion_production_details_result as  $spring_extrusion_production_details_row) {
                    ...
                    if ($setup_jobcard_no != '') {
                      ...
                      foreach ($material_manufacturing_setup_result as $material_manufacturing_setup_row) {
                        ...
                        $data_reserved_1 = array(
                          'manu_order_no' => $material_manufacturing_setup_row->manu_order_no,
                    ...
                    if ($purging_jobcard != '') {
                      ...
                      $material_manu_purging_data = array(
                        'manu_order_no' => $purging_jobcard,
                        'archive' => 0
                      );
              ...
              if ($jobcard_type == '2') {
                ...
                $material_manu_print_data = array(
                  'manu_order_no' => $this->uri->segment(3),
                  'archive' => 0
                );
  ...
  public function mail()
  {
    ...
    if ($result == FALSE) {
      ...
      <body>
      <div style='font-family:verdana;border:1px solid #ddd;padding:10px;width:900px;'>
      ...
      <table cellpadding='5'><tr><th>DATE</th><th>ORDER NO</th><th>ARTICLE NO</th><th>ORDER QNT</th><th>JOBCARD NO</th><th>JOBCARD QNT</th><th>ACTION</th><th>CREATED BY</th></tr>";
      foreach ($result as $job_card_row) {

        $dataa = array(
          'order_no' => $job_card_row->sales_ord_no,
          'article_no' => $job_card_row->article_no
        );
        $data['spec'] = $this->common_model->select_one_active_record_nonlanguage_without_archives('order_details', $this->session->userdata['logged_in']['company_id'], $dataa);
        //echo $this->db->last_query();
        ...
        $order_flag = '';
        $data['order_master'] = $this->common_model->select_one_active_record('order_master', $this->session->userdata['logged_in']['company_id'], 'order_no', $job_card_row->sales_ord_no);
        foreach ($data['order_master'] as $order_master_row) {
          $order_flag = $order_master_row->order_flag;
        }

        echo  $html .= "<tr>
                <td>" . $job_card_row->manu_plan_date . "</td>
                <td>" . $job_card_row->sales_ord_no . "</td>
                <td>" . $job_card_row->article_no . "</td>
                <td>" . $job_card_row->order_qnt . "</td>
                <td>" . $job_card_row->mp_pos_no . "</td>
                <td>" . $job_card_row->jobcard_qnt . "</td>
                <td><a href=" . base_url('index.php/' . $this->router->fetch_class() . '/issue_jobcard/' . $job_card_row->mp_pos_no . '/' . $spec_id . '/' . $version) . ">Issue</a></td><td>" . $job_card_row->name1 . " " .
          $job_card_row->name2 . "</td>
              </tr>";

        $update_result = $this->Auto_mailer->update_jobcardflag($job_card_row->mp_pos_no);
        echo $this->db->last_query();


        /*$update = array('mail_sent' => '1');
        $this->common_model->update_one_active_record('production_master', $update, 'mp_pos_no', $job_card_row->mp_pos_no, $this->session->userdata['logged_in']['company_id']);*/
      }
