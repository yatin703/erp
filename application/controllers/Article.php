...
class Article extends CI_Controller
{
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->new == 1) {
              ...
              $this->form_validation->set_rules('type_of_supply', 'Type', 'required|trim|xss_clean');
              ...
              $this->form_validation->set_rules('article_name', 'Article Name', 'required|trim|xss_clean|is_unique[article_name_info.lang_article_description]|strtoupper|max_length[500]|callback_check_inv_name');
              ...
              if ($this->input->post('main_group') == 3) {

                $this->form_validation->set_rules('customer_category', 'Customer Group', 'required|trim|xss_clean');
              }
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                if (!empty($this->input->post('second_sub_group')) && $this->input->post('sub_group') != '34') {
                ...
                //Springtube Word is added in description of SPSM/SPSP/SSRM/SSRP on 30 Dec 2020

                $spring_word = 'SPRING TUBE';

                $spring_arr = array('314', '315', '328', '329');

                $lang_article_description = strtoupper($this->input->post('article_name'));

                if ($this->input->post('main_group') == '3' && in_array($sub_group, $spring_arr)) {
                  $lang_article_description = $lang_article_description . ' ' . $spring_word;
                }
                ...
                $data = array(
                  ...
                  'article_no' => $this->input->post('article_no'),
                  'lang_article_description' => $lang_article_description,
                ...
                if ($this->input->post('main_group') == '1' && $sub_group == '12') {
                  $maintain_in_batches = 'Yes';
                }
                ...
                $name = $lang_article_description;
                $part_no = $this->input->post('article_no');
                ...
                // if($this->input->post('main_group')=='3' && in_array($sub_group,$spring_arr)){
                //     $name=$name.' '.$spring_word;
                // }


                if ($this->input->post('second_sub_group') != '') {
                  ...
                  foreach ($data['second_sub_group'] as $second_sub_group_row) {
                    $under_group = strtoupper($second_sub_group_row->second_sub_group);
                  }
                } else if ($this->input->post('sub_group') != '') {
                  ...
                  foreach ($data['sub_group'] as $sub_group_row) {
                    $under_group = strtoupper($sub_group_row->sub_group);
                  }
                } else {
                  $data['main_group'] = $this->common_model->select_one_active_record('article_main_group', $this->session->userdata['logged_in']['company_id'], 'main_group_id', $this->input->post('main_group'));
                  foreach ($data['main_group'] as $main_group_row) {
                    $under_group = strtoupper($main_group_row->lang_main_group_desc);
                  }
                }

                $uom_result = $this->uom_model->select_one_active_record('uom_master', 'uom_id', $this->input->post('uom'), $this->session->userdata['logged_in']['language_id']);
                foreach ($uom_result as $uom_row) {
                  $units = strtoupper($uom_row->lang_uom_short);
                }

                $excise_rates_master_result = $this->tariff_model->select_one_active_record('excise_rates_master', 'excise_rates_master.erm_id', $this->input->post('tariff'), $this->session->userdata['logged_in']['company_id']);

                foreach ($excise_rates_master_result as $excise_rates_master_row) {
                  $hsn_no = $excise_rates_master_row->cetsh_no;
                  $hsn_description = strtoupper($excise_rates_master_row->lang_tariff_heading);
                }
                ...
                $data['note'] = 'Create Transaction Completed ' . $this->input->post('article_no');
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->modify == 1) {
              ...
              */
              ...
              $this->form_validation->set_rules('article_name', 'Article Name', 'required|trim|strtoupper|max_length[700]');
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                $name = '';
                $new_name = strtoupper($this->input->post('article_name'));

                $data['article'] = $this->article_model->select_one_active_record('article', $this->session->userdata['logged_in']['company_id'], 'article.article_no', $this->input->post('article_no'));
                foreach ($data['article'] as $article_row) {
                  $name = strtoupper($article_row->article_name);
                }
                ...
                $result = $this->common_model->update_one_active_record_tally('tally_stock_items_master', $tally);


                $data = array('lang_article_description' => $this->input->post('article_name'), 'lang_sub_description' => $this->input->post('article_desc'), 'adr_category_id' => $this->input->post('customer_category'));
                ...
                $units = '';
                $maintain_in_batches = 'No';
                if ($this->input->post('sub_group') == '12') {
                  $maintain_in_batches = 'yes';
                }
                ...
                //$name=strtoupper($this->input->post('article_name'));
                $part_no = $this->input->post('article_no');
                $description = $this->input->post('article_desc');
                ...
                // Code Commented Due to Error comes while updating in Tally.... 03-July-2019
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              ...
              $this->form_validation->set_rules('imported_flag', 'Imported', 'trim|xss_clean');
              $this->form_validation->set_rules('spec_item_flag', 'For Specification', 'trim|xss_clean');
              $this->form_validation->set_rules('type_of_supply', 'Type', 'trim|xss_clean');
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                $data = array(
                  ...
                  'article_name_info.adr_category_id' => $this->input->post('customer_category'),
                  'article.excise_rate_id' => $this->input->post('tariff'),
                  'article.uom' => $this->input->post('uom'),
                  'article.type_of_supply' => $this->input->post('type_of_supply'),
  ...
  public function check_inv_name($str)
  {
    ...
    if (!empty($str)) {
      ...
      if ($result == TRUE) {
        foreach ($result as $item) {
          $art_name =  $item['article_description'];
        }
      } else {
        //echo'0';
      }

      if ($article_name == $art_name) {
        $this->form_validation->set_message('check_inv_name', '{field} Already Exit');
        return false;
      } else {
        return TRUE;
      }
