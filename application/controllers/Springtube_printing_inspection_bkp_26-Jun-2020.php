        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            foreach($this->input->post('sr_no') as $sr_no => $sr_no_value){
              ...
              $this->form_validation->set_rules('printing_input_qty_'.$sr_no_value.'','Printing Input Qty '.$sr_no_value.'' ,'required|trim|xss_clean|max_length[10]');
                ...
                if($production_id!=''){
                  ...
                  foreach($this->input->post('sr_no') as $sr_no => $sr_no_value){
                    ...
                    $data=array(                      
                      ...
                      'printing_input_qty'=>$this->input->post('printing_input_qty_'.$sr_no_value),                      
                    ...
                    if($result){
                      ...
                      'sixth_layer_mb'=>$sleeve_mb_6,                           
                      'film_code'=>$film_code,
                      'aprint_wip_qty'=>$this->input->post('inspected_output_qty_'.$sr_no_value), 
              ...
              if($this->form_validation->run()==FALSE){
                ...
                foreach ($springtube_printing_inspection_details_result as $springtube_printing_inspection_details_row) {
                  ...
                  $data=array(
                    ...
                    'printing_input_qty'=>$springtube_printing_inspection_details_row->printing_input_qty,                    
                    'inspected_output_meters'=>$springtube_printing_inspection_details_row->inspected_output_meters,
                    'inspected_output_qty'=>$springtube_printing_inspection_details_row->inspected_output_qty,
                    // 'qc_ok_qty'=>$springtube_printing_inspection_details_row->qc_ok_qty,
                    // 'qc_hold_qty'=>$springtube_printing_inspection_details_row->qc_hold_qty,
                    // 'total_waste_qty'=>$springtube_printing_inspection_details_row->total_waste_qty,
                    'date'=>$springtube_printing_inspection_details_row->date,                    
                    //'qc_id'=>$springtube_printing_inspection_details_row->qc_id,
                    //'qc_remarks'=>$springtube_printing_inspection_details_row->qc_remarks
  ...
  public function article_check($str){
    ...
    public function jobcard_qty_check($inspected_output_qty,$jobcard_no){      
        ...
        foreach ($result_total_counter as  $total_counter_row) {
          ...
          echo $printing_input_qty=round($total_counter*2);
        }
            //echo $printing_input_qty=$this->input->post('printing_input_qty');
            ...
            if ($printing_input_qty >= ($inspected_output_qty+$total_inspected_output_qty_exist)){
              return TRUE;
            }else{
              $this->form_validation->set_message('jobcard_qty_check','The {field} is not more than Printing Input Qty .');
            return FALSE;
