      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            $data['capa_complaint_nature_master']=$this->common_model->select_one_active_record_with_limit('capa_complaint_nature_master',$this->session->userdata['logged_in']['company_id'],$abc="",'index',17,0);
      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            ...
            //$this->load->view('Home/nav',$data);
            //$this->load->view('Home/subnav');
            $data['doc']=array('doc_no'=>'Doc. No : QC/F/23','rev_no'=>'4','date'=>'11-05-2021');


            $capa_complaint_register_master = $data['capa_complaint_register_master'];
            
            foreach ($capa_complaint_register_master as $master_row){
              $date1 = $master_row->email_date;
            } 
 
            $date2 = '2023-05-15';

            if($date1 >= $date2){
               $this->load->view('Print/complaint-register-header',$data);
            }else{
