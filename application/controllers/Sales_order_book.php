        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              ...
              $data['order_master'] = $this->sales_order_book_model->active_record_my_oc('order_master', $search, $from = "", $to = "", $this->session->userdata['logged_in']['company_id'], $order_closed_arr = "", '2022-02-01', date('Y-m-d'), $cancelled_from_date = "", $cancelled_to_date = "", $this->session->userdata['logged_in']['user_id']);
              ...
              //$this->load->view('Loading/loading');
              $this->load->view(ucwords($this->router->fetch_class()) . '/active-title', $data);
              $this->load->view(ucwords($this->router->fetch_class()) . '/my-oc-list-form', $data);
              $this->load->view('Home/footer');
            } else {
              $data['note'] = 'No rights Thanks';
              $this->load->view('Home/header');
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->new == 1) {
              ...
              $this->form_validation->set_rules('po_date', 'Customer Purchase Order Date', 'required|trim|xss_clean|max_length[10]|callback_po_date');

              $this->form_validation->set_rules('ref_order_no', 'Referense SO', 'trim|xss_clean');

              if (!empty($this->input->post('for_stock')) && !empty($this->input->post('for_sampling'))) {
                $this->form_validation->set_rules('userfile', 'Po File', 'required');
              }
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                for ($i = 1; $i <= count($this->input->post('sr_no[]')); $i++) {
                  ...
                  } else if (substr($item_code[1], 0, 2) == "PB") {
                    $order_flag = 4;
                  } else {
                    $order_flag = 3;
                  }
                ...
                $filename = $sales_order_no . "_" . time() . "_" . preg_replace('/[^A-Za-z0-9.-]/ ', '', $_FILES['userfile']['name']);
                $config['upload_path'] = './assets/' . $this->session->userdata['logged_in']['company_id'] . '/po_file/';
                ...
                if ($result) {
                  ...
                  $data = array(
                    'total_value' => $this->common_model->save_number($net_amount, $this->session->userdata['logged_in']['company_id']),
                  ...
                  /*$data['employee']=$this->common_model->select_one_active_record('employee_master',$this->session->userdata['logged_in']['company_id'],'employee_id',$this->session->userdata['logged_in']['user_id']);
                        if($data['employee']==FALSE){
                          ...
                          foreach($data['employee'] as $employee_row) {
                                  ...
                                  $this->email->bcc('pravin.shinde@3d-neopac.com');
                                  $this->email->subject("".$this->input->post('order_no')." is Sent For Approval");
                                  $this->email->message("
                                    Followup Link
                                    http://192.168.0.33/erp/index.php/Sales_order_followup/");
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->modify == 1) {
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()) . '/active-title');
              $this->load->view(ucwords($this->router->fetch_class()) . '/send-mail-form', $data);
              $this->load->view('Home/footer');
            } else {
              $data['note'] = 'No Modify rights Thanks';
              $this->load->view('Home/header');
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->modify == 1) {
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()) . '/active-title');
              $this->load->view(ucwords($this->router->fetch_class()) . '/modify-form-delivery-date', $data);
              $this->load->view('Home/footer');
            } else {
              $data['note'] = 'No Modify rights Thanks';
              $this->load->view('Home/header');
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->modify == 1) {
              ...
              if ($for_sampling == 0 && (empty($this->input->post('po_file')) && empty($_FILES['userfile']['name']))) {
                $this->form_validation->set_rules('userfile', 'PO FILE', 'required');
              } else {
                //echo "File No required";
              }
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                $config['remove_spaces'] = TRUE;
                $filename = $this->input->post('order_no') . "_" . time() . "_" . preg_replace('/[^A-Za-z0-9.-]/ ', '', $_FILES['userfile']['name']);
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->modify == 1) {
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                $this->load->view('Home/subnav');
                $this->load->view(ucwords($this->router->fetch_class()) . '/active-title', $data);
                $this->load->view(ucwords($this->router->fetch_class()) . '/modify-delivery-form', $data);
                $this->load->view('Home/footer');
              } else {
                ...
                for ($i = 1; $i <= count($this->input->post('sr_no[]')); $i++) {
                  $delivery_date = $this->common_model->change_date_format($this->input->post('delivery_date_' . $i . ''), $this->session->userdata['logged_in']['company_id']);
                  $update_data = array('delivery_date' => $delivery_date);
                  $result = $this->common_model->update_one_active_record('order_details', $update_data, 'order_no', $this->input->post('order_no'), $this->session->userdata['logged_in']['company_id']);
                  ...
                  foreach ($data['employee'] as $employee_row) {
                    ...
                    $this->email->subject("Delivery Date of " . $this->input->post('order_no') . " is changed to " . $this->common_model->view_date($delivery_date, $this->session->userdata['logged_in']['company_id']) . "");
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              ...
              $this->load->view(ucwords($this->router->fetch_class()) . '/view-oc-form', $data);
              /*
...
*/
              ...
              if ($data['specification']) {
                ...
                if ($data['bill_of_material']) {
                  ...
                  foreach ($data['bill_of_material'] as $bill_of_material_row) {
                    ...
                    /*Changes Saurabh*/
                    $data['bom_guage_total'] = $this->sales_order_book_model->bom_gauge_total($bom_no, $bom_version_no);
                  }
  ...
  public function customer_check($str)
  {
    ...
    if (!empty($str)) {
      ...
      if (!empty($customer_code[1])) {
        $data = array(
          'address_master.adr_company_id' => $customer_code[1],
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()) . '/active-title', $data);
              $this->load->view(ucwords($this->router->fetch_class()) . '/search-po-form', $data);
              $this->load->view('Home/footer');
            } else {
              $data['note'] = 'No rights Thanks';
              $this->load->view('Home/header');
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              ...
              if (!empty($this->input->post('cancelled_from_date')) && !empty($this->input->post('cancelled_to_date'))) {

                $this->form_validation->set_rules('from_date', 'From Date', 'trim|xss_clean|max_length[10]');
                $this->form_validation->set_rules('to_date', 'To Date', 'trim|xss_clean|max_length[10]');
              }

              if (!empty($this->input->post('oc_from_date')) && !empty($this->input->post('oc_to_date'))) {

                $this->form_validation->set_rules('oc_from_date', 'OC From Date', 'trim|xss_clean|max_length[10]');
                $this->form_validation->set_rules('oc_to_date', 'OC To Date', 'trim|xss_clean|max_length[10]');
              }
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                if ($this->input->post('cancel_flag') != '') {
                  $search['cancel_flag'] = $this->input->post('cancel_flag');
                }
                ...
                $cancelled_from_date = '';
                if (!empty($this->input->post('cancelled_from_date'))) {
                  $cancelled_from_date = $this->common_model->change_date_format($this->input->post('cancelled_from_date'), $this->session->userdata['logged_in']['company_id']);
                }

                $cancelled_to_date = '';
                if (!empty($this->input->post('cancelled_to_date'))) {
                  $cancelled_to_date = $this->common_model->change_date_format($this->input->post('cancelled_to_date'), $this->session->userdata['logged_in']['company_id']);
                }

                $oc_from_date = '';
                if (!empty($this->input->post('oc_from_date'))) {
                  $oc_from_date = $this->common_model->change_date_format($this->input->post('oc_from_date'), $this->session->userdata['logged_in']['company_id']);
                }

                $oc_to_date = '';
                if (!empty($this->input->post('oc_to_date'))) {
                  $oc_to_date = $this->common_model->change_date_format($this->input->post('oc_to_date'), $this->session->userdata['logged_in']['company_id']);
                }


                $data['order_master'] = $this->sales_order_book_model->active_record_search_new('order_master', $search_data, $from, $to, $this->session->userdata['logged_in']['company_id'], $order_closed_arr, $approval_from_date, $approval_to_date, $cancelled_from_date, $cancelled_to_date, $oc_from_date, $oc_to_date);
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                // echo $search['customer_no'];

                $cust_order_no = $this->input->post('cust_order_no');

                $data['order_master'] = $this->sales_order_book_model->pending_orders('order_master', $cust_order_no, $customer_no);
                ...
                if ($data['order_master'] != FALSE) {
                  ...
                  $this->load->view(ucwords($this->router->fetch_class()) . '/active-title', $data);
                  $this->load->view(ucwords($this->router->fetch_class()) . '/search-po-form', $data);
                  $this->load->view(ucwords($this->router->fetch_class()) . '/search-po-result', $data);
                  $this->load->view('Home/footer');
                } else {
                  $data['page_name'] = 'Sales';
                  $data['module'] = $this->common_model->select_active_module($this->session->userdata['logged_in']['user_id'], $this->session->userdata['logged_in']['company_id']);
  ...
  public function compareDate($str)
  {
    if (!empty($str)) {
      ...
      if ($days <= 1) {
        $this->form_validation->set_message('compareDate', 'The delivery date should be greather than 1 days');
        return FALSE;
      } else {
        return TRUE;
      }
  ...
  public function po_date($str)
  {
    if (!empty($str)) {
      $po_date = strtotime($str);
      $todays_date = strtotime(date("Y-m-d"));
      $datediff = $todays_date - $po_date;
      $days = floor($datediff / (60 * 60 * 24));
      if ($days < 0) {
        $this->form_validation->set_message('po_date', 'Enter the correct PO Date');
        return FALSE;
      } else if ($days >= 7) {
        $this->form_validation->set_message('po_date', 'PO Date  allowed upto last 7 Days');
        return FALSE;
      } else {
        return TRUE;
      }
    ...
    if ($data['module'] != FALSE) {
      foreach ($data['module'] as $module_row) {
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              //echo $this->input->post('to_mails');

              if (!empty($this->input->post('to_mails[]'))) {
                $to_mails = implode(', ', $this->input->post('to_mails[]'));
              } else {
                $to_mails = "";
              }
              ...
              //echo $to_mails;
              $this->form_validation->set_rules('order_no', 'Order No', 'required|trim|xss_clean');
              $this->form_validation->set_rules('from_mail', 'From', 'required|trim|xss_clean');
              $this->form_validation->set_rules('to_mails[]', 'To ', 'required|trim|xss_clean');

              $this->form_validation->set_rules('cc', 'CC', 'required|trim|xss_clean|valid_emails');
              ...
              if ($this->form_validation->run() == FALSE) {
                ...
                $order_master = $this->sales_order_book_model->select_one_active_record('order_master', $this->session->userdata['logged_in']['company_id'], 'order_master.order_no', $this->input->post('order_no'));
                $order_details = $this->sales_order_book_model->active_details_records('order_details', array('order_no' => $this->input->post('order_no')), $this->session->userdata['logged_in']['company_id']);

                $this->load->model('tax_grid_model');
                $tax_master = $this->tax_grid_model->select_sales_order_tax_grid($this->input->post('order_no'));
                ...
                foreach ($order_master as $order_master_row) :
                  ...
                  <body>';
                  $email_content .= "<pre>" . $this->input->post('message') . "</pre><br/>";
                  ...
                  <div style='text-align:center;padding:15px;font-size:10px;'><span style='background-color:black;padding:4px;color:white;font-weight:bold;'>ORDER CONFIRMATION</span><br/><br>
                      <span style='background-color:black;padding:4px;color:white;font-weight:bold;'>DELIVERY BETWEEN " . $this->common_model->view_date($order_master_row->oc_date, $this->session->userdata['logged_in']['company_id']) . " TO " . $this->common_model->view_date(date('Y-m-d', strtotime("+3 day", strtotime($order_master_row->oc_date))), $this->session->userdata['logged_in']['company_id']) . "</span></div>
                      <br/>
                  <div>           
                  ...
                  $email_content .= '</table>
                          ...
                          <table cellpadding="5">
                              <tr >
                                  ...
                                  <th width="10%" style="text-align:center;">SPEC</th>
                                  <th width="10%" style="text-align:center;">QUANTITY</th>
                                  <th width="10%" style="text-align:center;">UNIT RATE</td>
                                  <th width="10%" style="text-align:center;">NET AMOUNT  ' . (!empty($order_master_row->currency_id) ? '(' . $order_master_row->currency_id . ')' : '') . '</th>';
                  ...
                  foreach ($tax_master as $tax_value) {
                    $tax_arr[$i] = 0;
                    $email_content .= "<th width='15%' colspan='2' width='10%' style='text-align:center;'>" . strtoupper($tax_value->lang_tax_code_desc) . "</th>";
                    $i++;
                  }

                  $email_content .= '<th width="10%" style="text-align:center;">TOTAL' . (!empty($order_master_row->currency_id) ? '(' . $order_master_row->currency_id . ')' : '') . '</th>
                              </tr>
                              <tr>
                                  <th colspan="6"></th>
                                  ';
                  ...
                  foreach ($order_details as $order_details_row) {
                    ...
                    if (!empty($order_details_row->spec_id)) {
                      if (substr($order_details_row->spec_id, 0, 1) == "S") {
                        $email_content .= "<b>" . $order_details_row->spec_id . "_" . $order_details_row->spec_version_no . "</b>";
                      } else {
                        $bom = array(
                    ...
                    if ($order_master_row->for_export == 1) {
                      $email_content .= "<td width='10%' style='text-align:center;'>" . $order_details_row->calc_sell_price . "</td>";
                    } else {
                      $email_content .= "<td width='10%' style='text-align:center;'>" . $this->common_model->read_number($order_details_row->selling_price, $this->session->userdata['logged_in']['company_id']) . "</td>";
                    }

                    $email_content .= "<td width='10%' style='border-top:1px solid #D9d9d9;border-right:1px solid #D9d9d9;text-align:center'>" . money_format('%.0n', $amount) . "</td>";
                    $m = 0;
                    $k = 0;
                    foreach ($tax_master as $tax_value) {
                      $output = array();
                      ...
                      $flag = 0;
                      $out = array();
                      $email_content .= "<td style='border-top:1px solid #D9d9d9;border-right:1px solid #D9d9d9;text-align:center'>" . $this->common_model->read_number($tax_value->tax_rate, $this->session->userdata['logged_in']['company_id']) . "%</td><td style='text-align:center;'>";
                      ...
                      if (!empty($out)) {
                        ...
                        if ($t_amount[$k] == '') {
                          ...
                          $email_content .= money_format('%.0n', $t_amount[$k]);
                        }
                    ...
                    $email_content .= "<td style='border-top:1px solid #D9d9d9;text-align:center'>" . money_format('%.0n', $this->common_model->read_number($order_details_row->total_selling_price, $this->session->userdata['logged_in']['company_id'])) . "</td>";
                  ...
                  $email_content .= "<tr >
                                          ...
                                          <td style='text-align:center;'><b>" . money_format('%!.0n', $this->common_model->read_number($total_quantity, $this->session->userdata['logged_in']['company_id'])) . "/-</td>
                                          <td></td>
                                          <td style='text-align:center;'><b>" . money_format('%.0n', $total_amount) . "/-</td>";
                  $l = 0;
                  foreach ($tax_master as $tax_value) {
                    $email_content .= "<td></td>
                                                  <td style='text-align:center;'><b>" . money_format('%.0n', $tax_arr[$l]) . "/-</td>";
                    $l++;
                  }

                  $email_content .= "<td style='text-align:center;'><b>" . money_format('%.0n', $this->common_model->read_number($total_selling_price, $this->session->userdata['logged_in']['company_id'])) . "/-</td>
                                      </tr>
                                </table>";
        ...
        <p>
            Terms of your PO not in conflict with the OC shall stand as accepted.
        </p>

        <p>
            No response to this order confirmation within working 5 days will be considered as acceptance of Order Confirmation</p>


        <p>
            <b>Other Condition of Performance</b></p>

        <p>
            The Customer shall verify such studies and recommendations under its own control and responsibility, and shall ensure, prior to the design and manufacture of the Product or performance of the service, that the Product is compatible with its contemplated use, if need be by appropriate testing.
        </p>

        <p>
            To check the suitability of the tube with respect to the product, 3D technopack recomends customers to carry out compatibility / stability tests .  3D technopack does not take any responsibility for the effect of the bulk on the tube in case of missing compatibility / stability tests.
        </p>

        <p>
            The customer accepts full responsibility for ensuring that the artworks approved by him does not infringe the rights of a third party, especially copyright and proprietary rights. The customer shall indemnify us fully against any damage which may be sustained by us in connection with any infringement of the rights of third parties resulting from the fulfilment of his order.
        </p>

        <p>
            All terms herein, and in your Purchase order, shall be invalid if the manufacturing or delivery period is affected by Force Majeure or Strikes, Riots and Civil Commotion, including transportation strikes; any terms in your PO referencing ‘time is of the essence’ or the equivalent are specifically stricken by this OC.
        </p>


        <p>
            The referenced PO is non-cancellable and non-deferrable, unless confirmed by us in writing, except within the earlier of 7 days from this OC/ the commencement of manufacture by 3DT its suppliers or affiliates (hereinafter 3DT) of parts or components required to fulfil your PO/ the order by 3DT of parts /components required to fulfil your PO.
        </p>



        <p>
            3D takes no responsibility for label lift off unless the label stock that is used meets our standard for adhesion and rolls are properly wound with even tension. Furthermore, 3D takes no responsibility for printing defects on labels and any delay caused by this or other label issues that are properly the responsibility of the label vendor, whether such vendor was selected by you or by us.           
            
        </p>

        <p>
            Labels must be cut to the shape specified in 3D Standard KLD and must end 15mm below the cut length of the tube unless the glue has the tack value to hold against the bend of the tube at the sealing area.
            
        </p>

        <p>
            Any warranty for defects shall remain valid only from the date of manufacture and not from the date of shipment
        </p>

        <p>
            All warranties are expressly tied to our AQL agreement.
        </p>
        ...
        <p>
            The Products shall be transported at the Customer’s risks, even in the case where shipping is undertaken at 3D Technopack expenses.Delays in delivery, specifically those as a result of acts of God or as a result of unforeseeable circumstances not attributable to us, like disruption in the work of the factory, strikes, closing of the factory, a lack of transport, administrative decrees, do not count as delays in fulfillment of contract by us.
        </p>

        <p>
            3D Technopack Pvt Ltd (hereinafter 3DT) shall inform you within one week of the incidence if there is a long-term breakdown of a machine, that could result in a delay of your material, and shall seek to reschedule delivery to the satisfaction of both parties.
        </p>
        <p>
            The tubes referenced in this OC will be despatched from our factory within the dates referenced herein unless otherwise agreed in writing (including by email) between us.
        </p>

        <p>
            If the actual date of delivery is delayed by more than two weeks (from the 1st date) after the date of delivery on this OC you may cancel or defer without penalty, which shall not invite monetary or other penalties, and 3D shall bear the costs of materials manufactured specifically for such PO (Stocking Orders will continue to be payable in full for up to the agreed maximum quantity).
            
        </p>
        <p>
            In the event of an agreed cancellation without a delay of more than two weeks (from the 1st date), or deferral 3DT shall promptly bill you for materials made, procured, or ordered by it, and you shall make payments for such invoices on your normal terms for the supply of goods. Caps made by 3D shall be charged at the prices prevailing for third party caps of the same or similar specification.
            
        </p>
        <p>
            Any changes (Eg- Quantity, artwork, product, Specification etc.) in your Purchase Order made by you will result in a new OC with a new date.
        </p>
        <p>
            If customer does not have stocking agreements with 3DT then 3DT shall also provide you with warehousing facility for tubes completed including capping (but not for tube components Viz. Shoulders, caps, sleeves etc) for maximum of 3 months at cost of Rs.125/Pallet towards rental with additional fixed handling charges of Rs. 125/pallet
        </p>
        <p>
            All component purchases for your PO’s are expressly tied to our Stocking Agreement if applicable.
        </p>
        <p>
            3D shall take no responsibility for delays in outsourced material and its impact on the committed date of delivery on your order.</p>

        <p>
            <b>Payments</b></p>

        <p>Payment should be release as per the PO agreed term. Delay in Payment will likely to attract interest rate of 2% per month.Without prior intimation.</p>
        <p>
            You are requested to release payment as per agreed PO terms. Delays in payment beyond agreed payment terms are likely to attract interest at 2% per month without prior intimation.
        </p>

        <p>
            <b>Intellectual and Industrial property</b></p>

        <p>Artwork provided by the customer will be processed by us to suit particular printing operations. 3D Technopack is liable to return customer provided artworks and not our processed artworks.</p>
                                      <br/>
                                      <p style='text-align:center;'><b>If any query contact to sales@3d-neopac.com</b></p>
                                      <p style='text-align:center;'><b>This is an electronically generated document, no signature required.</b></p>
                  ...
                  if ($this->email->send()) {
                    ...
                    $data_update = array('oc_sent_flag' => '1', 'oc_sent_by' => $this->session->userdata['logged_in']['user_id']);
                    $this->common_model->update_one_active_record('order_master', $data_update, 'order_no', $this->input->post('order_no'), $this->session->userdata['logged_in']['company_id']);

                    $insert_data = array('order_no' => $this->input->post('order_no'), 'company_id' => $this->session->userdata['logged_in']['company_id'], 'to_email' => $to_mails, 'cc_email' => $this->input->post('cc'), 'mail_sent_on' => time(), 'status' => 'Sent', 'user_id' => $this->session->userdata['logged_in']['user_id']);
                    $this->common_model->save('order_confirmation_emails', $insert_data);
                    ...
                    print_r($errors);
                    $insert_data = array('order_no' => $this->input->post('order_no'), 'company_id' => $this->session->userdata['logged_in']['company_id'], 'to_email' => $to_mails, 'cc_email' => $this->input->post('cc'), 'mail_sent_on' => time(), 'status' => 'Failed', 'user_id' => $this->session->userdata['logged_in']['user_id']);
                    $this->common_model->save('order_confirmation_emails', $insert_data);
                  }
                endforeach;
              }
        ...
        if ($module_row->module_name === 'Sales') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              ...
              // New Condition is added on 4th aug 2021 to Cancel the orders by Admin [Jobcard created] 

              if ($supplyqty == 0 && $jobcard_generated_flag == 1 && $for_stock_flag == 0 && $this->session->userdata['logged_in']['admin'] == 1) {
              ...
              */
                ...
                if (in_array($this->input->post('trans_closed_reason'), $order_closed_arr)) {
                  ...
                  if ($this->input->post('trans_closed_reason') == '12') {
                    ...
                    $to = array('suryanarayan.swain@3d-neopac.com', 'mathew.kutty@3d-neopac.com', 'ramakant.swami@3d-neopac.com', 'mukesh.patel@3d-neopac.com', 'michael.wagner@3d-neopac.com');
                    $subject = strtoupper($order_no . ' is ' . $reasons);
                    $body = 'SHORT CLOSED';
                    $data_update = array(
                      'trans_closed_reason' => $this->input->post('trans_closed_reason'),
                      'trans_closed_remarks' => $this->input->post('trans_closed_remarks'),
                    ...
                    if ($result) {
                      $cc_arr = array("erp@3d-neopac.com", "mayur.joshi@3d-neopac.com", "anant.gadre@3d-neopac.com", "prasad.sathe@3d-neopac.com", "" . $this->common_model->get_user_email($this->session->userdata['logged_in']['user_id'], $this->session->userdata['logged_in']['company_id']) . "", "manish.thalia@3d-neopac.com", "rohit.patil@3d-neopac.com", "atul.singh@3d-neopac.com");
                      $email_send = $this->on_order_closed_send_email($order_no, $to, $subject, $body, $cc_arr);
                      if ($email_send == 1) {
                        echo "Email sent";
                      } else {
                        echo "Email not sent";
                  ...
                  $subject = strtoupper($order_no . ' ' . $reasons);
                  $body = 'CANCEL ORDER';
                  if ($result) {
                    $cc_arr = array("erp@3d-neopac.com", "mayur.joshi@3d-neopac.com", "anant.gadre@3d-neopac.com", "suryanarayan.swain@3d-neopac.com", "manish.thalia@3d-neopac.com", "ramakant.swami@3d-neopac.com", "mukesh.patel@3d-neopac.com", "rohit.patil@3d-neopac.com", "atul.singh@3d-neopac.com", "michael.wagner@3d-neopac.com");
                    $email_send = $this->on_order_closed_send_email($order_no, $to, $subject, $body, $cc_arr);
                    if ($email_send == 1) {
                      echo "Email sent";
                    } else {
  ...
  public function createExcel()
  {
    ...
    if ($this->uri->segment(4) != "CUST.NO.") {
      $arr = explode('_', $this->uri->segment(4));
      $customer_no = $arr[1];
      // echo $customer_no;
    } else {
      $customer_no = "";
    ...
    $order_data = $this->sales_order_book_model->pending_orders('order_master', ($this->uri->segment(3) == "PO.NO." ? "" : str_replace("_", "/", $this->uri->segment(3))), ($this->uri->segment(4) == "CUST.NO." ? "" : $customer_no));

    // $order_data = $this->sales_order_book_model->pending_orders('order_master',str_replace("_","/", $this->uri->segment(3)),$customer_no);  
    ...
    $table_coloumns = array("Customer", "PO NO");
    $column = 0;

    foreach ($table_coloumns as $field) {
    ...
    foreach ($order_data as $val) {
      $object->getActiveSheet()->setCellValueByColumnAndRow(0, 2, $this->common_model->get_customer_name($val->customer_no, $this->session->userdata['logged_in']['company_id']));
    }


    $object->getActiveSheet()->setCellValueByColumnAndRow(1, 2, str_replace("_", "/", $this->uri->segment(3)));

    $table_coloumns = array("Order No", "PRODUCT CODE", "PRODUCT NAME", "ORDER QTY", "PENDING QTY", "OC Date");
    ...
    foreach ($order_data as $val) {
      $details_data = array('order_no' => $val->order_no);
      $result = $this->sales_order_book_model->active_details_records_new('order_details', $details_data, $this->session->userdata['logged_in']['company_id'], $delivery_from_date = "", $delivery_to_date = "");
      $article_no = "";
      $order_quantity = 0;
      foreach ($result as $drow) {
        $article_no = $drow->article_no;
        $order_quantity = $drow->total_order_quantity;
      }

      $object->getActiveSheet()->setCellValueByColumnAndRow(0, $excel_row, $val->order_no);
      $object->getActiveSheet()->setCellValueByColumnAndRow(1, $excel_row, $val->cust_product_no);
      $object->getActiveSheet()->setCellValueByColumnAndRow(2, $excel_row, $this->common_model->get_article_name($article_no, $this->session->userdata['logged_in']['company_id']));
      $object->getActiveSheet()->setCellValueByColumnAndRow(3, $excel_row, $this->common_model->read_number($order_quantity, $this->session->userdata['logged_in']['company_id']));
      $object->getActiveSheet()->setCellValueByColumnAndRow(4, $excel_row, $val->pending);
      $object->getActiveSheet()->setCellValueByColumnAndRow(5, $excel_row, ($val->oc_date == "0000-00-00" ? "-" : $val->oc_date));
      $excel_row++;
    }
    ...
    $object->getActiveSheet()->getMergeCells('A1:D1');

    $object_writer = PHPExcel_IOFactory::createWriter($object, 'CSV');
    header('Content-Type: application/vnd.ms-excel');
    header('Content-Disposition: attachment;filename="po.csv"');
    ...
    if (empty($this->input->get('from_date')) && empty($this->input->get('to_date'))) {
      ...
      foreach ($data['order_master'] as $key => $order_master_row) {
        ...
        foreach ($data['order_details'] as $key => $order_details_row) {
          ...
          $cancel_qty = 0;
          $search_arr = array(
            'ref_ord_no' => $order_details_row->order_no,
  ...
  function order_check($order_no)
  {
    ...
    if ($this->session->userdata['logged_in']['admin'] == 1) {
      return true;
    } else if ($for_stock_flag == 0 && $for_sampling == 0 && $jobcard_generated_flag == 1 && $supply_qty == 0 && $this->session->userdata['logged_in']['admin'] != 1) {
    ...
    // CASE 7: COMMERCIAL SO, JOBCARD GENERATED, NO DISPATCH -- Admin Can Cancle the Order on request
    else if ($for_stock_flag == 0 && $for_sampling == 0 && $jobcard_generated_flag == 1 && $supply_qty == 0 && $this->session->userdata['logged_in']['admin'] == 1) {
      return true;
    }
  }
  ...
  function on_order_closed_send_email($order_no, $to, $subject, $body, $cc)
  {
    ...
    foreach ($order_master as $order_master_row) :
      ...
      <div style='padding:20px;background-color:white;'>  
        ...
        <div style='margin-top:20px;'>
          <table cellpadding='5'>          
            <tr>
              <th width='15%'>SALES ORDER NO</td>
              <th width='35%'>" . "<a href='" . base_url('index.php/sales_order_book/view/' . $order_master_row->order_no) . "' target='_blank'>" . $order_master_row->order_no . "</td>
              <th width='15%'>PO NO</td>
              <th width='35%'>" . $order_master_row->cust_order_no . ($order_master_row->cust_product_no != '' ? $order_master_row->cust_product_no : '-') . "</td>
            </tr>
            <tr>
      ...
      $email_content .= '
            <tr>
              <td>ACCEPTANCE TOLERANCE</td>
              <td style="border-right:1px solid #D9d9d9;">' . $order_master_row->dispatch_tolerance . ' %</td>
              <td>PO FILE</td>
              <td>' . ($order_master_row->po_file != '' ? '<a href="' . base_url('assets/' . $this->session->userdata['logged_in']['company_id'] . '/po_file/' . $order_master_row->po_file . '') . '" target="_blank">View File</a>' : '') . '</td>
            </tr>
            </table>
  ...
  */
      ...
      $total_a_quantity = $total_quantity + ($total_quantity / 100) * $order_master_row->dispatch_tolerance;
      ...
      if ($invoice_result == FALSE) {
        ...
        foreach ($invoice_result as $invoice_row) {
          $email_content .= "<tr >
              ...
              <td ><a href='" . base_url('index.php/costsheet/view/' . $invoice_row->ar_invoice_no . '/' . $order_master_row->order_no . '/' . $invoice_row->article_no) . "' target='_blank'>" . $invoice_row->ar_invoice_no . "</a></td>              
        ...
        $short_supply = ($total_a_quantity > $total_invoice_qty ? abs($total_a_quantity - $total_invoice_qty) : 0);
    ...
    <table cellpadding="5" cellspacing="0">
      <tr >
          <th colspan="10">ORDER SUMMARY</th>
      </tr>
      <tr>
        <th>SR NO</th>                  
        <th>ORDER NO</th>                
      ...
      <tr>
        ...
        <td>' . round(($short_supply / $total_a_quantity) * 100) . '</td>
    ...
    //$emailid_arr=array($user_email,"erp@3d-neopac.com","mayur.joshi@3d-neopac.com","anant.gadre@3d-neopac.com","manish.thalia@3d-neopac.com","ramakant.swami@3d-neopac.com");
  ...
  /* Order Confirmation Reminder Start */


  function oc_reminder()
  {
    $result = $this->sales_order_book_model->sales_user_model();
    if ($result == true) {
      foreach ($result as $row) {
        $from_date = date("y-m-d");
        $to_date =  date('Y-m-d', strtotime($from_date . ' + 4 days'));
        ...
        if ($data == true) {
          ...
          $email_content = ' <table cellpadding="5" cellspacing="8">
                                     <tr>
                                       <th>OC Date </th>
                                       <th>Customer </th>                
                                       <th>Order No</th>                
  ...
  /* Order Confirmation Reminder End */

  public function get_ajax_data()
  {
    //$psm_no=explode(//',$this->input->post('product_name_1'));
    //echo $this->input->post('product_name_1');
    //$psm_no=explode('//',$this->input->post('product_name_1'));  
    //echo $psm_no[1];

    $psm_no = $this->input->post('product_name_1');
    $this->sales_order_book_model->get_ajax_data_mismatch($psm_no);
    //echo $this->db->last_query();        
  }
}
