...
class Coex_extrusion_heading_scrap extends CI_Controller {
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->view==1){
              $table='coex_extrusion_heading_scrap';
              include('pagination.php');
              $data['coex_extrusion_heading_scrap']=$this->coex_extrusion_heading_model->select_active_records_heading_scrap($config["per_page"],$this->uri->segment(3),$table,$this->session->userdata['logged_in']['company_id']);
