...
class Springtube_bodymaking_production extends CI_Controller {
      ...
      if($module_row->module_name==='Production'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            $this->form_validation->set_rules('shift','Shift' ,'required|trim|xss_clean|callback_check_duplicate_jobcard');            
            ...
            foreach($this->input->post('sr_no') as $sr_no => $sr_no_value){
                ...
                //echo $this->input->post('total_sleeve_produced_'.$sr_no_value.'');

                //echo "<br/>";
                
                $this->form_validation->set_rules('jobcard_no_'.$sr_no_value.'','Jobcard No '.$sr_no_value.'' ,'required|trim|xss_clean|max_length[50]');

                // $this->form_validation->set_rules('jobcard_no_'.$sr_no_value.'','Jobcard No '.$sr_no_value.'' ,'required|trim|xss_clean|max_length[50]|callback_material_issue_check');
                ...
                $this->form_validation->set_rules('sleeve_with_heading_'.$sr_no_value.'','Sleeve with heading'.$sr_no_value.'' ,'required|trim|xss_clean|max_length[10]|less_than_equal_to['.$this->input->post('total_sleeve_produced_'.$sr_no_value.'').']');              
                $this->form_validation->set_rules('sleeve_with_cap_'.$sr_no_value.'','Sleeve with cap'.$sr_no_value.'' ,'required|trim|xss_clean|max_length[10]|less_than_equal_to['.$this->input->post('total_sleeve_produced_'.$sr_no_value.'').']');
                ...
                if($production_id!=''){
                  ...
                  foreach($this->input->post('sr_no') as $sr_no => $sr_no_value){
                    ...
                    $data=array(                      
                      ...
                      'date'=>date('Y-m-d'), 
                      'bodymaking_done'=>(!empty($this->input->post('bodymaking_done_'.$sr_no_value) ?1:0))

                    );                

                    $result=$this->common_model->save('springtube_bodymaking_production_details',$data); 

                    // Bodymaking Done status is added 22-Mar-2021

                    if(!empty($this->input->post('bodymaking_done_'.$sr_no_value)) && $this->input->post('bodymaking_done_'.$sr_no_value)==1 ){
                      
                      $data=array('bodymaking_done'=>1);
          ...
          foreach ($data['formrights'] as $formrights_row) {
            ...
            if($formrights_row->modify==1){
              ...
              foreach($this->input->post('sr_no') as $sr_no => $sr_no_value){
                ...
                //$this->form_validation->set_rules('total_sleeve_produced_'.$sr_no_value.'','Total Sleeve Produced (Qty)'.$sr_no_value.'' ,'required|trim|xss_clean|max_length[10]|callback_jobcard_sleeve_check['.$this->input->post('total_sleeve_produced_'.$sr_no_value.'').']');	

                $this->form_validation->set_rules('total_sleeve_produced_'.$sr_no_value.'','Total Sleeve Produced (Qty)'.$sr_no_value.'' ,'required|trim|xss_clean|max_length[10]');    
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->delete==1){
              ...
              foreach ($data['springtube_bodymaking_production_details'] as $details_row) {
                ...
                $data=array('archive'=>'1');
                $result=$this->common_model->update_one_active_record('springtube_bodymaking_wip_master',$data,'details_id',$details_id,$this->session->userdata['logged_in']['company_id']);
          ...
          if($formrights_row->view==1){
            ...
            if($this->form_validation->run()==FALSE){
              ...
              $dataa=array(                  
                  ...
                  'springtube_bodymaking_production_master.shift'=>$this->input->post('shift')
                );
                ...
                $this->load->view('Home/footer');
                  ...
                  $specs_result=$this->sales_order_book_model->select_film_specs_record('specification_sheet_details',$this->session->userdata['logged_in']['company_id'],$specs);
                    ...
                    if($qc_ok_qty>0){
                      ...
                      'sixth_layer_mb'=>$sleeve_mb_6,                           
                      'film_code'=>$film_code,                                          
                      'bm_wip_qty'=>$this->input->post('qc_ok_qty'),
                      'created_date'=>date('Y-m-d h:i:s'),
                      'company_id'=>$this->session->userdata['logged_in']['company_id'],
                      'from_process'=>'5',
                      'user_id'=>$this->session->userdata['logged_in']['user_id']
                      ...
                      $result=$this->common_model->save('springtube_bodymaking_wip_master',$data);
  ...
  function article_check($str){
    ...
    function jobcard_sleeve_check($total_sleeve_produced,$jobcard_no){    

            //echo   $jobcard_no;
            //echo "<br/>";
            //echo $total_sleeve_produced;
            //echo "<br/>";

          if($total_sleeve_produced!=''){
            
           /* $jobcard_no="";
            if($this->input->post('sr_no')==TRUE){
              foreach($this->input->post('sr_no') as $sr_no => $sr_no_value){
              $jobcard_no=$this->input->post('jobcard_no_'.$sr_no_value.'');
            }
              }else{
              $this->form_validation->set_message('jobcard_sleeve_check','Please enter Jobcard in {field}.');
              return FALSE;
            }*/
    ...
    function shift_check_new($production_date){
  ...
  function material_issue_check($jobcard_no){
    ...
    if($material_manufacturing_result==FALSE){
      ...
      foreach ($material_manufacturing_result as $material_manufacturing_row){
        if($material_manufacturing_row->work_proc_no=='11'|| $material_manufacturing_row->work_proc_no=='2'){
    ...
    if($material_pending==1){      
      $this->form_validation->set_message('material_issue_check', 'Cap/Shoulder Material issue is pending for Jobcard no. '.$jobcard_no.', So entry not allowed');
        return FALSE;
    }else{
      return TRUE;
    }  
  ...
  function check_duplicate_jobcard(){

    $a=$this->input->post('sr_no');
    //print_r($a);
    $b=array_unique($this->input->post('sr_no'));
    //print_r($b);
    if(count($a)!==count($b)){               
      $this->form_validation->set_message('check_duplicate_jobcard', 'Duplicate jobcards not allowed in same shift entry');   
      return FALSE;  
    }else{
      return TRUE;
    }
