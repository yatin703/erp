...
class Bill_of_material_followup extends CI_Controller {

	function __construct(){
    parent::__construct();
    if($this->session->userdata('logged_in')){
      $this->load->model('common_model');
      $this->load->model('bill_of_material_followup_model');
    ...
    foreach ($data['module'] as $module_row) {
    	if($module_row->module_name==='Followup'){
          ...
          if($formrights_row->view==1){
            $table='followup';
            $data['followup_received']=$this->bill_of_material_followup_model->select_followup_received_records($table,$this->session->userdata['logged_in']['company_id'],'followup.user_id',$this->session->userdata['logged_in']['user_id'],'status','1','followup.form_id','791');

            $data['followup_sent']=$this->bill_of_material_followup_model->select_followup_received_records('followup',$this->session->userdata['logged_in']['company_id'],'followup.contact_person_id',$this->session->userdata['logged_in']['user_id'],'status','1','followup.form_id','791');
           ...
           // $data=array('followup.status'=>'999','followup.approved_flag'=>'2','followup.form_id'=>'791','followup.user_id'=>$this->session->userdata['logged_in']['user_id']);
            //$order_by=array('followup.transaction_no'=>'desc');

           // $data['followup_rejected']=$this->bill_of_material_followup_model->select_followup_rejected_records($table,$this->session->userdata['logged_in']['company_id'],$data,$order_by);
      ...
      if($module_row->module_name==='Followup'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            $result=$this->common_model->update_one_active_record_where('bill_of_material',$data,'bom_no',$this->uri->segment(3),'bom_version_no',$this->uri->segment(4),$this->session->userdata['logged_in']['company_id']);
            ...
            $data['followup_received']=$this->bill_of_material_followup_model->select_followup_received_records('followup',$this->session->userdata['logged_in']['company_id'],'followup.user_id',$this->session->userdata['logged_in']['user_id'],'status','1','followup.form_id','791');
            $data['followup_sent']=$this->bill_of_material_followup_model->select_followup_received_records('followup',$this->session->userdata['logged_in']['company_id'],'followup.contact_person_id',$this->session->userdata['logged_in']['user_id'],'status','1','followup.form_id','791');
            $data['page_name']='Followup';
            $data['module']=$this->common_model->select_active_module($this->session->userdata['logged_in']['user_id'],$this->session->userdata['logged_in']['company_id']);
             $data['formrights']=$this->common_model->select_active_formrights_of_form($this->session->userdata['logged_in']['user_id'],$this->session->userdata['logged_in']['company_id'],7,$this->router->fetch_class());
