          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->new==1){
             ...
             //echo $this->db->last_query();
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()).'/active-title',$data);
              $this->load->view(ucwords($this->router->fetch_class()).'/outsourced-create-form',$data);
              $this->load->view('Home/footer');
            }else{
              $data['note']='No rights Thanks';
              $this->load->view('Home/header');
                ...
                if($production_id!=''){
                  ...
                  foreach($this->input->post('sr_no') as $sr_no => $sr_no_value){
                    ...
                    if(!empty($this->input->post('inspection_done_'.$sr_no_value)) && $this->input->post('inspection_done_'.$sr_no_value)==1 ){
                      ...
                      $this->email->to('ramakant.swami@3d-neopac.com');
                      $this->email->subject('Inspection Completed for '.$jobcard_no.'');
                      $this->email->message("Hi, FYI");
                       if ($this->email->send()){
