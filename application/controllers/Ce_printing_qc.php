...
class Ce_printing_qc extends CI_Controller {
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->view==1){
              $table='coex_extrusion_printing_qc';
              include('pagination.php');
              $data['coex_extrusion_printing_qc']=$this->coex_extrusion_printing_model->select_active_records_printing_qc($config["per_page"],$this->uri->segment(3),$table,$this->session->userdata['logged_in']['company_id']);
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->modify==1){
                  ...
                  $data['hold_data']=$this->coex_extrusion_printing_model->select_one_active_record_hold('coex_extrusion_printing_qc',$this->session->userdata['logged_in']['company_id'],'coex_extrusion_printing_qc.prt_qc_id',$this->uri->segment(3));
                  $this->load->view('Home/header');
                  $this->load->view('Home/nav',$data);
                  $this->load->view('Home/subnav');
          ...
          if($formrights_row->new==1){
            ...
            if($this->form_validation->run()==FALSE){
              ...
              $data_wip=array(
                ...
                'qc_id'=> $this->input->post('prt_qc_id'),
              ...
              $result=$this->common_model->save('coex_extrusion_printing_scrap',$data_scrap);

              $data_ce=array(
                 'flag'=> '1',
                 'printing_date'=> date('Y-m-d')
              );
              $result=$this->common_model->update_one_active_record('coex_extrusion_printing_qc',$data_ce,'prt_qc_id',$this->input->post('prt_qc_id'),$this->session->userdata['logged_in']['company_id']);
