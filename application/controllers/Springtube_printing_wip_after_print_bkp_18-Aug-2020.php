            ...
            if($this->form_validation->run()===FALSE){
              ...
              if($result){
                ...
                if(!empty($cap_code)){                           
                  ...
                  $cap_result=$this->common_model->select_one_active_record('specification_sheet',$this->session->userdata['logged_in']['company_id'],'article_no',$cap_code);

                  foreach($film_code_result as $film_code_row){                   
                    $cap_spec_id=$film_code_row->spec_id;
                    $cap_spec_version=$film_code_row->spec_version_no;
                  }
                  ...
                  foreach ($cap_specs_result as $cap_specs_row) {
                    ...
                    $cap_mold_finish=$cap_specs_row->CAP_MOLD_FINISH;
                    $cap_orifice=$cap_specs_row->CAP_ORIFICE;
                    $cap_dia=$cap_specs_row->CAP_DIA;                  
                    $cap_shrink_sleeves=$cap_specs_row->CAP_SHRINK_SLEEVE_CODE;
              ...
              foreach ($springtube_printing_wip_master_after_result as $springtube_printing_wip_master_after_row ) {
                ...
                $data=array('status'=>'1','body_making_jobcard_created'=>'1','body_making_jobcard_no'=>$jobcard_no);
