...
function index(){
  ...
  /*public function get_coa()  { 
    if ($this->input->is_ajax_request()) {  
      $this->certificate_of_analysis_model->get_coa();
    }
  }*/
          ...
          if($formrights_row->new==1){
            ...
            if($this->form_validation->run()==FALSE){
              ...
              if(!empty($this->input->post('approval_by'))){
                    ...
                    $data['followup']=$this->common_model->select_one_active_record_nonlanguage_without_archive('followup',$this->session->userdata['logged_in']['company_id'],'record_no',$this->input->post('certificate_no'));
                    ...
                    if($data['followup']==FALSE){
                      ...
                      $data=array(
                        ...
                        'record_no'=>$this->input->post('certificate_no'));
    ...
    foreach ($data['module'] as $module_row) {
      if($module_row->module_name==='Production'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
            ...
            //$data['certificate_of_analysis']=$this->certificate_of_analysis_model->select_one_layer_active_records(' certificate_of_analysis',$this->session->userdata['logged_in']['company_id'],'od.order_no',$this->uri->segment(5),'bom.bom_no',$this->uri->segment(4),1,0);

            $data['certificate_of_analysis']=$this->certificate_of_analysis_model->get_coa_details_id($this->session->userdata['logged_in']['company_id'],$this->uri->segment(3));
            //echo $this->db->last_query();
      ...
      if($module_row->module_name==='Production'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->delete==1){
            ...
            //echo $this->db->last_query();
              $data['page_name']='Production';
             // $data['certificate_of_analysis']=$this->common_model->select_active_records($config["per_page"],$this->uri->segment(3),$table,$this->session->userdata['logged_in']['company_id']);
...
/*public function search_result(){
    ...
    if($this->form_validation->run()==FALSE){
      ...
      $this->load->view('Home/footer');
          ...
          $data['certificate_of_analysis']=$this->common_model->active_record_search('certificate_of_analysis',$data,$this->session->userdata['logged_in']['company_id']);
