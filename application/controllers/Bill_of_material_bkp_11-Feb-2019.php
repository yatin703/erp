      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            $this->form_validation->set_rules('comment','Comment' ,'required|trim|xss_clean|max_length[512]');
                ...
                foreach ($data['bom_version'] as $bom_version_row) {
                  ...
                  if($bom_version_row->bom_version_no==NULL){
                    ...
                    foreach($result as $row){
                      ...
                      $max_pkey=$max_pkey+1;
                      $bom_no=str_pad($max_pkey,4,0,STR_PAD_LEFT);
                      $bom_no="BOM".$bom_no;
                    }
                    $bom_version_no=1;
                  }else{
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
              ...
              if($this->form_validation->run()){
                ...
                $table='bill_of_material';                
                $data['bill_of_material']=$this->bill_of_material_model->active_records_search($table,$this->session->userdata['logged_in']['company_id'],$data,$from,$to);
