...
class Springtube_printing_wip_before_print extends CI_Controller {
      ...
      if($module_row->module_name==='Production'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            ...
            $this->form_validation->set_rules('return_flag','Is return' ,'trim|xss_clean'); 
            ...
            if($this->form_validation->run()==FALSE){
              ...
              if($this->input->post('return_flag')!=''){
                $data_search['springtube_printing_wip_master_before.return_flag']=$this->input->post('return_flag');
              }
  ...
  function save_printing_jobcard(){
      ...
      if($module_row->module_name==='Production'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            $this->form_validation->set_rules('release_qty','Printing Job Card Quantity' ,'required|trim|xss_clean|greater_than[0]|callback_printing_jobcard_quantity_check');
            ...
            if($this->form_validation->run()===FALSE){              
              ...
              $data_bwip_search=array('order_no'=>$this->input->post('order_no'),
                'article_no'=>$this->input->post('article_no'),
                'status'=>'0','archive'=>0,'return_flag'=>0);
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->view==1){
              ...
              $table='springtube_printing_wip_master_before';
              //include('pagination_wip.php');
              $data['springtube_printing_wip_master_before']=$this->springtube_printing_wip_before_print_model->select_pending_jobcards($table,$this->session->userdata['logged_in']['company_id']);
             
              $this->load->view('Home/header');
              $this->load->view('Home/nav',$data);
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()).'/active-title',$data);
              $this->load->view(ucwords($this->router->fetch_class()).'/pending-jobcards',$data);
              $this->load->view('Home/footer');
            }else{
              $data['note']='No rights Thanks';
              $this->load->view('Home/header');
   ...
   function wip_return($bprint_wip_id){
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->modify==1){
              ...
              $table='springtube_printing_wip_master_before';
              $data['springtube_printing_wip_master_before']=$this->common_model->select_one_active_record($table,$this->session->userdata['logged_in']['company_id'],'bprint_wip_id',$bprint_wip_id);
              
              $wip_id='';
              foreach ($data['springtube_printing_wip_master_before']  as $key => $row) {
                $wip_id=$row->ref_wip_id;
              }
              $data['springtube_extrusion_wip_master']=$this->common_model->select_one_active_record('springtube_extrusion_wip_master',$this->session->userdata['logged_in']['company_id'],'wip_id',$wip_id);
             ...
             // echo $this->db->last_query();
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()).'/active-title',$data);
              $this->load->view(ucwords($this->router->fetch_class()).'/wip-return-form',$data);
              $this->load->view('Home/footer');
            }else{
              $data['note']='No rights Thanks';
              $this->load->view('Home/header');
...
function wip_return_save(){
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->new==1){
              ...
              $this->form_validation->set_rules('bprint_wip_id','bprint_wip_id' ,'required|trim|xss_clean');       
              ...
              if($this->form_validation->run()==FALSE){
                ...
                // exit;
                  ...
                  $bprint_wip_id='';
                  $bprint_wip_id=$this->input->post('bprint_wip_id');
                  ...
                  foreach ($data['springtube_printing_wip_master_before']  as $key => $row) {
                    $wip_id=$row->ref_wip_id;
                    $print_jobcard_no=$row->print_jobcard_no;
                    $release_meters=$row->bprint_wip_meters;
                    $release_qty=$row->bprint_wip_qty;
                  }
                  ...
                  $result=$this->common_model->update_one_active_record('springtube_printing_wip_master_before',$data_update,'bprint_wip_id',$bprint_wip_id,$this->session->userdata['logged_in']['company_id']);

                  if($result && $print_jobcard_no!=''){

                    $data_jobcard=array('archive'=>'1');
                    $result=$this->common_model->update_one_active_record('production_master',$data_jobcard,'mp_pos_no',$print_jobcard_no,$this->session->userdata['logged_in']['company_id']);
                  ...
                  if($result){
                    ...
                    foreach ($data['springtube_extrusion_wip_master'] as $key => $row) {
                      ...
                      $data_insert=array(
                        ...
                        'total_ok_meters'=>$row->release_meters,
                        'ok_qty'=>$row->release_qty,
                        'wip_cost_per_meter'=>$row->wip_cost_per_meter,
                      ...
                      $result=$this->common_model->save('springtube_extrusion_wip_master',$data_insert);
                ...
                $data['springtube_extrusion_wip_master']=$this->common_model->select_one_active_record('springtube_extrusion_wip_master',$this->session->userdata['logged_in']['company_id'],'wip_id',$wip_id);
                  ...
                  $data['module']=$this->common_model->select_active_module($this->session->userdata['logged_in']['user_id'],$this->session->userdata['logged_in']['company_id']);
                  $data['formrights']=$this->common_model->select_active_formrights_of_form($this->session->userdata['logged_in']['user_id'],$this->session->userdata['logged_in']['company_id'],5,$this->router->fetch_class());                 
                  $data['note']='Return Transaction Completed'; 

                  header("refresh:1;url=".base_url()."index.php/".$this->router->fetch_class().'/pending_jobcards');
  ...
  function printing_jobcard_quantity_check(){

    $actual_qty_manufactured=0;
    $calc_jobcard_qty=0;
    $order_qty=$this->input->post('order_qty');
    $job_card_quantity=$this->input->post('release_qty');
    //Jobcard Perc check----------------              
    $job_card_perc='';
    ...
    $data=array('sales_ord_no'=>$this->input->post('order_no'),
              'article_no'=>$this->input->post('article_no'),
              'archive'=>'0',
              'jobcard_type'=>2);
    ...
    $this->form_validation->set_message('printing_jobcard_quantity_check', 'The {field} can not be more than specified %');
    return FALSE;
