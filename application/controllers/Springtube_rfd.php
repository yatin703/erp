...
class Springtube_rfd extends CI_Controller
{
        ...
        if ($module_row->module_name === 'Production') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->view == 1) {
              ...
              $data['rdf_data'] = $this->springtube_rfd_model->totalrfd();
              //echo $this->db->last_query();
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()) . '/active-title', $data);
              $this->load->view(ucwords($this->router->fetch_class()) . '/product-wise-form', $data);
              $this->load->view('Home/footer');
            } else {
              $data['note'] = 'No rights Thanks';
              $this->load->view('Home/header');
  ...
  /*function delete(){
      ...
      foreach ($data['module'] as $module_row) {
        if($module_row->module_name==='Production'){
          $data['formrights']=$this->common_model->select_active_formrights_of_form($this->session->userdata['logged_in']['user_id'],$this->session->userdata['logged_in']['company_id'],5,'springtube_rfd_master');
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->delete==1){
                ...
                $result=$this->common_model->archive_one_record('springtube_rfd_master',$data,'rfd_id',$this->uri->segment(3),$this->session->userdata['logged_in']['company_id']);

                $data['page_name']='Production';
                $data['module']=$this->common_model->select_active_module($this->session->userdata['logged_in']['user_id'],$this->session->userdata['logged_in']['company_id']);
                $data['formrights']=$this->common_model->select_active_formrights_of_form($this->session->userdata['logged_in']['user_id'],$this->session->userdata['logged_in']['company_id'],5,'springtube_rfd_master');
                $dataa = '';
                $data['springtube_rfd_master']=$this->common_model->select_active_records_where('springtube_rfd_master',$this->session->userdata['logged_in']['company_id'],$dataa);
      ...
      if($module_row->module_name==='Production'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->delete==1){
            ...
            $data['springtube_rfd_master']=$this->common_model->select_archive_records($config["per_page"],$this->uri->segment(3),$table,$this->session->userdata['logged_in']['company_id']);
            $data['page_name']='Production';
      ...
      if($module_row->module_name==='Production'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->dearchive==1){
            ...
            $result=$this->common_model->update_one_active_record('springtube_rfd_master',$data,'rfd_id',$this->uri->segment(3),$this->session->userdata['logged_in']['company_id']);
            ...
            if($result){
              ...
              $data['springtube_rfd_master']=$this->common_model->select_one_active_record('springtube_rfd_master',$this->session->userdata['logged_in']['company_id'],'rfd_id',$this->uri->segment(3));
        ...
        if ($module_row->module_name === 'Production') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if ($formrights_row->new == 1) {
              ...
              $data['order'] = $this->sales_order_book_model->active_details_records('order_master', $data, $this->session->userdata['logged_in']['company_id']);
              $data['rfd'] = $this->common_model->rfd_data_model($this->uri->segment(3));
              //echo $this->db->last_query();
              ...
              $data['select_active_records_rfd_data'] = $this->common_model->select_active_records_rfd_data('springtube_rfd_master', $this->session->userdata['logged_in']['company_id'], $this->uri->segment(3), $this->uri->segment(4));
              ...
              $data['rfd_transfer'] = $this->springtube_rfd_model->rfd_transfer_model($this->uri->segment(3));
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()) . '/active-title');
              $this->load->view(ucwords($this->router->fetch_class()) . '/rfd-qty-transfer-form', $data);
              $this->load->view('Home/footer');
            } else {
              $data['note'] = 'No Create rights Thanks';
              $this->load->view('Home/header');
        ...
        if ($module_row->module_name === 'Production') {
          ...
          foreach ($data['formrights'] as $formrights_row) {
            ...
            if ($formrights_row->new == 1) {
              ...
              $so_no = $this->input->post('order_no');
              $jobcard_no = $this->input->post('jobcard_no');
              $transfer_jobcard_no = $this->input->post('job_card_no');
              $article_no = $this->input->post('article_no');
              $transfer_article_no = $this->input->post('articl_no');
              $article_name = $this->input->post('article_name');
              $transfer_so_no = $this->input->post('so_no');
              $rfd_qty = $this->input->post('total_rfd_qty');
              $transfer_rfd_qty = $this->input->post('transfer_qty');
              $transfer_so_qty = $this->input->post('transfer_rfd_qty');
              $date = date('Y-m-d');
              $sleeve_dia = $this->input->post('dia');
              $total_microns = $this->input->post('total_microns');
              $sleeve_length = $this->input->post('sleeve_length');
              $second_layer_mb = $this->input->post('second_layer_mb');
              $sixth_layer_mb = $this->input->post('sixth_layer_mb');
              $film_code = $this->input->post('film_code');
              ...
              if ($rfd_qty > $transfer_rfd_qty && $article_no == $transfer_article_no) {
                $remaining_transfer_qty = $rfd_qty - $transfer_rfd_qty;
                $dataa = array(
                  'status' => '1',
                );
                $update = $this->common_model->update_one_active_record('springtube_rfd_master', $dataa, 'order_no', $so_no, '000020');
                ...
                $data = array(
                  'order_no' => $so_no,
                  'status' => '0',
                  'rfd_qty' => $remaining_transfer_qty,
                  'transfer_so_no' => $transfer_so_no,
                  'transfer_qty' => $transfer_rfd_qty,
                  'rfd_date' => $date,
                  'company_id' => '000020',
                  ...
                  'second_layer_mb' => $second_layer_mb,
                  'sixth_layer_mb' => $sixth_layer_mb,
                  'film_code' => $film_code,
                  'customer' => $customer,
                  'user_id' => $user_id,
                ...
                $result = $this->common_model->save('springtube_rfd_master', $data);
                $total_rfd = $transfer_so_qty + $transfer_rfd_qty;


                $dataaa = array(
                  'status' => '1',
                );
                $update = $this->common_model->update_one_active_record('springtube_rfd_master', $dataaa, 'order_no', $transfer_so_no, '000020');

                $datas = array(
                  'order_no' => $transfer_so_no,
                  'status' => '0',
                  'rfd_qty' => $total_rfd,
                  'resive_trns_so_no' => $so_no,
                  'resive_trns_qty' => $transfer_rfd_qty,
                  'rfd_date' => $date,
                  'company_id' => '000020',
                  'article_no' => $transfer_article_no,
                  'jobcard_no' => $transfer_jobcard_no,
                  'sleeve_dia' => $sleeve_dia,
                  'total_microns' => $total_microns,
                  'sleeve_length' => $sleeve_length,
                  'second_layer_mb' => $second_layer_mb,
                ...
                $result = $this->common_model->save('springtube_rfd_master', $datas);

                echo "DATA TRANSFER SUCCESS";

                $data['note'] = 'Update Succesfully';

                header("refresh:1;url=" . base_url() . "index.php/" . $this->router->fetch_class());
              } else {
                echo "BOTH SO NUMBER ARTICLE NUMBER NOT MATCH";
              }
            } else {
              $data['note'] = 'No rights Thanks';
              $data['page_name'] = 'home';
