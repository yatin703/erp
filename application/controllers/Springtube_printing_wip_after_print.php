...
/* Changes */
          ...
          if($formrights_row->new==1){
            ...
            if($this->form_validation->run()===FALSE){
              ...
              if($body_making_type!='' && $body_making_type=='FLOWSEAM'){
                ...
                $job_meters_in_km=($job_meters/1000)*2; 
  ...
  function wip_release($aprint_wip_id){
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->new==1){ 
              ...
              $table='springtube_printing_wip_master_after';
              $data['springtube_printing_wip_master_after']=$this->springtube_bodymaking_wip_model->select_one_active_record($table,$this->session->userdata['logged_in']['company_id'],'aprint_wip_id',$aprint_wip_id);
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->new==1){
                ...
                $this->form_validation->set_rules('release_qty','Release Qty','required|less_than[shortfall_qty]');
                $this->form_validation->set_rules('to_process','Release Towards','required');
                if($this->form_validation->run()==FALSE){
 
                $aprint_wip_id=$this->input->post('aprint_wip_id');               
                ...
                $this->load->view('Home/footer');
                  ...
                  foreach($data['springtube_printing_wip_master_after'] as $row){
                    $a=$row->release_qty;
                    $a;
                    ...
                    'order_no'         => $this->input->post('order_no'),
                    'aprint_wip_qty'   => $this->input->post('shortfall_qty'),
                    'release_qty'      => $a+$this->input->post('release_qty'),
                    'aprint_wip_date'  => $this->input->post('aprint_wip_date'),
                  ...
                  $result=$this->common_model->save('springtube_printing_wip_master_after',$data_wip);
  
                  $data_print=array(
                    'printing_flag'=> '1'
                 );
                $result=$this->common_model->update_one_active_record('springtube_printing_wip_master_after',$data_print,'aprint_wip_id',$this->input->post('aprint_wip_id'),$this->session->userdata['logged_in']['company_id']);

                $this->session->set_flashdata('success', 'Scrap update successfully');

                $table='springtube_printing_wip_master_after';
                include('pagination_wip.php');
                $data['springtube_printing_wip_master_after']=$this->springtube_bodymaking_wip_model->select_active_input_release_records_print($table,$this->input->post('aprint_wip_id'));
...
function wip_release_scrap($aprint_wip_id){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){ 
           ...
           //echo $this->db->last_query();
            ...
            $this->load->view('Home/subnav');
            $this->load->view(ucwords($this->router->fetch_class()).'/active-title',$data);
            $this->load->view(ucwords($this->router->fetch_class()).'/wip-release-form-scrap',$data);
            $this->load->view('Home/footer');
          }else{
            $data['note']='No rights Thanks';
            $this->load->view('Home/header');
...
function wip_release_save_scrap(){
...
function wip_scrap_for_printing(){
