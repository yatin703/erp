...
class Coex_extrusion_heading_wip extends CI_Controller {
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->view==1){
              $table='coex_extrusion_heading_wip';
              include('pagination.php');
              $data['coex_extrusion_heading_wip']=$this->coex_extrusion_heading_model->select_active_records_heading_wip($config["per_page"],$this->uri->segment(3),$table,$this->session->userdata['logged_in']['company_id']);
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->new==1){
              ...
              $table='coex_extrusion_heading_wip';
              $data['coex_extrusion_heading_wip']=$this->coex_extrusion_heading_model->select_one_active_record_wip($table,$this->session->userdata['logged_in']['company_id'],'hwip_id',$hwip_id);
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            if($this->form_validation->run()==FALSE){
              ...
              if($this->input->post('to_process')=='5'){
                $data_scrap=array(
                  ...
                  'archive'          => '0',
                  'status'           => '0',
                  'form_process'     => 'Heading WIP',
                  'user_id'          => $this->session->userdata['logged_in']['user_id']         
                );
                $result=$this->common_model->save('coex_extrusion_heading_wip_scrap',$data_scrap);
                ...
                $data_release_qc=array(
                  ...
                  'archive'          => '0',
                  'status'           => '1',
                  'form_process'     => 'Return Heading WIP',
                  'user_id'          =>$this->session->userdata['logged_in']['user_id']         
                );
                  $result=$this->common_model->save('coex_extrusion_heading_qc',$data_release_qc);
                }
              ...
              $result=$this->common_model->update_one_active_record('coex_extrusion_heading_wip',$data_ce,'hwip_id',$this->input->post('wip_id'),$this->session->userdata['logged_in']['company_id']);
              ...
              $data['coex_machine_master']=$this->common_model->select_active_records_where('coex_machine_master',$this->session->userdata['logged_in']['company_id'],$dataa);
              $data['shift_master']=$this->common_model->select_active_drop_down('springtube_shift_master',$this->session->userdata['logged_in']['company_id']);
              $data['coex_extrusion_heading_wip']=$this->coex_extrusion_heading_model->select_one_active_record_wip('coex_extrusion_heading_wip',$this->session->userdata['logged_in']['company_id'],'coex_extrusion_heading_wip.hwip_id',$this->uri->segment(3));
