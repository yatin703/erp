...
class Label_specification_model extends CI_Model {
	...
	public function select_active_records($limit,$start,$table,$company){
		...
		$this->db->where($table.'.dyn_qty_present','LABEL|1');
	...
	public function active_record_search_new($table,$data,$search,$from,$to,$company){
		...
		$sql.= "SELECT * FROM 
					( 	select 
					 	...
					 	WHERE a.dyn_qty_present like 'LABEL%' AND a.archive <> 1 AND a.company_id='$company' AND b.company_id='$company'";
					 	if(!empty($from) && !empty($to)){
		    ...
		    $sql.= ") A,
					( select spec_id,spec_version_no,
						MAX(CASE WHEN item_group_id =6 AND parameter_name='NON LACQUERING HEIGHT BY OPEN END' THEN IF (parameter_value!='',parameter_value,relating_master_value) END) AS 'OE',
				        MAX(CASE WHEN item_group_id =6 AND parameter_name='NON LABELING HEIGHT BY SHOULDER END' THEN IF(parameter_value!='',parameter_value,relating_master_value) END) AS 'SE',
	...
	public function approved_record_search_for_bom($table,$edit,$company){
			$query=$this->db->query("SELECT S.article_no, ANI.lang_article_description,A.main_group_id,ANI.lang_sub_description FROM $table S LEFT JOIN   article A ON S.article_no=A.article_no LEFT JOIN article_name_info ANI ON A.article_no=ANI.article_no  WHERE   S.company_id='$company' AND S.archive <>1 AND S.final_approval_flag=1  AND A.archive <>1 AND A.company_id =  '$company' AND A.main_group_id ='44' AND ANI.company_id =  '$company' AND ( ANI.lang_article_description LIKE '%$edit%'  OR ANI.lang_sub_description LIKE '%$edit%' OR ANI.article_no like  '%$edit%') ORDER BY ANI.lang_article_description");		
