...
class Springtube_bodymaking_wip_model extends CI_Model {	
	...
	function update_scrap($update_scrap,$bm_wip_id){
		$this->db->where('bm_wip_id',$bm_wip_id);
		$this->db->update('springtube_bodymaking_wip_master',$update_scrap);
    }

    public function select_active_input_release_records($table,$bm_wip_id){
        $this->db->select('*');
        $this->db->from($table);
        $this->db->where($table.'.bm_wip_id',$bm_wip_id);
        $query = $this->db->get();
        return $result=$query->result();
    }
	

	public function select_active_input_release_records_print($table,$aprint_wip_id){
        $this->db->select('*');
        $this->db->from($table);
        $this->db->where($table.'.aprint_wip_id',$aprint_wip_id);
        $query = $this->db->get();
        return $result=$query->result();
    }
