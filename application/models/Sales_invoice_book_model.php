...
//Search Reuslt()
	public function active_record_search($table,$data,$from,$to,$company,$in_arr){
		$this->db->select(''.$table.'.*,address_master.name1,address_master.strno,address_master.name2,address_master.street,address_master.name3,address_master.city_code,address_master.country_id,address_master.zip_code,address_master.email,address_master.isdn_local,address_master.telephone1,user_master.login_name,property_master.lang_property_name,ar_invoice_master_lang.lang_cust_po_info');
		$this->db->from($table);
	...
	public function active_record_search_index_report($limit,$start,$table,$data,$from,$to,$company){
		$this->db->select(''.$table.'.*,address_master.name1,address_master.strno,address_master.name2,address_master.street,address_master.name3,address_master.isdn_local,address_master.zip_code,address_master.telephone1,address_master.telephone2,property_master.lang_property_name,ar_invoice_master_lang.lang_cust_po_info,zip_code_master.state_code,zip_code_master.lang_city,ar_invoice_details.layer_no');
		$this->db->from($table);
	...
	public function sales_report_pcb($table,$company,$from_date,$to_date,$data){
		$this->db->select(''.$table.'.*,address_master.name1,address_master.strno,address_master.name2,address_master.street,address_master.name3,address_master.isdn_local,address_master.zip_code,address_master.telephone1,address_master.telephone2,property_master.lang_property_name,ar_invoice_master_lang.lang_cust_po_info,zip_code_master.state_code,zip_code_master.lang_city');
		$this->db->from($table);
		$this->db->join('ar_invoice_master_lang',''.$table.'.ar_invoice_no=ar_invoice_master_lang.ar_invoice_no','LEFT');
		$this->db->join('ar_invoice_details','ar_invoice_master.ar_invoice_no=ar_invoice_details.ar_invoice_no','INNER');
        $this->db->join('article_name_info','ar_invoice_details.article_no=article_name_info.article_no','INNER');
		$this->db->join('address_master',''.$table.'.customer_no=address_master.adr_company_id','LEFT');
		$this->db->join('zip_code_master','address_master.zip_code=zip_code_master.zip_code','LEFT');
		$this->db->join('address_details',$table.'.customer_no=address_details.adr_company_id','LEFT');
		$this->db->join('property_master','address_details.property_id=property_master.property_id','LEFT');
        ...
        /*if(!empty($ar_invoice_no)){
	    	$this->db->where('ar_invoice_master.ar_invoice_no',$ar_invoice_no);
	    }*/
		...
		$this->db->order_by(' '.$table.'.invoice_date' ,'asc');
		$this->db->order_by(' '.$table.'.ar_invoice_no' ,'asc');
		$query = $this->db->get();
		return $result=$query->result();
	}
...
//USED in REPORT SALES 5 Year Monthwise
	public function sales_five_year_apr_march_monthwise($table,$from_date,$to_date){
		$sql="SELECT * from (SELECT  /*
					...
					SUM( if( ar_invoice_master.invoice_date between '2019-04-01' AND '2020-03-31', (ar_invoice_details.arid_qty /100),0) ) AS quantity_seventeen_eighteen,
					...
					SUM( if( ar_invoice_master.invoice_date between '2020-04-01' AND '2021-03-31', (ar_invoice_details.arid_qty /100),0) ) AS quantity_eighteen_nineteen,
					...
					SUM( if( ar_invoice_master.invoice_date between '2021-04-01' AND '2022-03-31', (ar_invoice_details.arid_qty /100),0) ) AS quantity_nineteen_twenty,

					SUM( IF( ar_invoice_master.for_export=1 AND ar_invoice_master.invoice_date between '2021-04-01' AND '2022-03-31', ar_invoice_details.calc_sell_price*(ar_invoice_master.exchange_rate/100)*(ar_invoice_details.arid_qty/100), 0 ) )+SUM( IF( ar_invoice_master.for_export<>1 AND ar_invoice_master.invoice_date between '2021-04-01' AND '2022-03-31', (ar_invoice_details.arid_qty/100)*(ar_invoice_details.selling_price/100), 0 ) )  AS value_nineteen_twenty,

					SUM( if( ar_invoice_master.invoice_date between '2022-04-01' AND '2023-03-31', (ar_invoice_details.arid_qty /100),0) ) AS quantity_twenty_twentyone,

					SUM( IF( ar_invoice_master.for_export=1 AND ar_invoice_master.invoice_date between '2022-04-01' AND '2023-03-31', ar_invoice_details.calc_sell_price*(ar_invoice_master.exchange_rate/100)*(ar_invoice_details.arid_qty/100), 0 ) )+SUM( IF( ar_invoice_master.for_export<>1 AND ar_invoice_master.invoice_date between '2022-04-01' AND '2023-03-31', (ar_invoice_details.arid_qty/100)*(ar_invoice_details.selling_price/100), 0 ) )  AS value_twenty_twentyone,

					SUM( if( ar_invoice_master.invoice_date between '2023-04-01' AND '2024-03-31', (ar_invoice_details.arid_qty /100),0) ) AS quantity_twentyone_twentytwo,

					SUM( IF( ar_invoice_master.for_export=1 AND ar_invoice_master.invoice_date between '2023-04-01' AND '2024-03-31', ar_invoice_details.calc_sell_price*(ar_invoice_master.exchange_rate/100)*(ar_invoice_details.arid_qty/100), 0 ) )+SUM( IF( ar_invoice_master.for_export<>1 AND ar_invoice_master.invoice_date between '2023-04-01' AND '2024-03-31', (ar_invoice_details.arid_qty/100)*(ar_invoice_details.selling_price/100), 0 ) )  AS value_twentyone_twentytwo
