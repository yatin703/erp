...
class Coex_runtime_downtime_model extends CI_Model {
	...
	public function active_record_search($table,$data,$company){
		...
		//$this->db->join('production_master',$table.'.mp_pos_no=production_master.mp_pos_no','LEFT');

		$this->db->join('coex_machine_start_stop_reasons',$table.'.cemssr_id=coex_machine_start_stop_reasons.cemssr_id','LEFT');
		$this->db->where($table.'.archive<>', '1');
		$this->db->where($table.'.company_id',$company);
		//$this->db->where('production_master.archive<>', '1');
		//$this->db->where('production_master.company_id',$company);
	...
	public function active_record_search_group_by($table,$data,$from,$to,$company,$customer_category){
		$this->db->select($table.'.mp_pos_no,production_master.sales_ord_no,production_master.article_no');
		$this->db->from($table);
		$this->db->join('coex_machine_master',$table.'.machine_id=coex_machine_master.machine_id','LEFT');
		$this->db->join('production_master',$table.'.mp_pos_no=production_master.mp_pos_no','LEFT');
		$this->db->join('coex_machine_start_stop_reasons',$table.'.cemssr_id=coex_machine_start_stop_reasons.cemssr_id','LEFT');
		if($customer_category!=''){
			$this->db->join('article_name_info','production_master.article_no=article_name_info.article_no','LEFT');
			//$this->db->where('article_name_info.archive<>', '1');
		...
		$this->db->where(''.$table.'.machine_start_time <=', $to." 23:59:59");
		}		
		foreach($data as $key => $value) {
			$this->db->where($key,$value);
		}
		$this->db->group_by($table.'.mp_pos_no');
		$this->db->order_by($table.'.machine_start_time');
		$query = $this->db->get();
