...
class Springtube_printing_inspection_model extends CI_Model {	
	...
	public function select_total_inspection_qty($table,$data){
		$this->db->select('IFNULL(SUM(inspected_output_qty),0) as total_inspected_output_qty');
		$this->db->from($table);
		$this->db->join('springtube_printing_inspection_details',$table.'.production_id=springtube_printing_inspection_details.production_id','LEFT');
		if(!empty($data)){
			foreach($data as $key => $value) {
