...
class Certificate_of_analysis_model extends CI_Model {
	...
	public function select_active_records($limit,$start,$table,$company){
		...
		$this->db->group_by('certificate_no');
		$this->db->order_by(' '.$table.'.certificate_no','DESC');
...
public function get_ajax_ar_invoice_no_id($ar_invoice_no_id,$ar_order_no_id,$ar_product_no_id){
  if($ar_invoice_no_id=='' && $ar_order_no_id=='' && $ar_product_no_id==''){ 
    ...
    $query = $this->db->query("SELECT aim.ar_invoice_no as aim_id, aim.invoice_date as in_date, aid.ar_invoice_no as aid_id, aid.ref_ord_no as ref_ord_id, aid.article_no as aid_article_no, om.order_no as om_ord_id, om.customer_no as om_customer_id, om.order_flag as order_type, od.order_no as ord_id, bm.bom_no as bom_id, bm.shoulder_code as shoulder_id, bm.sleeve_code as sleeve_id, bm.cap_code as cap_code_id, ss.article_no as article_id, ss.spec_id as ss_id, sd.spec_id as sd_id, ss.spec_version_no as sd_version,sdl.relating_master_value,sdm.sleeve_diameter as sleeve_diameter_id,artd.version_no as art_version_no,ode.version_no as order_version_no,artd.ad_id as art_ad_id,ode.ad_id as od_ad_id,sartd.ad_id as sart_ad_id,sartd.version_no as sart_version_no,
  ...
  if(!empty($query)){   
      ...
      if(strcmp($item['ARTWORK_LACQ_TYPE_COEX'], 'RM-LQ-GLAC') !== 0){
        $ARTWORK_LACQ_TYPE="GLOSS";
      }elseif (strcmp($item['ARTWORK_LACQ_TYPE_SPRING'], 'RM-LQ-GLAC') !== 0) {
        $ARTWORK_LACQ_TYPE="GLOSS";
      }elseif (strcmp($item['ARTWORK_LACQ_TYPE_COEX'], 'RM-LQ-MLAC') !== 0) {
        $ARTWORK_LACQ_TYPE="MATT";
      }elseif (strcmp($item['ARTWORK_LACQ_TYPE_SPRING'], 'RM-LQ-MLAC') !== 0) {
        $ARTWORK_LACQ_TYPE="MATT";
      }else{
        $ARTWORK_LACQ_TYPE="-";
      }
