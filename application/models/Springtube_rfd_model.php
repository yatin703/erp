...
class Springtube_rfd_model extends CI_Model
{
	...
	public function totalrfd()
	{

		$this->db->select('rfd_date,sleeve_dia,sleeve_length,second_layer_mb,sixth_layer_mb,total_microns,rfd_qty,order_no');
		$this->db->select('article_no, SUM(rfd_qty) AS total_rfd_qty');
		$this->db->from('springtube_rfd_master');
		$this->db->where('archive<>', '1');
		$this->db->where('rfd_qty<>', '0');
		$this->db->where('status', '0');
		$this->db->where('rfd_date>', '2021-12-29');
		$this->db->group_by('article_no');
		$this->db->order_by('rfd_date', 'DESC');
		$query = $this->db->get();
		return $query->result();
	}
}
