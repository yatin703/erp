...
class Sales_order_book_model extends CI_Model
{
	...
	public function active_record_oc($table, $data, $from, $to, $company, $list_arr, $approval_from_date, $approval_to_date, $cancelled_from_date, $cancelled_to_date)
	{
		...
		$this->db->where('' . $table . '.for_sampling', '0');
		//$this->db->where(''.$table.'.for_export','0');
		$this->db->where('' . $table . '.order_flag<>', '3');
	...
	public function active_record_my_oc($table, $data, $from, $to, $company, $list_arr, $approval_from_date, $approval_to_date, $cancelled_from_date, $cancelled_to_date, $user_id)
	{
		$this->db->select('' . $table . '.*,address_master.name1,address_master.isdn_local,user_master.login_name,property_master.lang_property_name,order_master_lang.lang_addi_info,address_master.country_id,country_master_lang.lang_country_name as country_name,country_master.currency_symbol,address_master.max_lead_time');
		$this->db->from($table);
		$this->db->join('order_master_lang', '' . $table . '.order_no=order_master_lang.order_no', 'LEFT');
		...
		$this->db->where('' . $table . '.archive!=', '1');
		$this->db->where('' . $table . '.oc_date!=', '0000-00-00');
		$this->db->where('' . $table . '.oc_date>=', '2022-06-06');
		$this->db->where('' . $table . '.oc_sent_flag=', '0');
		...
		//$this->db->where(''.$table.'.for_export','0');
		$this->db->where('' . $table . '.order_flag<>', '3');
		$this->db->where('' . $table . '.order_closed<>', '2');
		$this->db->where('' . $table . '.trans_closed<>', '1');
		if ($from != '' && $to != '') {
			$this->db->where('' . $table . '.order_date >=', $from);
			$this->db->where('' . $table . '.order_date <=', $to);
	...
	public function select_shoulder_specs_record($table, $company, $data)
	{
		$this->db->select("spec_id,spec_version_no,
							...
							MAX(CASE WHEN item_group_id =4 AND parameter_name='HDPE THREE' THEN mat_article_no  END) AS 'SHOULDER_HDPE_THREE',
							MAX(CASE WHEN item_group_id =4 AND parameter_name='HDPE THREE' THEN mat_info  END) AS 'SHOULDER_HDPE_THREE_PERC',
							MAX(CASE WHEN item_group_id =4 AND parameter_name='SHOULDER FOIL TAG' THEN mat_article_no  END) AS 'SHOULDER_FOIL_TAG'");
	...
	public function select_orders_for_autocomplete($table, $data, $company)
	{
		...
		$this->db->where('archive<>', '1');
		$this->db->where('company_id', $company);
		$this->db->where('order_date>=', '2014-04-01');
		$query = $this->db->get();
		return $result = $query->result();
	}
	...
	public function select_open_orders_for_springtube($table, $data, $company)
	{
		...
		$this->db->where('company_id', $company);
		$ord_flag = array('1', '4');
		$this->db->where_in('order_flag', $ord_flag);
	...
	public function select_film_specs_record($table, $company, $data)
	{
		$this->db->select("spec_id,spec_version_no,
			...
			MAX(CASE WHEN item_group_id =3 AND parameter_name='HDPE' AND layer_no=1 THEN mat_article_no END) AS 'FILM_HDPE_1',
			MAX(CASE WHEN item_group_id =3 AND parameter_name='HDPE' AND layer_no=1 THEN mat_info  END) AS 'FILM_HDPE_PERC_1',
			...
			MAX(CASE WHEN item_group_id =3 AND parameter_name='HDPE' AND layer_no=5 THEN mat_article_no END) AS 'FILM_HDPE_5',
			MAX(CASE WHEN item_group_id =3 AND parameter_name='HDPE' AND layer_no=5 THEN mat_info  END) AS 'FILM_HDPE_PERC_5',
			...
			MAX(CASE WHEN item_group_id =3 AND parameter_name='HDPE' AND layer_no=7 THEN mat_article_no END) AS 'FILM_HDPE_7',
			MAX(CASE WHEN item_group_id =3 AND parameter_name='HDPE' AND layer_no=7 THEN mat_info  END) AS 'FILM_HDPE_PERC_7',
			");
	...
	public function pending_orders($table, $customer_order_no, $customer_no)
	{

		$sql = "SELECT om.oc_date,om.order_no,om.customer_no,om.order_date,om.cust_order_no,om.cust_order_date,om.cust_product_no,od.article_no,od.total_order_quantity /100 AS quantity, IFNULL(sum( ar.arid_qty /100 ),0) AS dispatch, if( IFNULL(sum( ar.arid_qty /100 ),0) < ( od.total_order_quantity /100 ) , (od.total_order_quantity /100) - IFNULL(sum( ar.arid_qty /100),0), 0 ) as pending,om.approval_date FROM `order_master` AS om INNER JOIN order_details AS od ON om.order_no = od.order_no LEFT JOIN ar_invoice_details AS ar ON om.order_no = ar.ref_ord_no AND od.article_no = ar.article_no WHERE " . ($customer_order_no <> "" ? "om.cust_order_no='$customer_order_no'  AND " : "") . "  " . ($customer_no <> "" ? "om.customer_no='$customer_no' AND" : "") . " om.final_approval_flag = '1' AND om.trans_closed<>1 AND om.for_sampling<>1 AND om.for_stock<>1 AND od.article_no NOT LIKE 'DD%' AND od.article_no NOT LIKE 'CAP%' GROUP BY om.order_no,od.article_no";
	...
	public function pending_sales_order($table, $from_date, $to_date)
	{
		...
		$sql = "SELECT  A.sleeve_dia,sum(A.SCREEN_FLEXO_PENDING) as SCREEN_FLEXO_PENDING_QTY,sum(A.SCREEN_FLEXO_PENDING_VAL) as SCREEN_FLEXO_PENDING_VALUE, sum(A.OFFSET_PENDING) as OFFSET_PENDING_QTY, sum(A.OFFSET_PENDING_VAL) as OFFSET_PENDING_VALUE,sum(A.LABEL_PENDING) as LABEL_PENDING_QTY,sum(A.LABEL_PENDING_VAL) as LABEL_PENDING_VALUE,sum(A.DIGITAL_PENDING) as DIGITAL_PENDING_QTY,sum(A.DIGITAL_PENDING_VAL) as DIGITAL_PENDING_VALUE,sum(A.UNKNOWN_PENDING) as UNKNOWN_PENDING_QTY from (SELECT 
					...
					AND om.for_stock<>1
					AND od.print_type<>''					
					AND od.article_no NOT LIKE  'DD%'
					AND od.article_no NOT LIKE  'CAP%'
	...
	public function pending_sales_order_monthwise_without_oc($table, $from_date, $to_date, $customer_category_id)
	{
		...
		$sql .= " AND om.trans_closed<>1
					...
					AND od.article_no NOT LIKE  '%CAP%'
					GROUP BY  om.order_no,od.article_no) AS A 
					GROUP BY A.order_year,A.order_month
					order by order_year,order_month_no asc";
	...
	public function pending_sales_order_on_delivery_date($table, $from_date, $to_date, $customer_category_id)
	{
		...
		$sql = "SELECT  sum(A.SCREEN_FLEXO_TO_BE_GO) as SCREEN_FLEXO_TO_BE_GO_QTY,A.order_month_no,A.order_month,A.order_year,A.sleeve_dia,sum(A.SCREEN_FLEXO_PENDING) as SCREEN_FLEXO_PENDING_QTY,sum(A.SCREEN_FLEXO_PENDING_VAL) as SCREEN_FLEXO_PENDING_VALUE, sum(A.OFFSET_PENDING) as OFFSET_PENDING_QTY, sum(A.OFFSET_PENDING_VAL) as OFFSET_PENDING_VALUE,sum(A.LABEL_PENDING) as LABEL_PENDING_QTY,sum(A.LABEL_PENDING_VAL) as LABEL_PENDING_VALUE,sum(A.DIGITAL_PENDING) as DIGITAL_PENDING_QTY,sum(A.DIGITAL_PENDING_VAL) as DIGITAL_PENDING_VALUE,sum(A.UNKNOWN_PENDING) as UNKNOWN_PENDING_QTY from (SELECT YEAR(od.delivery_date) AS order_year, LEFT(MONTHNAME (od.delivery_date),3) AS order_month,MONTH(od.delivery_date) as order_month_no,om.order_no, od.article_no,if(om.for_export<>1 , (od.selling_price/100),(od.calc_sell_price)*(om.exchange_rate/100))as unit_price, od.sleeve_dia, od.total_order_quantity /100 AS quantity,lm.printing_group, IFNULL(sum( ar.arid_qty /100 ),0) AS dispatch, 

					if( sum( ar.arid_qty /100 ) < ( od.total_order_quantity /100 ) , (od.total_order_quantity /100) - sum( ar.arid_qty /100 ), 0 ) AS pending,

					if(lm.printing_group='SCREEN+FLEXO',(od.total_order_quantity /100),0) as SCREEN_FLEXO_TO_BE_GO,
		...
		$sql .= " AND om.trans_closed<>1
					...
					AND od.article_no NOT LIKE  '%CAP%'
					GROUP BY  om.order_no,od.article_no) AS A 
					GROUP BY A.order_month,A.order_year
					order by order_year,order_month_no asc";
	...
	public function pending_sales_order_on_oc_date($table, $from_date, $to_date, $customer_category_id)
	{

		$sql = "SELECT  sum(A.SCREEN_FLEXO_TO_BE_GO) as SCREEN_FLEXO_TO_BE_GO_QTY,A.order_month_no,A.order_month,A.order_year,A.sleeve_dia,sum(A.SCREEN_FLEXO_PENDING) as SCREEN_FLEXO_PENDING_QTY,sum(A.SCREEN_FLEXO_PENDING_VAL) as SCREEN_FLEXO_PENDING_VALUE, sum(A.OFFSET_PENDING) as OFFSET_PENDING_QTY, sum(A.OFFSET_PENDING_VAL) as OFFSET_PENDING_VALUE,sum(A.LABEL_PENDING) as LABEL_PENDING_QTY,sum(A.LABEL_PENDING_VAL) as LABEL_PENDING_VALUE,sum(A.DIGITAL_PENDING) as DIGITAL_PENDING_QTY,sum(A.DIGITAL_PENDING_VAL) as DIGITAL_PENDING_VALUE,sum(A.UNKNOWN_PENDING) as UNKNOWN_PENDING_QTY from (SELECT YEAR(om.oc_date) AS order_year, LEFT(MONTHNAME (om.oc_date),3) AS order_month,MONTH(om.oc_date) as order_month_no,om.order_no, od.article_no,if(om.for_export<>1 , (od.selling_price/100),(od.calc_sell_price)*(om.exchange_rate/100))as unit_price, od.sleeve_dia, od.total_order_quantity /100 AS quantity,lm.printing_group, IFNULL(sum( ar.arid_qty /100 ),0) AS dispatch, 
	...
	public function pending_sales_order_on_oc_date_by_customer($table, $from_date, $to_date, $customer_category_id)
	{

		$sql = "SELECT  A.category_name as customer,sum(A.SCREEN_FLEXO_TO_BE_GO) as SCREEN_FLEXO_TO_BE_GO_QTY,A.order_month_no,A.order_month,A.order_year,A.sleeve_dia,sum(A.SCREEN_FLEXO_PENDING) as SCREEN_FLEXO_PENDING_QTY,sum(A.SCREEN_FLEXO_PENDING_VAL) as SCREEN_FLEXO_PENDING_VALUE, sum(A.OFFSET_PENDING) as OFFSET_PENDING_QTY, sum(A.OFFSET_PENDING_VAL) as OFFSET_PENDING_VALUE,sum(A.LABEL_PENDING) as LABEL_PENDING_QTY,sum(A.LABEL_PENDING_VAL) as LABEL_PENDING_VALUE,sum(A.DIGITAL_PENDING) as DIGITAL_PENDING_QTY,sum(A.DIGITAL_PENDING_VAL) as DIGITAL_PENDING_VALUE,sum(A.UNKNOWN_PENDING) as UNKNOWN_PENDING_QTY from (SELECT YEAR(om.oc_date) AS order_year, LEFT(MONTHNAME (om.oc_date),3) AS order_month,MONTH(om.oc_date) as order_month_no,am.category_name,om.order_no, od.article_no,if(om.for_export<>1 , (od.selling_price/100),(od.calc_sell_price)*(om.exchange_rate/100))as unit_price, od.sleeve_dia, od.total_order_quantity /100 AS quantity,lm.printing_group, IFNULL(sum( ar.arid_qty /100 ),0) AS dispatch, 
					...
					/*LEFT JOIN ar_invoice_master AS am ON ar.ar_invoice_no = am.ar_invoice_no*/
					LEFT JOIN lacquer_types_master AS lm ON od.print_type = lm.lacquer_type 
					LEFT JOIN article_name_info AS an ON an.article_no = od.article_no LEFT JOIN address_category_master AS am ON am.adr_category_id = an.adr_category_id";
		if (!empty($customer_category_id)) {
			$sql .= " LEFT JOIN article_name_info ON od.article_no=article_name_info.article_no";
		}
		...
		$sql .= " AND om.trans_closed<>'1'
					...
					having SCREEN_FLEXO_PENDING<>0 OR OFFSET_PENDING<>0 OR LABEL_PENDING<>0 OR DIGITAL_PENDING<>0 OR UNKNOWN_PENDING<>0 ORDER BY SCREEN_FLEXO_PENDING desc,OFFSET_PENDING desc,LABEL_PENDING desc,DIGITAL_PENDING desc) AS A
					GROUP BY A.order_month,A.order_year,A.category_name/*,A.sleeve_dia*/
					order by order_year,order_month_no/*,sleeve_dia*/ asc ,SCREEN_FLEXO_PENDING_QTY desc";
	...
	public function pending_sales_order_on_delivery_date_by_customer($table, $from_date, $to_date, $customer_category_id)
	{

		$sql = "SELECT  A.category_name as customer,sum(A.SCREEN_FLEXO_TO_BE_GO) as SCREEN_FLEXO_TO_BE_GO_QTY,A.order_month_no,A.order_month,A.order_year,A.sleeve_dia,sum(A.SCREEN_FLEXO_PENDING) as SCREEN_FLEXO_PENDING_QTY,sum(A.SCREEN_FLEXO_PENDING_VAL) as SCREEN_FLEXO_PENDING_VALUE, sum(A.OFFSET_PENDING) as OFFSET_PENDING_QTY, sum(A.OFFSET_PENDING_VAL) as OFFSET_PENDING_VALUE,sum(A.LABEL_PENDING) as LABEL_PENDING_QTY,sum(A.LABEL_PENDING_VAL) as LABEL_PENDING_VALUE,sum(A.DIGITAL_PENDING) as DIGITAL_PENDING_QTY,sum(A.DIGITAL_PENDING_VAL) as DIGITAL_PENDING_VALUE,sum(A.UNKNOWN_PENDING) as UNKNOWN_PENDING_QTY from (SELECT YEAR(od.delivery_date) AS order_year, LEFT(MONTHNAME (od.delivery_date),3) AS order_month,MONTH(od.delivery_date) as order_month_no,am.category_name,om.order_no, od.article_no,if(om.for_export<>1 , (od.selling_price/100),(od.calc_sell_price)*(om.exchange_rate/100))as unit_price, od.sleeve_dia, od.total_order_quantity /100 AS quantity,lm.printing_group, IFNULL(sum( ar.arid_qty /100 ),0) AS dispatch, 
	...
	public function approved_unapproved_pending_sales_order_delivery_date($table, $from_date, $to_date, $customer_category_id, $order_type)
	{

		$sql = "SELECT  A.category_name as customer,sum(A.SCREEN_FLEXO_TO_BE_GO) as SCREEN_FLEXO_TO_BE_GO_QTY,A.order_month_no,A.order_month,A.order_year,A.sleeve_dia,sum(A.SCREEN_FLEXO_PENDING) as SCREEN_FLEXO_PENDING_QTY,sum(A.SCREEN_FLEXO_PENDING_VAL) as SCREEN_FLEXO_PENDING_VALUE, sum(A.OFFSET_PENDING) as OFFSET_PENDING_QTY, sum(A.OFFSET_PENDING_VAL) as OFFSET_PENDING_VALUE,sum(A.LABEL_PENDING) as LABEL_PENDING_QTY,sum(A.LABEL_PENDING_VAL) as LABEL_PENDING_VALUE,sum(A.DIGITAL_PENDING) as DIGITAL_PENDING_QTY,sum(A.DIGITAL_PENDING_VAL) as DIGITAL_PENDING_VALUE,sum(A.UNKNOWN_PENDING) as UNKNOWN_PENDING_QTY,sum(A.UNKNOWN_PENDING_VAL) as UNKNOWN_PENDING_VALUE from (SELECT YEAR(od.delivery_date) AS order_year, LEFT(MONTHNAME (od.delivery_date),3) AS order_month,MONTH(od.delivery_date) as order_month_no,om.order_no,am.category_name,od.article_no,om.for_export,if(om.for_export<>1 , (od.selling_price/100),(od.calc_sell_price)*(om.exchange_rate/100))as unit_price, od.sleeve_dia, od.total_order_quantity /100 AS quantity,lm.printing_group, IFNULL(sum( ar.arid_qty /100 ),0) AS dispatch, 
					...
					if(om.final_approval_flag<>1, if( IFNULL(sum( ar.arid_qty /100),0) < ( od.total_order_quantity /100 ) , (od.total_order_quantity /100) - IFNULL(sum( ar.arid_qty /100),0), 0 ),0) as UNKNOWN_PENDING,


					if(om.final_approval_flag<>1,if(om.for_export<>1 , (od.selling_price/100),(od.calc_sell_price)*(om.exchange_rate/100))*if( IFNULL(sum( ar.arid_qty / 100),0) < ( od.total_order_quantity / 100 ) , ( od.total_order_quantity / 100 ) - IFNULL(sum( ar.arid_qty / 100),0) , 0 ),0) as UNKNOWN_PENDING_VAL
		...
		if ($order_type != '') {
			$sql .= " AND om.for_export IN ($order_type)";
		} else {
			$sql .= " AND om.for_export IN (0,1)";
		}
	...
	public function approved_unapproved_pending_sales_order_delivery_date_by_customer($table, $from_date, $to_date, $customer_category_id)
	{

		$sql = "SELECT  A.category_name as customer,sum(A.SCREEN_FLEXO_TO_BE_GO) as SCREEN_FLEXO_TO_BE_GO_QTY,A.order_month_no,A.order_month,A.order_year,A.sleeve_dia,sum(A.SCREEN_FLEXO_PENDING) as SCREEN_FLEXO_PENDING_QTY,sum(A.SCREEN_FLEXO_PENDING_VAL) as SCREEN_FLEXO_PENDING_VALUE, sum(A.OFFSET_PENDING) as OFFSET_PENDING_QTY, sum(A.OFFSET_PENDING_VAL) as OFFSET_PENDING_VALUE,sum(A.LABEL_PENDING) as LABEL_PENDING_QTY,sum(A.LABEL_PENDING_VAL) as LABEL_PENDING_VALUE,sum(A.DIGITAL_PENDING) as DIGITAL_PENDING_QTY,sum(A.DIGITAL_PENDING_VAL) as DIGITAL_PENDING_VALUE,sum(A.UNKNOWN_PENDING) as UNKNOWN_PENDING_QTY,sum(A.UNKNOWN_PENDING_VAL) as UNKNOWN_PENDING_VALUE from (SELECT YEAR(od.delivery_date) AS order_year, LEFT(MONTHNAME (od.delivery_date),3) AS order_month,MONTH(od.delivery_date) as order_month_no,om.order_no,am.category_name,od.article_no,if(om.for_export<>1 , (od.selling_price/100),(od.calc_sell_price)*(om.exchange_rate/100))as unit_price, od.sleeve_dia, od.total_order_quantity /100 AS quantity,lm.printing_group, IFNULL(sum( ar.arid_qty /100 ),0) AS dispatch, 
		...
		$sql .= " AND om.trans_closed<>'1'
					...
					/*AND am.cancel_invoice <>1*/
					GROUP BY  om.order_no,od.article_no
					having SCREEN_FLEXO_PENDING<>0 OR OFFSET_PENDING<>0 OR LABEL_PENDING<>0 OR DIGITAL_PENDING<>0 OR UNKNOWN_PENDING<>0 ORDER BY SCREEN_FLEXO_PENDING desc,OFFSET_PENDING desc,LABEL_PENDING desc,DIGITAL_PENDING desc,UNKNOWN_PENDING desc) AS A 
					GROUP BY A.order_month,A.order_year,A.category_name/*,A.sleeve_dia*/
					order by order_year,order_month_no/*,sleeve_dia*/ asc,SCREEN_FLEXO_PENDING_QTY desc";
...
*/
	...
	public function print_type_wise_sales_order($table, $from_date, $to_date)
	{
		$sql = "SELECT 
					...
					AND order_master.cancel_flag<>1
	...
	public function print_type_wise_total_sales_order_on_delivery_date($table, $from_date, $to_date)
	{
		$sql = "SELECT 
					A.ORDER_MONTH_NO,A.order_month as ORDER_MONTH,
					A.order_year as ORDER_YEAR,
					...
					FROM (
					SELECT 
					YEAR(order_details.delivery_date) as ORDER_YEAR, 
					MONTH(order_details.delivery_date) as ORDER_MONTH_NO,
					LEFT(MONTHNAME(order_details.delivery_date),3) as ORDER_MONTH,
					...
					FROM `order_master` INNER JOIN order_details ON order_master.order_no=order_details.order_no where order_details.delivery_date between '$from_date' AND '$to_date'
					AND order_master.for_stock<>1
					AND order_master.for_sampling <>1
					AND order_details.article_no NOT LIKE  'DD%'
					AND order_details.article_no NOT LIKE  'CAP%'
					
					 group by order_master.order_no, order_details.article_no order by order_details.delivery_date asc
	...
	public function total_order_received_by_customer($table, $from_date, $to_date, $sleeve_diaaa)
	{
		$sql = "SELECT YEAR(order_master.approval_date) as ORDER_YEAR, 
					...
					AND order_master.for_stock<>1 AND order_master.cancel_flag<>1";
	...
	public function total_order_received_by_customer_on_order_date($table, $from_date, $to_date, $sleeve_diaaa)
	{
		$sql = "SELECT YEAR(order_master.order_date) as ORDER_YEAR, 
					MONTH(order_master.order_date) as ORDER_MONTH_NO,
					LEFT(MONTHNAME(order_master.order_date),3) as ORDER_MONTH,
					sum((order_details.total_order_quantity/100)) as ORDER_QUANTITY,
					article_name_info.adr_category_id,
					...
					LEFT JOIN address_category_master
					ON article_name_info.adr_category_id=address_category_master.adr_category_id
					where order_master.order_date between '$from_date' AND '$to_date'
					AND order_master.cancel_flag<>1
					AND order_master.for_stock<>1
					AND order_master.for_sampling<>1
					AND order_details.article_no NOT LIKE  'DD%'
	...
	function get_pending_qty($order_no)
	{
	...
	public function sales_user_model()
	{
		$query = $this->db->query("SELECT em.employee_id,em.department_id,em.mailbox,um.user_id FROM `employee_master`as em LEFT JOIN user_master as um on em.`employee_id` = um.user_id WHERE `department_id`='6' ");
		return $result = $query->result();
	}

	public function oc_reminder_model($from_date, $to_date, $user_id)
	{

		$query = $this->db->query("select A.oc_date,A.customer,A.order_no,A.article_no,A.order_qty,A.unit_rate,A.user_id, A.dispatch_qty,A.user_id,A.mailbox,A.pending_qty 
        FROM ( SELECT om.oc_date,um.employee_id,em.mailbox,am.name1 as customer,od.order_no,od.article_no,(od.total_order_quantity)/100 as order_qty,od.calc_sell_price as unit_rate,
        (aid.arid_qty)/100 as dispatch_qty,IF(IFNULL(( aid.arid_qty /100 ),0) < ( od.total_order_quantity /100 ), 
        (od.total_order_quantity /100) - IFNULL(( aid.arid_qty /100),0), 0 ) as pending_qty,om.user_id 
        FROM order_master as om 
        LEFT JOIN order_details as od on om.order_no = od.order_no 
        LEFT JOIN ar_invoice_details as aid on om.order_no = aid.ref_ord_no 
        LEFT JOIN ar_invoice_master as aim on aid.ar_invoice_no = aim.ar_invoice_no 
        LEFT JOIN user_master as um on om.user_id = um.user_id 
        LEFT JOIN employee_master as em on um.employee_id=em.employee_id 
        LEFT JOIN address_master as am on om.customer_no = am.adr_company_id 
        WHERE om.oc_date BETWEEN '$from_date' AND '$to_date' AND om.user_id IN('EMP282','EMP284','EMP298')   order by (od.total_order_quantity)/100 ASC) as A 
        WHERE A.pending_qty<>0 ");

		return $result = $query->result();
	}

	public function get_ajax_data_mismatch($psm_no)
	{
		if ($psm_no == '') {
			header('Content-Type: application/json');
			echo json_encode(array('status' => 400, 'message' => 'Error!  PSM No. not found, try later!'));
		} else {
			$query = $this->db->query("SELECT artm.article_no as artm_article_no, bom.article_no as bom_article_no,bom.print_type as bom_print_type, ani.lang_article_description as product_name,
		MAX(CASE WHEN ssd.parameter_name='DIA'     THEN ssd.relating_master_value END) AS 'BOM_DIA',
		MAX(CASE WHEN ssd.parameter_name='LENGTH'  THEN ssd.parameter_value END) AS 'BOM_LENGTH',
		MAX(CASE WHEN artd.artwork_para_id='1'     THEN artd.parameter_value END) AS 'COEX_DIA',
		MAX(CASE WHEN artd.artwork_para_id='2'     THEN artd.parameter_value END) AS 'COEX_LENGTH',
		MAX(CASE WHEN artd.artwork_para_id='17'    THEN artd.parameter_value END) AS 'COEX_PRINT_TYPE'
		FROM artwork_devel_master as artm
		LEFT JOIN article_name_info as ani ON artm.article_no = ani.article_no 
		LEFT JOIN artwork_devel_details as artd ON artm.ad_id =artd.ad_id  AND artm.version_no = artd.version_no
		LEFT JOIN bill_of_material as bom ON artm.article_no = bom.article_no 
		LEFT JOIN specification_sheet as ss ON bom.sleeve_code = ss.article_no
		LEFT JOIN specification_sheet_details as ssd ON ss.spec_id = ssd.spec_id AND ss.spec_version_no = ssd.spec_version_no WHERE artm.article_no = '$psm_no'");
			if (!empty($query)) {
				$item = $query->row_array();

				$data_mismatch = '';

				if ($item['bom_print_type'] == $item['COEX_PRINT_TYPE'] && $item['BOM_DIA'] == $item['COEX_DIA'] && $item['BOM_LENGTH'] == $item['COEX_LENGTH']) {
					$data_mismatch = "TRUE";
				} else {
					$data_mismatch = "FALSE";
				}
				...
				$resultdata = array("data_mismatch" => $data_mismatch);
	...
	public function bom_gauge_total($bom_no, $bom_version_no)
	{
       ...
       FROM `bill_of_material` as bm
        ...
        where bm.bom_no='$bom_no' and bm.bom_version_no='$bom_version_no' ";
		$query = $this->db->query($sql);
		return $result = $query->result();
	}
}
