...
class Daily_plates_record_model extends CI_Model {
	...
	public function select_no_plates($table,$data){
		...
		$this->db->select("IFNULL(SUM(CASE WHEN artwork_para_id=9 THEN no_of_plates END ),0)offset,IFNULL(SUM(CASE WHEN artwork_para_id=10 THEN no_of_plates END ),0)screen,IFNULL(SUM(CASE WHEN artwork_para_id=18 THEN no_of_plates END),0)flexo");
		$this->db->from($table);				
		if(!empty($data)){
			foreach($data as $key => $value) {
