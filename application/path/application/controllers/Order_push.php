...
class Order_push extends CI_Controller {
    	...
    	if($module_row->module_name==='Sales'){
          ...
          if($formrights_row->view==1){
           ...
           // $this->load->view(ucwords($this->router->fetch_class()).'/active-title',$data);
            ...
            $to="";
            $order_closed_arr="";
            $data['order_master']=$this->sales_order_book_model->active_record_search_new('order_master',$search_data,'2021-01-01','2021-05-13',$this->session->userdata['logged_in']['company_id'],$order_closed_arr,'2021-04-01','2021-05-13');{
              $i=1;
              foreach($data['order_master'] as $orderr_row){
                echo $i."]".$orderr_row->order_no;
                echo "<br/>";
                $i++;


          $tally_sales_order_master_result=$this->common_model->select_one_details_record_noncompany('tally_sales_order_master','order_no',$orderr_row->order_no);
            ...
            if(count($tally_sales_order_master_result)==0){       
              ...
              $result=$this->common_model->update_one_active_record('order_master',$data,'order_no',$orderr_row->order_no,$this->session->userdata['logged_in']['company_id']);
              
              $order_no=$orderr_row->order_no;
              $order_date='';
              $sales_ledger='';
              $bill_to='';
              $ship_to='';
              ...
              $this->load->view('Home/footer');
               ...
               $abc=array('order_no'=>$this->input->post('order_no'),
                ...
                $data['employee']=$this->common_model->select_one_active_record('employee_master',$this->session->userdata['logged_in']['company_id'],'employee_id',$this->session->userdata['logged_in']['user_id']);
                        ...
                        $this->load->library('email', $config);
                        $this->email->from("auto.mailer@3d-neopac.com");
                        $this->email->to("planning@3d-neopac.com,erp@3d-neopac.com");
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            ...
            if($this->form_validation->run()==FALSE){
              ...
              if(!empty($this->input->post('for_stock'))){
  ...
  // function search_spring_track_order(){
  ...
  //           $this->load->view('Home/subnav');
  //           $this->load->view(ucwords($this->router->fetch_class()).'/active-title',$data);
  //           $this->load->view(ucwords($this->router->fetch_class()).'/search-form-spring-track',$data);
  //           $this->load->view('Home/footer');
  //         }else{
  //           $data['note']='No rights Thanks';
  //           $this->load->view('Home/header');
  ...
  // function search_result_spring_track_order(){
  ...
  //           $this->form_validation->set_rules('order_no','Sales order' ,'required|trim|xss_clean');
  ...
  //             $data['order_master']=$this->sales_order_book_model->active_record_search('order_master',$search_data,$from,$to,$this->session->userdata['logged_in']['company_id'],$order_closed_arr);
