      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            ...
            $this->form_validation->set_rules('cust_order_no','PO No' ,'trim|xss_clean');
            $this->form_validation->set_rules('cust_order_date','PO Date' ,'trim|xss_clean');
            $this->form_validation->set_rules('cap_code','Cap Code' ,'trim|xss_clean|callback_article_check');
            ...
            if(!empty($this->input->post('cancelled_from_date')) && !empty($this->input->post('cancelled_to_date'))){
              ...
              $this->form_validation->set_rules('cancelled_from_date','From Date' ,'trim|xss_clean|max_length[10]');
              $this->form_validation->set_rules('cancelled_to_date','To Date' ,'trim|xss_clean|max_length[10]');
              ...
              $data['user_master']=$this->common_model->select_active_drop_down('user_master',$this->session->userdata['logged_in']['company_id']);
                ...
                $this->load->view('Home/footer');
                 ...
                 /* if(!empty($this->input->post('cap_code'))){
                  ...
                  //print_r($search);
                 $data['order_master']=$this->sales_order_status_model->active_record_search_new('order_master',$search,$from,$to,$this->session->userdata['logged_in']['company_id'],$order_closed_arr,$approval_from_date,$approval_to_date,$cancelled_from_date,$cancelled_to_date);
