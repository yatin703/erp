...
class Springtube_aql_rfd extends CI_Controller {
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->new==1){
              ...
              $this->form_validation->set_rules('shoulder_short_shot','Shoulder Short shot' ,'trim|xss_clean');
              $this->form_validation->set_rules('cap_gap_issue','Cap Gap Issue' ,'trim|xss_clean');
              ...
              $this->form_validation->set_rules('peel_off_varnish_cut','Peel off and Varnish Cut' ,'trim|xss_clean');
              
              $this->form_validation->set_rules('other_print_defects','Other printing defects' ,'trim|xss_clean');

              $this->form_validation->set_rules('counter_sample_qty','Counter sample tubes' ,'trim|xss_clean|greater_than_equal_to[0]|less_than_equal_to['.$this->input->post('inspection_qty').']');
              ...
              if($this->form_validation->run()==FALSE){
                ...
                $peel_off_varnish_cut=0;

                $total_rejected_printing_issue=0;

                $total_rejected_qty=0;

                $counter_sample_qty=0;
                ...
                if(!empty($this->input->post('shoulder_short_shot '))){
                  $shoulder_short_shot=$this->input->post('shoulder_short_shot');

                }
                if(!empty($this->input->post('cap_gap_issue'))){
                  $cap_gap_issue=$this->input->post('cap_gap_issue');
                ...
                if(!empty($this->input->post('peel_off_varnish_cut'))){
                    $other_print_defects=$this->input->post('peel_off_varnish_cut');
                }
                ...
                if(!empty($this->input->post('counter_sample_qty'))){

                    $counter_sample_qty=$this->input->post('counter_sample_qty');
                }
                ...
                $total_microns=$micron_1+$micron_2+$micron_3+$micron_4+$micron_5+$micron_6+$micron_7; 
                 ...
                 $data=array(
                  ...
                  'shoulder_short_shot'=>$shoulder_short_shot,
                  'cap_gap_issue'=>$cap_gap_issue,
                ...
                if($aql_id){
                    ...
                    // Inserting in to RFD table------------------------

                  $data_rfd=array(
                  'rfd_date'=>$this->input->post('aql_date'),
                  'jobcard_no'=>$jobcard_no,                  
                  'customer'=>$customer,
                  'order_no'=>$order_no,
                  'article_no'=>$article_no,
                ...
                $rfd_id=$this->common_model->save_return_pkey('springtube_rfd_master',$data_rfd);
            ...
            if($formrights_row->modify==1){
              ...
              if($this->form_validation->run()==FALSE){
                ...
                // }
                    ...
                    $data_search=array('status'=>1,'jobcard_no'=>$jobcard_no,'archive'=>0,'ref_aql_id'=>$this->input->post('aql_id'));
  ...
  function spring_aql_input_boxes($jobcard_no,$qty){
