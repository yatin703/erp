...
function search_qc_hold_monthly(){    
        ...
        if($module_row->module_name==='Production'){
          ...
          foreach ($data['formrights'] as $formrights_row) {
            ...
            if($formrights_row->view==1){
              ...
              $data_search=array();
              $data['springtube_extrusion_qc_master']=$this->springtube_extrusion_qc_model->active_record_search_groupby('springtube_extrusion_qc_master',$this->session->userdata['logged_in']['company_id'],$data_search,$from_date,$to_date);
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()).'/active-title',$data);
              $this->load->view(ucwords($this->router->fetch_class()).'/search-form-qchold-monthly',$data);
  ...
  function search_result_qc_hold_monthly(){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            ...
            if($this->form_validation->run()==FALSE){
              ...
              $data['springtube_extrusion_qc_master']=$this->springtube_extrusion_qc_model->active_record_search_groupby('springtube_extrusion_qc_master',$this->session->userdata['logged_in']['company_id'],$data_search,$this->input->post('from_date'),$this->input->post('to_date')); 
             //echo $this->db->last_query();            
