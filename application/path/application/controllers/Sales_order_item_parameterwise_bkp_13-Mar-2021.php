...
class Sales_order_item_parameterwise extends CI_Controller {
      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            ...
            $data['extrusion_additional']=$this->common_model->select_one_active_record_nonlanguage_without_archives('reserved_quantity_manu',$this->session->userdata['logged_in']['company_id'],$dataa);
  ...
  function save_job_card(){
    ...
    echo $this->input->post('order_flag');
          ...
          if($formrights_row->view==1){
            ...
            if($this->form_validation->run()==FALSE){
              ...
              if(!empty($this->input->post('adr_company_id'))){
                ...
                $data['order_master']=$this->sales_order_item_parameterwise_model->active_record_search_new('order_master',$search,$from,$to,$approval_from_date,$approval_to_date,$delivery_from_date,$delivery_to_date,$this->session->userdata['logged_in']['company_id'],$order_by);
