...
class Springtube_extrusion_wip extends CI_Controller {
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            ...
            if($this->form_validation->run()==FALSE){
              ...
              $data['springtube_extrusion_wip_master']=$this->springtube_extrusion_wip_model->active_record_search('springtube_extrusion_wip_master',$this->session->userdata['logged_in']['company_id'],$data_search,$this->input->post('from_date'),$this->input->post('to_date')); 
             //echo $this->db->last_query();            
          ...
          foreach ($data['formrights'] as $formrights_row) {
            if($formrights_row->new==1){
              ...
              if($this->input->post('release_to')=='1' && strpos($this->input->post('order_no'),"ST")!=''){
                ...
                $this->form_validation->set_rules('release_article_no','Release To Article No' ,'required|trim|xss_clean|max_length[50]|callback_check_specification');
                  ...
                  if($this->input->post('release_to')=='1'){
                    ...
                    if($pending_meters>0){ 
                      ...
                      $data=array(
                        ...
                        $data=array( 'status'=>'1',
                                     'release_to_order_no'=>$this->input->post('release_to_order_no'), 
  ...
  function check_specification(){
    ...
    $release_to_order_details_result=$this->common_model->select_active_records_where('order_details',$this->session->userdata['logged_in']['company_id'],$data_order_details);
    ...
    foreach($film_code_result as $film_code_row){                   
      $release_to_film_spec_id=$film_code_row->spec_id;
      $release_to_film_spec_version=$film_code_row->spec_version_no;
    }
    ...
    if($sleeve_diameter!=$release_to_sleeve_diameter){
      ...
      $this->form_validation->set_message('check_specification', 'The Release Order Dia Not Matches');
      $flag_sleeve_diameter=0;
    }
    if($micron_1!=$release_to_micron_1){

      $this->form_validation->set_message('check_specification', 'The Release Order Layer 1 - Guage Not Matches');
      $flag_micron_1=0;
    }


    if($flag_sleeve_diameter==1 && $flag_micron_1==1 ){
