...
class Label_specification extends CI_Controller {
      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            ...
            include('pagination-label.php');
            $data['specification']=$this->label_specification_model->select_active_records($config["per_page"],$this->uri->segment(3),$table,$this->session->userdata['logged_in']['company_id']);
  ...
  function view_label(){
    ...
    foreach ($data['module'] as $module_row) {
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
            $data['specification']=$this->label_specification_model->select_one_active_record('specification_sheet',$this->session->userdata['logged_in']['company_id'],'specification_sheet.spec_id',$this->uri->segment(3),'specification_sheet.spec_version_no',$this->uri->segment(4));
      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            $this->load->view(ucwords($this->router->fetch_class()).'/active-title',$data);
            $this->load->view(ucwords($this->router->fetch_class()).'/create-label-form',$data);
            $this->load->view('Home/footer');
          }else{
            $data['note']='No rights Thanks';
            $this->load->view('Home/header');
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
            ...
            */
            ...
            $this->form_validation->set_rules('label_no','Label' ,'required|trim|xss_clean|callback_article_check');
            $this->form_validation->set_rules('lacquer_type_1','Lacquer One' ,'required|trim|xss_clean');
            $this->form_validation->set_rules('non_lacquering_height_by_open_end','Non Lacquering Height By Open End' ,'required|trim|xss_clean');
            $this->form_validation->set_rules('non_labeling_height_by_shoulder_end','Non Labeling Height By Shoulder End' ,'required|trim|xss_clean');

           $this->form_validation->set_rules('lacquer_mixing_pc_1','Lacquer One Mixing %' ,'trim|xss_clean|is_natural|max_length[3]|callback_lacquer_per_check');
            $this->form_validation->set_rules('lacquer_mixing_pc_2','Lacquer Two Mixing %' ,'trim|xss_clean|is_natural|max_length[3]|callback_lacquer_per_check');

            if(!empty($this->input->post('lacquer_mixing_pc_1'))){
              $this->form_validation->set_rules('lacquer_type_1','Lacquer' ,'trim|xss_clean|required');
            }if(!empty($this->input->post('lacquer_mixing_pc_2'))){
              $this->form_validation->set_rules('lacquer_type_2','Lacquer' ,'trim|xss_clean|required');
            }
            ...
            $data['approval_authority']=$this->common_model->select_one_active_approval_authority_record('authority_master',$this->session->userdata['logged_in']['company_id'],'authority_master.form_id','985');
              ...
              if(!empty($this->input->post('label_no'))){
                ...
                $data['label']=$this->article_model->select_one_active_record('article',$this->session->userdata['logged_in']['company_id'],'article.article_no',$label_no[1]);

                  foreach($data['label'] as $label_row){
                    $label_name=$label_row->article_name;
                 }
              ...
              $article_description=$label_no[0]." ".$lacquer_type_1." ".$lacquer_mixing_pc_1." ".$lacquer_type_2." ".$lacquer_mixing_pc_2." ".$non_lacquering_height_by_open_end." ".$non_labeling_height_by_shoulder_end;
              ...
              $data['article']=$this->article_model->select_one_active_record('article',$this->session->userdata['logged_in']['company_id'],'article_name_info.lang_article_description',$article_description);
                      ...
                      if($data['article']==FALSE){
                        ...
                        $data['auto']=$this->common_model->select_one_active_record_nonlanguage_without_archive('autogeneration_format_master',$this->session->userdata['logged_in']['company_id'],'form_id','1241');
                        $no="";
                        ...
                        $this->common_model->update_one_active_record('autogeneration_format_master',$data,'form_id','1241',$this->session->userdata['logged_in']['company_id']);
                        $spec_version_no='1';
              ...
              $result=$this->common_model->save('specification_sheet_details',$data);
                ...
                'item_group_id'=>'6',
                'item_group_flag'=>'1',
                'parameter_name'=>'LACQUER ONE',
                'parameter_value'=>'',
                'material'=>'1',
                'item_group_material'=>'14',
                'supplier_no'=>'',
                'relating_master_value'=>'',
                'mat_info'=>$this->input->post('lacquer_mixing_pc_1'),
                'property_id'=>'',
                'mat_article_no'=>$this->input->post('lacquer_type_1'),
                'srd_id'=>'8_2',
                'item_group_material_flag'=>'1',
                'dyn_qty_no'=>'1',
                'sub_company_id'=>$this->session->userdata['logged_in']['company_id'],
              ...
              $data=array('company_id'=>$this->session->userdata['logged_in']['company_id'],
                ...
                'item_group_id'=>'6',
                'item_group_flag'=>'1',
                'parameter_name'=>'LACQUER TWO',
                'material'=>'1',
                'item_group_material'=>'14',
                'parameter_value'=>'',
                'relating_master_value'=>'',
                'supplier_no'=>'',
                'mat_info'=>$this->input->post('lacquer_mixing_pc_2'),
                'property_id'=>'',
                'mat_article_no'=>$this->input->post('lacquer_type_2'),
                'srd_id'=>'8_3',
                'item_group_material_flag'=>'1',
                'dyn_qty_no'=>'1',
                'sub_company_id'=>$this->session->userdata['logged_in']['company_id'],
              ...
              $data=array('company_id'=>$this->session->userdata['logged_in']['company_id'],
                ...
                'material'=>'',
                'item_group_material'=>'',
                'parameter_name'=>'NON LACQUERING HEIGHT BY OPEN END',
                'parameter_value'=>$this->input->post('non_lacquering_height_by_open_end'),
                'relating_master_value'=>'',
                'supplier_no'=>'',
                'mat_info'=>'',
                'property_id'=>'',
              ...
              $data=array('company_id'=>$this->session->userdata['logged_in']['company_id'],
                ...
                'material'=>'',
                'item_group_material'=>'',
                'parameter_name'=>'NON LABELING HEIGHT BY SHOULDER END',
                'parameter_value'=>$this->input->post('non_labeling_height_by_shoulder_end'),
                'relating_master_value'=>'',
                'supplier_no'=>'',
                'mat_info'=>'',
                'property_id'=>'',
              ...
              if(!empty($this->input->post('approval_authority'))){
                    ...
                    $result=$this->label_specification_model->update_one_active_record('specification_sheet',$data,'spec_id',$spec_no,'spec_version_no',$spec_version_no,$this->session->userdata['logged_in']['company_id']);
                ...
                $data['error']='Same Label Component is Exist';
              }
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->modify==1){
            ...
            $data['specification']=$this->tube_specification_model->select_one_active_unapproved_record('specification_sheet',$this->session->userdata['logged_in']['company_id'],'specification_sheet.spec_id',$this->uri->segment(3),'specification_sheet.spec_version_no',$this->uri->segment(4),'specification_sheet.user_id',$this->session->userdata['logged_in']['user_id']);
              ...
              $this->load->view(ucwords($this->router->fetch_class()).'/label-modify-form',$data);
              $this->load->view('Home/footer');
          }else{
              $data['note']='No Modify rights Thanks';
              $this->load->view('Home/header');
          ...
          foreach ($data['formrights'] as $formrights_row) {
           ...
           $this->form_validation->set_rules('lacquer_mixing_pc_1','Lacquer One Mixing %' ,'trim|xss_clean|is_natural|max_length[3]|callback_lacquer_per_check');
            ...
            if($this->form_validation->run()==FALSE){
              ...
              $article_description=$label_name." ".$lacquer_type_1." ".$lacquer_mixing_pc_1." ".$lacquer_type_2." ".$lacquer_mixing_pc_2." ".$non_lacquering_height_by_open_end." ".$non_labeling_height_by_shoulder_end;
              ...
              $result=$this->common_model->update_one_active_record('article_name_info',$data,'article_no',$this->input->post('article_no'),$this->session->userdata['logged_in']['company_id']);
                  ...
                  $data=array('mat_article_no'=>$label_no[1]);

                  $result=$this->label_specification_model->update_details_record_where('specification_sheet_details',$data,'spec_id',$this->input->post('spec_id'),'spec_version_no',$this->input->post('spec_version_no'),'srd_id','8_1',$this->session->userdata['logged_in']['company_id']);

                  $data=array('mat_article_no'=>$this->input->post('lacquer_type_1'));

                  $result=$this->label_specification_model->update_details_record_where('specification_sheet_details',$data,'spec_id',$this->input->post('spec_id'),'spec_version_no',$this->input->post('spec_version_no'),'srd_id','8_2',$this->session->userdata['logged_in']['company_id']);

                  $data=array('mat_info'=>$this->input->post('lacquer_mixing_pc_1'));

                  $result=$this->label_specification_model->update_details_record_where('specification_sheet_details',$data,'spec_id',$this->input->post('spec_id'),'spec_version_no',$this->input->post('spec_version_no'),'srd_id','8_2',$this->session->userdata['logged_in']['company_id']);

                  $data=array('mat_article_no'=>$this->input->post('lacquer_type_2'));

                  $result=$this->label_specification_model->update_details_record_where('specification_sheet_details',$data,'spec_id',$this->input->post('spec_id'),'spec_version_no',$this->input->post('spec_version_no'),'srd_id','8_3',$this->session->userdata['logged_in']['company_id']);

                  $data=array('mat_info'=>$this->input->post('lacquer_mixing_pc_2'));

                  $result=$this->label_specification_model->update_details_record_where('specification_sheet_details',$data,'spec_id',$this->input->post('spec_id'),'spec_version_no',$this->input->post('spec_version_no'),'srd_id','8_3',$this->session->userdata['logged_in']['company_id']);

                  $data=array('parameter_value'=>$this->input->post('non_lacquering_height_by_open_end'));

                  $result=$this->label_specification_model->update_details_record_where('specification_sheet_details',$data,'spec_id',$this->input->post('spec_id'),'spec_version_no',$this->input->post('spec_version_no'),'srd_id','8_4',$this->session->userdata['logged_in']['company_id']);
                 
                  $data=array('parameter_value'=>$this->input->post('non_labeling_height_by_shoulder_end'));

                  $result=$this->label_specification_model->update_details_record_where('specification_sheet_details',$data,'spec_id',$this->input->post('spec_id'),'spec_version_no',$this->input->post('spec_version_no'),'srd_id','8_5',$this->session->userdata['logged_in']['company_id']);
                  ...
                  $data['error']='No change in Label component';
                ...
                $data['specification']=$this->label_specification_model->select_one_active_record('specification_sheet',$this->session->userdata['logged_in']['company_id'],'specification_sheet.spec_id',$this->input->post('spec_id'),'specification_sheet.spec_version_no',$this->input->post('spec_version_no'),'specification_sheet.user_id',$this->session->userdata['logged_in']['user_id']);
      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->view==1){
              ...
              $this->form_validation->set_rules('label_no','Label Code' ,'trim|xss_clean|callback_article_check');
              $this->form_validation->set_rules('label_name','Label Material' ,'trim|xss_clean|callback_article_check');

              $this->form_validation->set_rules('lacquer_type_1','Lacquer Type 1' ,'trim|xss_clean');
              $this->form_validation->set_rules('lacquer_mixing_pc_1','Lacquer Type 1 %' ,'trim|xss_clean|numeric|max_length[3]');
              $this->form_validation->set_rules('lacquer_type_2','Lacquer Type 2' ,'trim|xss_clean');
              $this->form_validation->set_rules('lacquer_mixing_pc_2','Lacquer Type 2 %' ,'trim|xss_clean|numeric|max_length[3]');

              $this->form_validation->set_rules('non_lacquering_height_by_open_end','Non Lacquering Height by Open End' ,'trim|xss_clean');
              $this->form_validation->set_rules('non_labeling_height_by_shoulder_end','Non Lacquering Height by Shoulder End' ,'trim|xss_clean|numeric|max_length[3]');
              $this->form_validation->set_rules('final_approval_flag','Approval Status' ,'trim|xss_clean');


              if(!empty($this->input->post('label_code'))){
                  $label_code_arr=explode('//',$this->input->post('label_code'));
                }else{
                  $label_code_arr[1]='';
                }

                if(!empty($this->input->post('label_name'))){
                  $label_name_arr=explode('//',$this->input->post('label_name'));               }else{
                  $label_name_arr[1]='';
                }


                $master_array= array('a.article_no' => $label_code_arr[1],
                                      'a.final_approval_flag'=>$this->input->post('final_approval_flag'),
                                      'a.user_id'=>$this->input->post('user_id')
                                    );  
                $data1=array_filter($master_array,'strlen');

                $data_search=array(
                  'label_name'=>$label_name_arr[1],
                  'oe'=>$this->input->post('non_lacquering_height_by_open_end'),
                  'se'=>$this->input->post('non_labeling_height_by_shoulder_end'),                 
                  'label_lacquer_one'=>$this->input->post('lacquer_type_1'),
                  'label_lacquer_one_perc'=>$this->input->post('lacquer_mixing_pc_1'),
                  'label_lacquer_two'=>$this->input->post('lacquer_type_2'),
                  'label_lacquer_two_perc'=>$this->input->post('lacquer_mixing_pc_2'),
                );

                $search=array_filter($data_search,'strlen');
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->copy==1){
            ...
            $data['approval_authority']=$this->common_model->select_one_active_approval_authority_record('authority_master',$this->session->userdata['logged_in']['company_id'],'authority_master.form_id','985');
              ...
              $this->load->view('Home/subnav');
              $this->load->view(ucwords($this->router->fetch_class()).'/active-title');
              $this->load->view(ucwords($this->router->fetch_class()).'/copy-label-form',$data);
              $this->load->view('Home/footer');
          }else{
              $data['note']='No Modify rights Thanks';
              $this->load->view('Home/header');
...
public function lacquer_per_check($str){
    $lacquer_mixing_pc_1=$this->input->post('lacquer_mixing_pc_1');
    $lacquer_mixing_pc_2=(!empty($this->input->post('lacquer_mixing_pc_2')) ? $this->input->post('lacquer_mixing_pc_2') : '0');
    $total_per=$lacquer_mixing_pc_2+$lacquer_mixing_pc_1;

    if($total_per!=100){
      $this->form_validation->set_message('lacquer_per_check', 'The {field} field is incorrect');
      return FALSE;
    }else{
      return TRUE;
    }
