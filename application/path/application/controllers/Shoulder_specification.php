      ...
      if($module_row->module_name==='Sales'){
        ...
        foreach ($data['formrights'] as $formrights_row) {
          if($formrights_row->new==1){
           ...
           //$this->form_validation->set_rules('sh_hdpe_one_per','Shoulder HDPE One %' ,'trim|xss_clean|is_natural|max_length[3]|in_list[30]|callback_shoulder_per_check');
            ...
            if(!empty($this->input->post('sh_hdpe_one_per'))){
              $this->form_validation->set_rules('sh_hdpe_one','Shoulder Hdpe One' ,'trim|xss_clean|required|in_list[RM-HDPE-000-0009]');
            }if(!empty($this->input->post('sh_hdpe_two_per'))){
              $this->form_validation->set_rules('sh_hdpe_two','Shoulder Hdpe Two' ,'trim|xss_clean|required|in_list[RM-HDPE-000-0008]');
            }
            ...
            if($data['shoulder_types_master']==FALSE){
             ...
             if($this->shoulder_orifice_dependancy_model->select_one_active_combination_record('shoulder_orifice_dependancy',$this->session->userdata['logged_in']['company_id'],$combination_data)>0){
              ...
              $article_description=$sleeve_dia[0]." ".$shoulder[0]." ".$shoulder_orifice[0]." ".$sh_masterbatch." ".$this->input->post('sh_mb_per')."% ".$sh_hdpe_one." ".$this->input->post('sh_hdpe_one_per')."% ".$sh_hdpe_two." ".$this->input->post('sh_hdpe_two_per')."% ".$shoulder_foil_tag;
