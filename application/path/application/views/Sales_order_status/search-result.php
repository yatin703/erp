...
<?php //echo 'class="record_table_design_without_fixed" ';?>
	<div class="record_inner_design" style="overflow: scroll;">
				...
				else 
				{
					...
					foreach($order_master as $mrow){
						...
						if(!empty($this->input->post('cap_code'))){
		                     $cap_arr=explode('//',$this->input->post('cap_code'));
		                     $cap_code=$cap_arr[1];
		                     $details_data['cap_code']=$cap_code;
		                  }
						$customer_category='';
				    ...
				    //          	}
				             $html.="</td>
				             <td rowspan='detail_rowspan'>".($mrow->trans_closed_date!='0000-00-00'?$this->common_model->view_date($mrow->trans_closed_date,$this->session->userdata['logged_in']['company_id']):"")."</td>
				            <td  rowspan='detail_rowspan'>";
							...
							$html.="</td>
							<td rowspan='detail_rowspan'>";
							$order_close_reasons_master_result=$this->common_model->select_one_active_record('order_close_reasons_master',$this->session->userdata['logged_in']['company_id'],'id',$mrow->trans_closed_reason);
							foreach ($order_close_reasons_master_result as $key => $order_close_reasons_master_row) {
								$html.=$order_close_reasons_master_row->reasons.", ".$mrow->trans_closed_remarks;
							}
							$html.="</td>							
							...
							foreach ($result as $drow){
								...
								$short_qty=0;
								$short_qty_amount=0;
								$order_qty=$this->common_model->read_number($drow->total_order_quantity,$this->session->userdata['logged_in']['company_id']);
								$supply_qty_1=$this->common_model->read_number($supply_qty,$this->session->userdata['logged_in']['company_id']);

								if($mrow->for_export==1){
									$unit_rate=0;
									$unit_rate=$drow->calc_sell_price;
									$net_amount=$ci->common_model->read_number($drow->total_order_quantity,$this->session->userdata['logged_in']['company_id'])*$unit_rate;

								}else{
									$unit_rate=0;
									$unit_rate=$this->common_model->read_number($drow->selling_price,$this->session->userdata['logged_in']['company_id']);
									$net_amount=$ci->common_model->read_number($drow->total_order_quantity,$this->session->userdata['logged_in']['company_id'])*$unit_rate; 
								}
								$sum_amount+=$net_amount;

								if($mrow->trans_closed=='1'){
									$short_qty=($order_qty>$supply_qty_1 && $supply_qty_1>0?($order_qty-$supply_qty_1):0);
									$short_qty_amount=$short_qty*$unit_rate;

									$sum_short_qty+=$short_qty;
									$sum_short_qty_amount+=$short_qty_amount;
								...
								$html.="<td rowspan='".$invoice_rowspan."' title='Article No'>".$drow->article_no."</td>
									...
									<td class='positive right aligned' rowspan='".$invoice_rowspan."' title='Quantity'>".number_format($ci->common_model->read_number($drow->total_order_quantity,$this->session->userdata['logged_in']['company_id']),0,'.',',')."</td>
									<td rowspan='".$invoice_rowspan."' title='Rate'>".$unit_rate."</td>
									<td rowspan='".$invoice_rowspan."' title='Net Amount'>".$net_amount."</td>
									";
									...
									$html.="</td>
									<td class='warning right aligned' rowspan='".$invoice_rowspan."' title='Supply Quantity'>".number_format($ci->common_model->read_number($supply_qty,$this->session->userdata['logged_in']['company_id']),0,'.',',')."</td>
									<td rowspan='".$invoice_rowspan."' title='Pending Qty'>".number_format(($pending_qty>0?$pending_qty:0),0,'.',',')."</td>
									<td rowspan='".$invoice_rowspan."' title='Short Qty'>".number_format(($short_qty>0?$short_qty:0),0,'.',',')."</td>
									<td rowspan='".$invoice_rowspan."' title='Short Qty Amount'>".$short_qty_amount."</td>
				...
				<?php 
					echo"<tr>
							...
							<td class='positive right aligned' title='order Qty'><b>".number_format($sum_order_quantity,0,'.',',')."</b></td>
							<td></td>
							<td title='Net amount'>".number_format($sum_amount,0,'.',',')."</td>
							<td title='Jobcard Qty'><b>".number_format($sum_total_jobcard_qty,0,'.',',')."</b></td>
							<td class='positive right aligned' title='Supply Qty'><b>".number_format($sum_invoice_quantity,0,'.',',')."</b></td>
							 
							<td title='Pending Qty'></td>

							<td title='Short Qty'>".number_format($sum_short_qty,0,'.',',')."</td>
							<td title='Short Amount'>".number_format($sum_short_qty_amount,0,'.',',')."</td>
							<td></td>
							<td></td>
							<td></td>
							<td title='Invoice Qty'><b>".number_format($sum_invoice_quantity,0,'.',',')."</b></td>
							<td title='Gross Amount'><b>".number_format($sum_net_amount,2,'.',',')."</b></td>
							<td title='Amount Received'><b>".number_format($sum_amount_received,2,'.',',')."</b></td>
							<td></td>
							<!--<td></td>
							<td></td>
							<td></td>
