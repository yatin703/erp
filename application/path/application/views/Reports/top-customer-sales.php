...
<script>
	$(document).ready(function(){
		...
		$("#search").click(function(){
			...
			$.ajax({type: "POST",url: "<?php echo base_url('index.php/ajax/top_customer_sales');?>",data: {from_date : $(".from_date").val(),to_date :$('.to_date').val(),convert:$('.convert').val(),sleeve_dia : $('.sleeve_dia:checked').serialize(),inv_type : $('.inv_type:checked').serialize()},cache: false,success: function(html){
				...
				<span id="check">
					...
					if($top_customer_coex==FALSE){
						...
						echo '<table class="ui sortable selectable celled table" style="font-size:9px;">
					        	<thead>
								   <tr>
								    	<th colspan="26"><a class="ui orange label">SALES </a>';
								    	if($account_periods_master==FALSE){
								    	}else{
								    		foreach ($account_periods_master as $account_periods_master_row ){
