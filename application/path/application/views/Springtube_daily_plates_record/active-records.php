			...
			<table class="record_table_design_without_fixed">
				...
				if($springtube_daily_plates_master==FALSE){
					echo "<tr><td colspan='19'>No Active Records Found</td></tr>";
								...
								echo"<tr>
									...
									echo "</td>									 
										...
										foreach($formrights as $formrights_row){ 
											...
											echo ($formrights_row->modify==1 && $row->final_approval_flag<>1 && $row->job_setup_status==0  ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->dpr_id).'"><i class="edit icon"></i></a> ' : '');										
											echo ($row->archive<>1 && $formrights_row->delete==1 && $row->final_approval_flag<>1 && $row->job_setup_status==0 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->dpr_id).'"><i class="trash icon"></i></a> ' : '');
							...
							$total_offset+=$row->total_plates;
							$total_sheet_used+=$row->sheet_used;
							// $total_screen+=$screen;
							// $total_flexo+=$flexo;
							...
							<tr>
								...
								<td><?php echo $total_sheet_used;?></td>
								<td></td>
								<td colspan=5></td>
							</tr>	 
