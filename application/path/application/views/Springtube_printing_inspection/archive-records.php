				...
				if($springtube_printing_inspection_master==FALSE){
					echo "<tr><td colspan='30'>No Active Records Found</td></tr>";
						...
						foreach($springtube_printing_inspection_master as $master_row){
							...
							$result=$this->springtube_printing_inspection_model->inactive_details_records('springtube_printing_inspection_details',$details_data);
					    	...
					    	if($rowspan>0){					    		
								...
								echo"<tr>
									...
									foreach ($formrights as $formrights_row) {
										...
										echo ($formrights_row->dearchive==1 ? ' | <a href="'.base_url('index.php/'.$this->router->fetch_class().'/dearchive/'.$master_row->production_id.'').'" title="Delete" target="_blank">Dearchive</a> ' : '');
									...
									foreach ($result as $details_row){
										...
										<td>".$details_row->printing_input_qty."</td>
										<!--<td>".$details_row->inspected_output_meters."</td>-->
										<td>".$details_row->inspected_output_qty."</td>
										<td>".($details_row->printing_input_qty-$details_row->inspected_output_qty)."</td>
										<!--<td>";

										$sum_printing_input_qty+=$details_row->printing_input_qty;
										//$sum_inspected_output_meters+=$details_row->inspected_output_meters;
										$sum_inspected_output_qty+=$details_row->inspected_output_qty;

										$sum_waste_qty+=($details_row->printing_input_qty-$details_row->inspected_output_qty);
						...
						echo"<tr><td colspan='15' style='text-align:right;'><b>TOTAL</b></td><td>".$sum_printing_input_qty."</td> <td>".$sum_inspected_output_qty."</td><td>".$sum_waste_qty."</td><td></td><td></td><td></td><td></td></tr>";
