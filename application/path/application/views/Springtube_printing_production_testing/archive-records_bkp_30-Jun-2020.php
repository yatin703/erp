			...
			<table class="record_table_design_without_fixed">
				...
				if($springtube_printing_production_master==FALSE){
					...
					foreach($springtube_printing_production_master as $master_row){
						...
						$result=$this->springtube_printing_production_model->select_inactive_shiftwise_master_records('springtube_printing_production_master',$details_data,$this->session->userdata['logged_in']['company_id']);
		    			...
		    			if($rowspan>0){
		    				...
		    				foreach($formrights as $formrights_row){ 
								echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/view/'.$master_row->production_date.'/'.$master_row->shift.'').'" target="_blank"><i class="print icon"></i></a> ' : '');
							}	

							echo '</td>
							<td rowspan="'.$rowspan.'">'.$n++.'</td>
							<td rowspan="'.$rowspan.'">'.$master_row->production_date.'</td>
							...
							foreach ($springtube_printing_production_details_result as $springtube_printing_production_details_row) {
								...
								<td>".$drow->user_id."</td>
								<td>".$drow->job_started_on."</td>
								<td>".$drow->job_ended_on."</td>
								...
								<td >";
									foreach ($formrights as $formrights_row) {
										...
										echo ($formrights_row->dearchive==1 ? ' | <a href="'.base_url('index.php/'.$this->router->fetch_class().'/dearchive/'.$drow->production_id.'').'" title="Dearchive" target="_blank"><i class="recycle icon"></i></a> ' : '');
