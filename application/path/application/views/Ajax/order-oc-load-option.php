...
<div class="record_form_design">
		...
		<?php if(!empty($order_no)){ echo "<p class='alert alert-success'>Order confirmation date of $order_no is ".$this->common_model->view_date($oc_date,$this->session->userdata['logged_in']['company_id'])."</p>";}?>
				...
				else 
				{
					...
					foreach($order_master as $mrow){
						...
						document.getElementsByName("oc_date")[0].setAttribute('min', today);
							...
							$("#oc_date_<?php echo $i;?>").on('blur',function(){
							...
							$("#update_<?php echo $i;?>").on('click',function(){
								...
								if($('#oc_date_<?php echo $i;?>').val()==""){
									...
									$.ajax({type: "POST",url: "<?php echo base_url(); ?>" + "index.php/ajax/oc_date_confirmation",data: {order_no : $('#order_no_<?php echo $i;?>').val(),article_no : $('#article_no_<?php echo $i;?>').val(),oc_date:$('#oc_date_<?php echo $i;?>').val(),order_created_by:$('#created_by_<?php echo $i;?>').val()},cache: false,success: function(html){
					...
					echo "<tr>
							...
							<td rowspan='".$rowspan."'><input type='hidden' name='order_no' id='order_no_".$i."' value='".$mrow->order_no."'>".($mrow->final_approval_flag==1 ? "<i class='check circle icon'></i>" : "")." <a href=".base_url('index.php/'.$this->router->fetch_class().'/view/'.$mrow->order_no)." target='_blank'>".$mrow->order_no."</a></td>
							...
							foreach ($artwork_details_result as $artwork_details_row) {
								...
								<!--<td>".$lacquer."</td>
					    			...
					    			if($r==0){
					    				...
					    				echo "<td  rowspan='".$rowspan."'>".($mrow->for_export==1 ? "EXPORT":"LOCAL")."</td>
						            	<td  rowspan='".$rowspan."'>".($mrow->for_sampling==1 ? "SAMPLE":"")."</td>

					    					<td class='ellipses' rowspan='".$rowspan."'>
										...
										*/
						            	...
						            	<td>".($mrow->max_lead_time!='' ? $mrow->max_lead_time." Days" : "")."</td>
								            <td><input type='date' id='oc_date_".$i."' class='oc_date' name='oc_date'></td>
