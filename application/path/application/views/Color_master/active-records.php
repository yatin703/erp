					...
					echo "<tr><td colspan='9'>No Active Records Found</td></tr>";
							...
							foreach($color_master as $row){
								echo "<tr>
									...
									echo ($formrights_row->modify=='1' ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->color_id).'" title="Modify"><i class="edit icon"></i>|</a> ' : '');

									echo ($formrights_row->delete=='1' ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->color_id).'" title="Delete"><i class="trash icon"></i></a> ' : '');
