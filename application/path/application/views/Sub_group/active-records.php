				...
				<?php if($sub_group==FALSE){
					echo "<tr><td colspan='14'>No Active Records Found</td></tr>";
							...
							foreach($sub_group as $row){
								...
								echo "<tr>
									<td>$row->article_group_id</td>
									<td>".strtoupper($row->main_group)."</td>
									<td>".strtoupper($row->sub_group)."</td>
									<td>".strtoupper($row->sub_group_short_id)."</td>
									<td>$row->category</td>
									<td>$row->tariff_no</td>
									...
									foreach($formrights as $formrights_row){ 
										echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/view/'.$row->article_group_id.'').'" target="_blank">View</a> ' : '');
										echo ($formrights_row->copy==1 ? '<a href="#">Copy</a> ' : '');
										echo ($formrights_row->modify==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->article_group_id.'').'">Modify</a> ' : '');
										echo ($row->archive<>1 && $formrights_row->delete==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->article_group_id.'').'">Delete</a> ' : '');
									}
									echo "</td>
							</tr>";
							}
