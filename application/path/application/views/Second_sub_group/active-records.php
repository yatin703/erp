							...
							foreach($second_sub_group as $row){
								...
								echo "<tr>
									...
									foreach($formrights as $formrights_row){ 
										echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/view/'.$row->sub_sub_grp_id.'').'" target="_blank">View</a> ' : '');
										echo ($formrights_row->copy==1 ? '<a href="#">Copy</a> ' : '');
										echo ($formrights_row->modify==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->sub_sub_grp_id.'').'">Modify</a> ' : '');
										echo ($row->archive<>1 && $formrights_row->delete==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->sub_sub_grp_id.'').'">Delete</a> ' : '');
									}
									echo "</td>
							</tr>";
							}
