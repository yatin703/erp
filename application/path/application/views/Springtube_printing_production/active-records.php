				...
				if($springtube_printing_production_master==FALSE){
					...
					foreach($springtube_printing_production_master as $master_row){
						...
						if(!empty($this->input->post('order_no'))){
						 	...
						 	$details_data['springtube_printing_production_master.order_no']=$this->input->post('order_no');
						}
						if(!empty($this->input->post('jobcard_no'))){
						 	
						 	$details_data['springtube_printing_production_master.jobcard_no']=$this->input->post('jobcard_no');
						}
						...
						if(!empty($this->input->post('article_no'))){
						 	...
						 	$article_no=$arr[1];
						 	$details_data['springtube_printing_production_master.article_no']=$article_no;
						}

						if(!empty($this->input->post('customer'))){
						 	$customer_arr=explode("//",$this->input->post('customer'));
						 	$customer=$customer_arr[1];
						 	$details_data['springtube_printing_production_master.customer']=$customer;
						}

						if(!empty($this->input->post('sleeve_dia'))){
						 	
						 	$details_data['springtube_printing_production_master.sleeve_dia']=$this->input->post('sleeve_dia');
						}

						if(!empty($this->input->post('sleeve_length'))){
						 	
						 	$details_data['springtube_printing_production_master.sleeve_length']=$this->input->post('sleeve_length');
						}

						if(!empty($this->input->post('job_type'))){
						 	
						 	$details_data['springtube_printing_production_master.job_type']=$this->input->post('job_type');
						}

						if(!empty($this->input->post('job_category'))){
						 	
						 	$details_data['springtube_printing_production_master.job_category']=$this->input->post('job_category');
						}

						if(!empty($this->input->post('laminate_color'))){
						 	
						 	$details_data['springtube_printing_production_master.laminate_color']=$this->input->post('laminate_color');
						}
						if(!empty($this->input->post('print_type'))){
						 	
						 	$details_data['springtube_printing_production_master.print_type']=$this->input->post('print_type');
						}
						if(!empty($this->input->post('shift_issues'))){
						 	
						 	$details_data['springtube_printing_production_master.shift_issues']=$this->input->post('shift_issues');
						}
						if($this->input->post('inspection_done')!=''){
						 	
						 	$details_data['inspection_done']=$this->input->post('inspection_done');
						}
						if($this->input->post('job_setup')!=''){
						 	
						 	$details_data['springtube_printing_jobsetup_master.jobcard_no']=($this->input->post('job_setup')=='1'?'IS NOT NULL':'IS NULL');
						}
		    			...
		    			if($rowspan>0){		    				
							...
							foreach ($result as $drow){
								...
								//------TOTAl PRINTING QTY from Production
								//echo $drow->shift_issues;
								
								if($drow->shift_issues=='14'){
									//echo $drow->production_id;								

									$data_total_counter=array(                  
					                'jobcard_no'=>$drow->jobcard_no                        
						            );
						            ...
						            foreach ($result_total_counter as  $total_counter_row) {
						                $total_printing_qty=($total_counter_row->total_counter*$ups);
						            }					            

									//$scrap_qty=-100;

						            $scrap_qty=$jobcard_qty-$total_printing_qty; 
							...
							*/
								...
								<td class='negative right aligned'><b>".number_format($scrap_qty,0,'.',',')."</b>
								</td>
								<td class='negative right aligned'><b>".($drow->extrusion_scrap!=''?number_format($drow->extrusion_scrap,0,'.',','):'')."</b>
								</td>
								...
								<td>".$drow->print_type."</td>
								<td>".$drow->body_making_type."</td>
								<td>".($cold_foil_2!=''?$this->common_model->get_article_name($cold_foil_1,$this->session->userdata['logged_in']['company_id']).' ,'.$this->common_model->get_article_name($cold_foil_2,$this->session->userdata['logged_in']['company_id']):$this->common_model->get_article_name($cold_foil_1,$this->session->userdata['logged_in']['company_id']))."</td>	
								<td>".$drow->job_type."</td>
								...
								<td >";
									foreach ($formrights as $formrights_row) {
										...
										echo ($formrights_row->modify==1 && $inspection_done==0 ? ' | <a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$drow->production_id.'').'" title="Modify" target="_blank"><i class="edit icon"></i></a>' : '');
							...
							$sum_extrusion_scrap+=$drow->extrusion_scrap;
					...
					echo'<tr style="color:blue;font-weight:bold;"><td colspan="10" style="text-align:right;">TOTAL</td>
							...
							<td class="negative right aligned"><b>'.number_format($sum_scrap_qty,0,'.',',').'</b></td>
							<td class="negative right aligned"><b>'.number_format($sum_extrusion_scrap,0,'.',',').'</b></td>
							<td></td>
							<td></td>
							<td></td>
							<td></td>
