<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    $table='specification_sheet';
  	...
  	echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'').'" class="ui teal tag label">Cap</a> : ' : '');


    echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/create_cap').'" class="ui blue tag label" ><i class="add circle icon"></i>Create</a> : ' : '');
