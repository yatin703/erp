							...
							foreach($specification as $row){
								...
								echo "<tr>
									...
									foreach($formrights as $formrights_row){ 
										...
										echo ($formrights_row->copy==1 && $row->final_approval_flag==1 && substr($row->dyn_qty_present,7,1)==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/copy_single/'.$row->spec_id.'/'.$row->spec_version_no).'" target="_blank"><i class="copy icon"></i></a> ' : '');

										echo ($formrights_row->modify==1 && $row->final_approval_flag<>1 && $row->pending_flag<>1  && $row->user_id==$this->session->userdata['logged_in']['user_id'] && substr($row->dyn_qty_present,7,1)==2 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify_two_layer_with_cap/'.$row->spec_id.'/'.$row->spec_version_no).'"><i class="edit icon"></i></a> ' : '');

										echo ($formrights_row->copy==1 && $row->final_approval_flag==1 && substr($row->dyn_qty_present,7,1)==2 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/copy_two/'.$row->spec_id.'/'.$row->spec_version_no).'" target="_blank"><i class="copy icon"></i></a> ' : '');

										echo ($formrights_row->modify==1 && $row->final_approval_flag<>1 && $row->pending_flag<>1  && $row->user_id==$this->session->userdata['logged_in']['user_id'] && substr($row->dyn_qty_present,7,1)==3 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify_three_layer_with_cap/'.$row->spec_id.'/'.$row->spec_version_no).'"><i class="edit icon"></i></a> ' : '');

										echo ($formrights_row->copy==1 && $row->final_approval_flag==1 && substr($row->dyn_qty_present,7,1)==3 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/copy_three/'.$row->spec_id.'/'.$row->spec_version_no).'" target="_blank"><i class="copy icon"></i></a> ' : '');

										echo ($formrights_row->modify==1 && $row->final_approval_flag<>1 && $row->pending_flag<>1  && $row->user_id==$this->session->userdata['logged_in']['user_id'] && substr($row->dyn_qty_present,7,1)==5 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify_five_layer_with_cap/'.$row->spec_id.'/'.$row->spec_version_no).'"><i class="edit icon"></i></a> ' : '');

										echo ($formrights_row->copy==1 && $row->final_approval_flag==1 && substr($row->dyn_qty_present,7,1)==5 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/copy_five/'.$row->spec_id.'/'.$row->spec_version_no).'" target="_blank"><i class="copy icon"></i></a> ' : '');
