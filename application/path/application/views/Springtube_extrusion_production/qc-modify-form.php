	...
	$(document).ready(function(){
		...
		$("#total_ok_meters").live('keyup',function(){
			...
			if($("#total_ok_meters").val()!=''){
				...
				if($("#total_meters_produced").val()>=$("#total_ok_meters").val()){
...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/update_qc_save');?>" method="POST" >
							...
							<select name="qc_hold_reasons" id="qc_hold_reasons" ><option value=''>--Select QC Hold Reason--</option>
								<?php if($springtube_extrusion_reel_scrap_reasons==FALSE){
									...
									foreach($springtube_extrusion_reel_scrap_reasons as $reason_row){
										...
										$selected=($reason_row->id==$detail_row->qc_hold_reasons?'selected':'');
										
										echo "<option value='".$reason_row->id."'  ".set_select('qc_hold_reasons',$reason_row->id)."".$selected.">".$reason_row->scrap_reason."</option>";
									}
								}?>
							</select>
						</td>
						<td>
							<textarea name="qc_remarks" id="qc_remarks"  value="<?php echo set_value('qc_remarks',trim($detail_row->qc_remarks));?>" maxlength="500" style="height: 40px;width:250px"><?php echo set_value('qc_remarks',trim($detail_row->qc_remarks));?>	
							</textarea>	
						</td>
						<td>
