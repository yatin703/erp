...
<div class="record_form_design">
    ...
    //           	}
				...
				$result=$ci->sales_order_book_model->active_details_records_new('order_details',$details_data,$this->session->userdata['logged_in']['company_id'],$delivery_from_date,$delivery_to_date);
			    ...
			    if($rowspan>0){
					...
					<td rowspan='".$rowspan."'>".($mrow->order_closed==1 ? "<a class='ui blue label'>COMPLETE</a>" : ($mrow->trans_closed<>1 ? ($mrow->hold_flag == 1  ? "<a class='ui red label'>HOLD BY ".strtoupper($ci->common_model->get_user_name($mrow->hold_by,$this->session->userdata['logged_in']['company_id']))."</a>" : "<a class='ui green label'>UNHOLD</a>") : "<a class='ui blue label'>CLOSED</a>"))."</td>
