				...
				else 
				{
					...
					foreach($order_master as $mrow){
					    ...
					    if($rowspan>0){
							...
							<td rowspan='".$rowspan."'>".($mrow->order_closed==1 ? "<a class='ui blue label'>COMPLETE</a>" : ($mrow->trans_closed<>1 ? ($mrow->hold_flag == 1  ? "<a class='ui red label'><i>HOLD BY ".strtoupper($ci->common_model->get_user_name($mrow->hold_by,$this->session->userdata['logged_in']['company_id']))."</i></a>" : "<a class='ui green label'>UNHOLD</a>") : ''))."</td>
