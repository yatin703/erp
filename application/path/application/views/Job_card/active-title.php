<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    $table='production_master';
  	...
  	//echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/create').'" class="ui blue tag label">Create</a> : ' : '');
    ...
    echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/mannual_issue').'" class="ui blue tag label"><i class="add circle icon"></i>Mannual Issue Form</a> : ' : '');

    echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/search_manual').'" class="ui tag label"><i class="search icon"></i>Mannual Issue Search </a> : ' : '');
