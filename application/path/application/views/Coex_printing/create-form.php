...
<script>
	...
	$(document).ready(function(){
		...
		$("#machine").live('change',function() {
			...
			$.ajax({type: "POST",url: "<?php echo base_url('index.php/ajax/printing_machine_start');?>",data: {printing_date : $("#printing_date").val(),shift:$("#shift").val(),machine:$("#machine").val()},cache: false,success: function(html){
		...
		$("#machine_start").live('click',function() {
			$("#loading").show(); $("#cover").show();
			$('#loading').html('<img src="<?php echo base_url('assets/img/loading.gif');?>"> Loading...');
			var job_no=$("#mp_pos_no").val();
			if($("#coex_printing_machine_start_reasons").val()!="" && $("#mp_pos_no").val()!="" && job_no.length==14){
				$.ajax({type: "POST",url: "<?php echo base_url('index.php/ajax/printing_machine_start_entry');?>",data: {machine:$("#machine").val(),coex_printing_machine_start_reasons:$("#coex_printing_machine_start_reasons").val(),mp_pos_no:$("#mp_pos_no").val()},cache: false,success: function(html){
				setTimeout(function () {$("#loading").hide();$("#cover").hide();},200);
				$("#machine_start_load_details").html(html);
				} 
				});
			}else{
				alert('Select Machine Start Reason & Job No');
				$("#loading").hide();$("#cover").hide();
			}
		});
				...
				<td width="45%">
					<table class="form_table_inner">
						...
						<tr>
							...
							<td><input type="date" name="printing_date"  id="printing_date" size="10" value="<?php echo set_value('printing_date',date('Y-m-d'));?>" required/>
								<b>Shift</b><span style="color:red;">*</span> :
								<select name="shift" id="shift" required><option value=''>--Shift--</option>
