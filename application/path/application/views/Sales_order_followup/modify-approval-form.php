...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/approved_a');?>" method="POST" >
		...
		<table class="form_table_design">
            ...
            echo "<option value='".$customer_row->name1."//".$customer_row->adr_company_id."//".$customer_row->lang_property_name."' $selected ".set_select('adr_company_id',''.$customer_row->adr_company_id.'').">".$customer_row->name1."//".$customer_row->adr_company_id."//".$customer_row->lang_property_name."</option>";
									...
									<tr>
										...
										echo "<textarea disabled rows='3' cols='30'>".$customer_row->strno." ".$customer_row->name2." ".$customer_row->street." ".$customer_row->name3." PIN ".$customer_row->city_code."</textarea>";
											}
										}
											?></td>
            ...
            echo "<option ".$selected." value='".$ship_to_row->related_company_id."' ".set_select('consin_adr_company_id',''.$ship_to_row->related_company_id.'').">".$ship_to_row->relate."//".$ship_to_row->related_company_id."//".$ship_to_row->lang_property_name."</option>";
									...
									<tr>
										...
										<td><input type="date" name="po_date" value="<?php echo set_value('po_date',$order_row->cust_order_date);?>" disabled/></td>
									</tr>
									...
									<tr>
										...
										<td>
											<select name="exchange_rate" id="exchange_rate" disabled>
				...
				<table>
					...
					if($data['order_comment']==FALSE){
							...
							foreach($data['order_comment'] as $order_comment_row){
								...
								<td><textarea name="comment" value="'.set_value('comment',$order_comment_row->lang_addi_info).'" disabled>'.set_value('comment',$order_comment_row->lang_addi_info).'</textarea></td>
								</tr>';
							}
					}
					?>
					...
					<tr>
						<td class="label">Hold/Unhold Reason  :</td>
						<td><textarea name="hold_reason" value="<?php echo set_value('hold_reason',$order_row->hold_reason);?>" disabled><?php echo set_value('hold_reason',$order_row->hold_reason);?></textarea></td>
					</tr>

					<tr>
						<?php if(!empty($order_row->po_file)){
							echo '<tr>
														<td>Po File  <input type="hidden" name="po_file"  value="'.$order_row->po_file.'" readonly> </td>

														<td><a href="'.base_url('assets/'.$this->session->userdata['logged_in']['company_id'].'/po_file/'.$order_row->po_file.'').'" target="_blank" ><i class="file pdf outline icon"></i></a></td>
												</tr>';
							}?>
					</tr>
...
<div class="middle_form_design">
					<div class="middle_form_inner_design" style="overflow:scroll;">
						...
						if($this->input->post('sr_no')){
							...
							*/
							...
							<tr id="tr_<?php echo $i;?>">
								...
								<td><input type="text" name="product_name_<?php echo $i;?>" id="product_name_<?php echo $i;?>" size="50" value="<?php echo set_value('product_name_'.$i.'');?>" placeholder="Goods Information" disabled/>
								...
								<!--EKNATH-->
								<td><input type="date" name="delivery_date_<?php echo $i;?>" id="delivery_date_<?php echo $i;?>" size="10" value="<?php echo set_value('delivery_date_'.$i.'');?>" disabled/>
								</td>

								<td><input type="text" name="quantity_<?php echo $i;?>" id="quantity_<?php echo $i;?>" value="<?php echo set_value('quantity_'.$i.'');?>" maxlength="15" size="10" class="quantity" disabled/></td>
								<td><input type="text" name="unit_rate_<?php echo $i;?>" id="unit_rate_<?php echo $i;?>" value="<?php echo set_value('unit_rate_'.$i.'');?>" maxlength="15" size="10"  class="unit_rate" disabled/></td>
								<td><input type="text" name="amount_<?php echo $i;?>" id="amount_<?php echo $i;?>" value="<?php echo set_value('amount_'.$i.'');?>" maxlength="15" size="10" class="amount" disabled/></td>
								<td><select name="tax_grid_<?php echo $i;?>" id="tax_grid_<?php echo $i;?>" class="tax_grid" disabled><option value="">--Select Tax--</option>
								<?php 
										if($tax_grid==FALSE){
								...
								<td><span id="tax_amount_<?php echo $i;?>"><input type="text" name="total_tax_amount" class="total_tax_amount" value="<?php echo $this->input->post('tax_grid_'.$i.'') ? $total_tax_amount : '0';?>"  disabled></span></td>
							</tr>
...
*/
					...
					$("#product_name_<?php echo $order_details_row->ord_pos_no;?>").live('keyup',function(){
						...
						<tr id="tr_<?php echo $order_details_row->ord_pos_no;?>">
							...
							<td>
								<input type="text" name="product_name_<?php echo $order_details_row->ord_pos_no;?>" id="product_name_<?php echo $order_details_row->ord_pos_no;?>" size="50"  value="<?php echo set_value('product_name_'.$order_details_row->ord_pos_no.'',$order_details_row->description."//".$order_details_row->article_no);?>" placeholder="Goods Information" disabled/>
							...
							<!--EKNATH-->
							<td>
								<input type="date" name="delivery_date_<?php echo $order_details_row->ord_pos_no;?>" id="delivery_date_<?php echo $order_details_row->ord_pos_no;?>" size="10" value="<?php echo set_value('delivery_date_'.$order_details_row->ord_pos_no.'',$order_details_row->delivery_date);?>" disabled/>
							<td>
								<input type="text" name="quantity_<?php echo $order_details_row->ord_pos_no;?>"  id="quantity_<?php echo $order_details_row->ord_pos_no;?>" class="quantity" value="<?php echo set_value('quantity_'.$order_details_row->ord_pos_no.'', $this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id']));?>" maxlength="15" size="10" disabled/></td>
								<?php if($order_row->for_export==1){
									echo '
								<td>
									<input type="text" name="unit_rate_'.$order_details_row->ord_pos_no.'"  id="unit_rate_'.$order_details_row->ord_pos_no.'" class="unit_rate" value="'.set_value('unit_rate_'.$order_details_row->ord_pos_no.'',$order_details_row->calc_sell_price).'" maxlength="15" size="10" disabled/></td>';
							...
							<td><select name="tax_grid_<?php echo $order_details_row->ord_pos_no;?>" id="tax_grid_<?php echo $order_details_row->ord_pos_no;?>" class="tax_grid" disabled><option value="">--Select Tax--</option>
								<?php 
										if($tax_grid==FALSE){
											echo "<option value=''>--Tax Grid Set up Required--</option>";
										}else{
