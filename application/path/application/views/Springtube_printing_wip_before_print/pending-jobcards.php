			...
			<tbody>
				<?php if($springtube_printing_wip_master_before==FALSE){
					echo "<tr><td colspan='14'>No Active Records Found</td></tr>";
						...
						//print_r($springtube_printing_wip_master_before);
						foreach($springtube_printing_wip_master_before as $master_row){
							...
							echo"<tr>
									...
									foreach ($formrights as $formrights_row) {
										...
										echo ($formrights_row->modify==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/wip_return/'.$master_row->bprint_wip_id.'').'" title="Release"><i class="edit icon"></i></a>' : '');
					...
					echo"<tr><td colspan='12' style='text-align:right;'><b>TOTAL</b></td><td class='positive right aligned'><b>".number_format($sum_bprint_wip_meters,2,'.',',')."</b><i> MTRS</i></td><td class='positive right aligned'><b>".number_format($sum_bprint_wip_qty,2,'.',',')."<b><i> NOS<i></td><td class='positive'></td><td class='positive right aligned'><b>&#x20B9; ".number_format($sum_amount,2,'.',',')."</b></td><td></td><td></td><td></td></tr>";	
