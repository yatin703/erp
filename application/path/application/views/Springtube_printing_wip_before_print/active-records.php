	...
	<div class="record_inner_design" style="overflow: scroll;white-space: nowrap;">
			...
			<tbody>
				<?php if($springtube_printing_wip_master_before==FALSE){
						...
						foreach($springtube_printing_wip_master_before as $master_row){
							...
							echo"<tr>
									...
									foreach ($formrights as $formrights_row) {
										...
										echo ($formrights_row->new==1 && $master_row->status==0 && $master_row->return_flag==0 && $hold_flag<>1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/create_printing_jobcard/'.$master_row->order_no.'/'.$article_no).'" title="Create Printing Jobcard"><i class="edit icon"></i></a>' : '');

										//echo ($formrights_row->modify==1 && $master_row->status==0 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/bwip_release/'.$master_row->bprint_wip_id.'').'" title="Release"><i class="edit icon"></i></a>' : '');
									...
									echo "</td>
									<td>".($master_row->return_flag==1 ? "<a href='#' target='_blank' style='color:#06c806;'><i class='check circle icon'></i><a>": "")."
									</td>
									<td>";
					...
					echo"<tr><td colspan='14' style='text-align:right;'><b>TOTAL</b></td><td class='positive right aligned'><b>".number_format($sum_bprint_wip_meters,2,'.',',')."</b><i> MTRS</i></td><td class='positive right aligned'><b>".number_format($sum_bprint_wip_qty,2,'.',',')."<b><i> NOS<i></td><td class='positive'></td><td class='positive right aligned'><b>&#x20B9; ".number_format($sum_amount,2,'.',',')."</b></td><td></td><td></td><td></td><td></td><td></td><td></tr>";	
