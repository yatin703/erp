					...
					foreach($order_master as $mrow){
						...
						if(count($specification_result)>0 OR 1){
							...
							echo"<tr>
								...
								<td><a href='".base_url()."/index.php/artwork_new/view/".$mrow->ad_id."/".$mrow->version_no." ' target='blank'>".($mrow->ad_id!=""? $mrow->ad_id."_R".$mrow->version_no:"")."</a></td>
								...
								<td>";
									...
									foreach($jobcard_result as $jobcard_row){
										...
										if(!empty($mrow->spec_id)){
											if(substr($mrow->spec_id,0,1)=="S"){
												...
												$bom=array('bom_no'=>$mrow->spec_id,
			                                    'bom_version_no'=>$mrow->spec_version_no);
			                                    ...
			                                    echo "<a target='_blank' href='".base_url('index.php/'.$this->router->fetch_class().'/view_new/'.$jobcard_row->mp_pos_no.'/'.$mrow->spec_id.'/'.$mrow->spec_version_no)."' >";
