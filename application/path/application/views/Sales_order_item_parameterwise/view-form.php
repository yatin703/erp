...
?>
                            ...
                            if(!empty($specification_sleeve_details_row->mat_info) && !empty($specification_sleeve_details_row->mat_article_no)){
                                ...
                                echo "<tr class='item'>
                                        ...
                                        foreach(array_combine($array_rm, $array_rm_per) as $val_rm => $val_rm_per){
                                            ...
                                            echo "<td style='border-right:1px solid #D9d9d9;'>".$article_desc." (".$val_rm_per.") = <b>" .$extrusion_val."</b> ".$article_uom."</td>";
               ...
               $part_pos_no_heading=1;
                foreach($data['specification_shoulder_details'] as $specification_shoulder_details_row){
                    if(!empty($specification_shoulder_details_row->mat_info) && !empty($specification_shoulder_details_row->mat_article_no)){
                         ...
                         echo "
                         <td style='border:1px solid #D9d9d9;'>".$article_desc." (".$specification_shoulder_details_row->mat_info.")= <b>".$heading_val."</b> ".$article_uom."</td>";
                         $part_pos_no_heading++;
                    }
                }
    ...
    <table cellpadding="5" cellspacing="0">
                    ...
                    <?php 
                    $data_search= array('manu_order_no' =>$jobcard_no ,
                                  'type_flag'=>'4');
                    $data_additional=$this->common_model->select_active_records_where('reserved_quantity_manu',$this->session->userdata['logged_in']['company_id'],$data_search);
                    if($data_additional==FALSE){
                        echo "<tr>
                                <td></td>
                                </tr>";
                    }else{
                        $i=1;
                       foreach($data_additional as $data_additional_row){

                        $article_desc="";
                        $article_uom="";
                        $data['article']=$this->article_model->select_one_active_record('article',$this->session->userdata['logged_in']['company_id'],'article.article_no',$data_additional_row->article_no);
                        foreach($data['article'] as $article_row){
                            $article_desc=$article_row->article_name;
                            $article_uom=$article_row->uom;
                        }
                        ...
                        echo "<tr>
                                <td width='20%''>$i</td>
                                <td></td>
                                <td>".$article_desc."= <b>".$this->common_model->read_number($data_additional_row->qty,$this->session->userdata['logged_in']['company_id'])." </b> ($article_uom)</td>
                                <td>$data_additional_row->article_no</td>
                                <td></td>
