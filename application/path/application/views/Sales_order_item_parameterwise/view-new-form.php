...
<script>
    ...
    $(document).ready(function(){
        ...
        $(".invoice-box").css("max-width", "1200px");
...
?>
                ...
                <table cellpadding="0" cellspacing="0" style="border:1px solid #D9d9d9;">
                    ...
                    <tr class="item">
                        ...
                        <td style="border-top:1px solid #D9d9d9;" ><?php echo strtoupper($production_row->comment);?></td>
                        <td style="border-top:1px solid #D9d9d9;"><b>DISPATCH TOLERANCE %</b></td>
                        <td style="border-top:1px solid #D9d9d9;">  </td>
                        <td style="border-top:1px solid #D9d9d9;"> <?php echo $tolerance;?> %</td>
                ...
                if($purging==FALSE){
                    ...
                    foreach($purging as $purging_row){
                        ...
                        $search=array('jobcard_no'=>$jobcard_no,
                                      'part_no'=>$purging_row->article_no,
                                      'qty'=>$this->common_model->read_number($purging_row->demand_qty,$this->session->userdata['logged_in']['company_id']));
                    ...
                    echo '
                        <tr class="item">
                            <td width="10%"></td>
                            <td width="5%"></td>
                            <td style="border:1px solid #D9d9d9;">'.$completed=($purging_row->completed_flag==0?"": ($error!=''?"<i style='color:red;'class='times circle icon'></i>":"<i class='check green circle icon'></i>")).' '.$article_desc.'= <b>'.$this->common_model->read_number($purging_row->demand_qty,$this->session->userdata['logged_in']['company_id']).'</b> '.$uom.'</td>
                        </tr>';
                ...
                if($box==FALSE){
                    ...
                    echo '
                        <tr>
                            <td width="10%"></td>
                            <td width="5%"></td>
                            <td style="border:1px solid #D9d9d9;">'.$completed=($box_row->completed_flag==0?"": ($error!=''?"<i style='color:red;'class='x red circle icon'></i>":"<i class='check green circle icon'></i>")).' '.$article_desc.' '.$article_sub_description.'= <b>'.$this->common_model->read_number($box_row->demand_qty,$this->session->userdata['logged_in']['company_id']).'</b> '.$uom.($error!=''? ' (<b><i>'.$error.'</i></b>)':'').'</td>
                        </tr>';
                        $j++;
               ...
               $part_pos_no_heading=1;
                ...
                foreach($data['specification_shoulder_details'] as $specification_shoulder_details_row){
                    if(!empty($specification_shoulder_details_row->mat_info) && !empty($specification_shoulder_details_row->mat_article_no)){
                            ...
                            $data['article']=$this->article_model->select_one_active_record('article',$this->session->userdata['logged_in']['company_id'],'article.article_no',$specification_shoulder_details_row->mat_article_no);
                                ...
                                $search_head=array('jobcard_no'=>$jobcard_no,
                                              'part_no'=>ltrim($val_rm," "),
                                              'qty'=>$heading_val);
                                $data['tally_material_issue_master_heading']=$this->jobcard_issue_tally_model->active_record_search('tally_material_issue_master',$search_head,'','');
                                //echo $this->db->last_query();
                                //print_r($data['tally_material_issue_master_extr']);
                                $error='';
                         ...
                         echo "
                         <td style='border:1px solid #D9d9d9;'>".$completed=($com_head==0?"":($error!=''?"<i class='x red circle icon'></i>":"<i class='check green circle icon'></i>"))." ".$article_desc." (".$specification_shoulder_details_row->mat_info."%)= <b>".$heading_val."</b> $uom </td>";
                         $part_pos_no_heading++;
                    }
                }
                    ...
                    foreach($printing as $printing_row){
                        ...
                        $data['article']=$this->article_model->select_one_active_record('article',$this->session->userdata['logged_in']['company_id'],'article.article_no',$printing_row->article_no);
                                foreach($data['article'] as $article_row){
                                    ...
                                    $search_printing=array('jobcard_no'=>$jobcard_no,
                                    ...
                                    $data['tally_material_issue_master_printing']=$this->jobcard_issue_tally_model->active_record_search('tally_material_issue_master',$search_printing,'','');
                                //echo $this->db->last_query();
                                //print_r($data['tally_material_issue_master_extr']);
                                $error='';
                        ...
                        <td ></td>
                        <td colspan="2">'.$completed=($printing_row->completed_flag==1 ? ($error!=''?"<i style='color:red;'class='x red circle icon'></i>":"<i class='check green circle icon'></i>") : "").' '.$this->common_model->get_article_name($printing_row->article_no,$this->session->userdata['logged_in']['company_id']).' = <b>'.$this->common_model->read_number($printing_row->demand_qty,$this->session->userdata['logged_in']['company_id']).'</b> '.$uom.($error!=''? ' (<b><i>'.$error.'</i></b>)':'').'</td>
                        <!--<td width="10%"></td>
                        <td width="5%"></td>
                        <td width="35%"></td>-->
                        </tr>';
                ...
                if($labeling==FALSE){
                    ...
                    foreach($labeling as $labeling_row){
                        ...
                        $search_labeling=array(
                            'jobcard_no'=>$jobcard_no,
                            'part_no'=>$labeling_row->article_no,
                            'qty'=>$label_qty
                        );

                        $data['tally_material_issue_master_labeling']=$this->jobcard_issue_tally_model->active_record_search('tally_material_issue_master',$search_labeling,'','');
                                //echo $this->db->last_query();
                                //print_r($data['tally_material_issue_master_extr']);
                        $error='';
                    ...
                    echo '
                        <tr class="item">
                            ...
                            <td colspan="2">'.$completed=($labeling_row->completed_flag==1 ? ($error!=''?"<i style='color:red;'class='x red circle icon'></i>":"<i class='check green circle icon'></i>") : "").' '.$this->common_model->get_article_name($labeling_row->article_no,$this->session->userdata['logged_in']['company_id']).' = <b>'.$label_qty.'</b> '.$uom.($error!=''? ' (<b><i>'.$error.'</i></b>)':'').'</td>
                        </tr>';
                ...
                <?php
                    ...
                    foreach($foiling as $foiling_row){
                        ...
                        $data['tally_material_issue_master_foiling']=$this->jobcard_issue_tally_model->active_record_search('tally_material_issue_master',$search_foiling,'','');
                                //echo $this->db->last_query();
                                //print_r($data['tally_material_issue_master_extr']);
                        $error='';
                        ...
                        <td width="10%" ></td>
                        <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                        <td colspan="2">'.$completed=($foiling_row->completed_flag==1 ? ($error!=''?"<i style='color:red;'class='x red circle icon'></i>":"<i class='check green circle icon'></i>") : "").' '.$this->common_model->get_article_name($foiling_row->article_no,$this->session->userdata['logged_in']['company_id']).' = <b>'.$this->common_model->read_number($foiling_row->demand_qty,$this->session->userdata['logged_in']['company_id']).'</b> '.$uom.($error!=''? ' (<b><i>'.$error.'</i></b>)':'').'</td>
                        <!--<td width="10%"></td>
                        <td width="5%"></td>
                        <td width="35%"></td>-->
                        </tr>';
                ...
                <?php
                    ...
                    foreach($shoulder_foiling as $shoulder_foiling_row){
                        ...
                        $search_shoulder_foiling=array(
                            'jobcard_no'=>$jobcard_no,
                            'part_no'=>$shoulder_foiling_row->article_no,
                            'qty'=>$this->common_model->read_number($shoulder_foiling_row->demand_qty,$this->session->userdata['logged_in']['company_id'])
                        );
                        $data['tally_material_issue_master_shlfoiling']=$this->jobcard_issue_tally_model->active_record_search('tally_material_issue_master',$search_shoulder_foiling,'','');
                                //echo $this->db->last_query();
                                //print_r($data['tally_material_issue_master_extr']);
                        $error='';
                        ...
                        <td width="10%"></td>
                        <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                        <td colspan="2">'.$completed=($shoulder_foiling_row->completed_flag==1 ? ($error!=''?"<i style='color:red;'class='x red circle icon'></i>":"<i class='check green circle icon'></i>") : "").' '.$this->common_model->get_article_name($shoulder_foiling_row->article_no,$this->session->userdata['logged_in']['company_id']).' = <b>'.$this->common_model->read_number($shoulder_foiling_row->demand_qty,$this->session->userdata['logged_in']['company_id']).'</b> '.$uom.($error!=''? ' (<b><i>'.$error.'</i></b>)':'').'</td>
                        <!--<td width="10%"></td>
                        <td width="5%"></td>
                        <td width="35%"></td>-->
                        </tr>';
