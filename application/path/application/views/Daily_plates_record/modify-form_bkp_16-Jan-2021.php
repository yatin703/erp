				...
				<td width="50%">
					<table class="form_table_inner">
						<tr>
							...
							<td>
								<input type="hidden" name="dpr_id" value="<?php echo set_value('dpr_id',$graphics_daily_plates_master_row->dpr_id );?>" />
								<input type="text" name="order_no" id="order_no"  size="20" value="<?php echo set_value('order_no',$graphics_daily_plates_master_row->order_no);?>" required placeholder="Order No"/></td>
						...
						<tr>
							...
							<td><input type="text" name="artwork_no" id="artwork_no"  size="20" value="<?php echo set_value('artwork_no',$graphics_daily_plates_master_row->artwork_no);?>"   placeholder="Artwork" readonly/></td>
							<td class="label">Version  * :</td>
							<td><input type="text" name="version_no" id="version_no"  size="20" value="<?php echo set_value('version_no',$graphics_daily_plates_master_row->version_no);?>"    placeholder="Version" readonly/></td>
						</tr>
						<tr><td class="label">Comment  * :</td>
							<td ><textarea name="comment" value="<?php echo set_value('comment',$graphics_daily_plates_master_row->comment);?>"><?php echo set_value('comment',$graphics_daily_plates_master_row->comment);?></textarea></td>
						...
						<td><select name="operator_id" id="operator_id">					
								...
								<?php if($graphics_operator_master==FALSE){
									...
									foreach ($graphics_operator_master as $row) {
										$selected=($graphics_daily_plates_master_row->operator_id==$row->operator_id?'selected':'');
							...
							<td><select name="shift_id" id="shift_id">					
										...
										<?php if($graphics_shift_master==FALSE){
											...
											foreach ($graphics_shift_master as $row) {
												...
												$selected=($graphics_daily_plates_master_row->shift_id==$row->shift_id?'selected':'');
							...
							<td><select name="machine_id" id="machine_id">					
								...
								<?php if($graphics_machine_master==FALSE){
									...
									foreach ($graphics_machine_master as $row) {
										$selected=($graphics_daily_plates_master_row->machine_id==$row->machine_id?'selected':'');
							...
							<td><select name="reason_id" id="reason_id">					
								...
								<?php if($graphics_plate_making_reasons==FALSE){
									...
									foreach ($graphics_plate_making_reasons as $row) {
										$selected=($graphics_daily_plates_master_row->plate_making_reason==$row->reason_id?'selected':'');
				...
				<table class="middle_form_table_design" id="tablePanton">
								...
								<?php
									...
									if(!empty($this->input->post('pantone[]'))){
										...
										$result_graphics_daily_plates_details=$this->common_model->select_one_details_record_noncompany('graphics_daily_plates_details','dpr_id',$graphics_daily_plates_master_row->dpr_id);

										$j=1;
										foreach($result_graphics_daily_plates_details as $details_row ){
											...
											echo'<tr id="'.$j++.'">
												...
												echo'<td><input type="text"   name="pantone[]"   value="'.set_value("pantone[]",$details_row->pantone_name).'"  placeholder="Pantone Name"></td><td><input type="date" name="date_on_plate[]" value="'.set_value('date_on_plate[]',$details_row->date_on_plate).'"   placeholder="Date On Plate"/></td><td><input type="text"   name="label_on_plate[]" value="'.set_value('label_on_plate[]',$details_row->label_on_plate).'"  placeholder="Lable On Plate" ></td><td><input type="number" name="plate_count[]" value="'.set_value('plate_count[]',$details_row->no_of_plates).'"    placeholder="No. Of Plate"></td></tr>';
										}
