				...
				<?php if($graphics_daily_plates_master==FALSE){
					echo "<tr><td colspan='19'>No Active Records Found</td></tr>";
								...
								echo"<tr>
									...
									echo "</td>
										...
										foreach($formrights as $formrights_row){ 
											...
											echo ($formrights_row->modify==1 && $row->final_approval_flag<>1  ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->dpr_id).'"><i class="edit icon"></i></a> ' : '');										
											echo ($row->archive<>1 && $formrights_row->delete==1 && $row->final_approval_flag<>1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->dpr_id).'"><i class="trash icon"></i></a> ' : '');

											echo ($row->archive<>1 && $formrights_row->copy==1 && $row->final_approval_flag<>1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/approve/'.$row->dpr_id).'"><i class="thumbs outline up icon"></i></a> ' : '');
							...
							<tr>
								...
								<td><?php echo $total_offset;?></td>
								<td><?php echo $total_screen;?></td>
								<td><?php echo $total_flexo;?></td>
								<td colspan=5></td>
							</tr>	
						</table>
