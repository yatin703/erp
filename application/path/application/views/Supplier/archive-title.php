<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    echo ($formrights_row->delete==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'').'">Active ('.$this->supplier_model->active_record_count($table,$this->session->userdata['logged_in']['company_id']).')</a> : ' : '');
