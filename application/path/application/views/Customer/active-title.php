<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    $table='address_master';
  	...
  	echo ($formrights_row->delete==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/archive_records').'" class="ui red tag label"><i class="minus circle icon"></i>Archive ('.$this->customer_model->archive_record_count($table,$this->session->userdata['logged_in']['company_id']).')</a> : ' : '');
