				...
				<?php if($artwork==FALSE){
					echo "<tr><td colspan='19'>No Active Records Found</td></tr>";
							...
							foreach($artwork as $row){
								...
								echo "<tr>
									<td>".$i."</td>
									<td><b>".$row->ad_id."_R".$row->version_no."</b></td>
									<td> ".($row->artwork_image_nm!='' ? '<a href="'.base_url('assets/'.$this->session->userdata['logged_in']['company_id'].'/artwork/'.$row->artwork_image_nm.'').'" target="_blank"><i class="file pdf outline icon"></i></a>' :'')." ".($row->final_approval_flag==1 ? '<i class="check circle icon"></i>' : '')."</td>
