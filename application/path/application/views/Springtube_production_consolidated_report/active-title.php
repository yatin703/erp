<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
   ...
   // echo ($formrights_row->copy==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/search_planning').'" class="ui tag label"><i class="search icon"></i>Planning Sheet Search</a> : ' : '');
    //echo ($formrights_row->copy==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/search_planning_springtube').'" class="ui tag label"><i class="search icon"></i>SpringTube Planning Sheet Search</a> : ' : '');
  	?> 
  <?php endforeach;?>
  </h2>
