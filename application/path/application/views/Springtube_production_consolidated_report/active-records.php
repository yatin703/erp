							...
							foreach($job_card as $row){
								...
								echo "<tr>
									...
									if($order_flag=='1'){
										...
										echo "<td><a href='".base_url('index.php/'.$this->router->fetch_class().'/view_new/'.$row->mp_pos_no.'/'.$spec_id.'/'.$version)."' target='_blank'>$row->mp_pos_no</a></td>";
									}else{
									...
									foreach($formrights as $formrights_row){ 
										echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/against_jobcard/'.$row->mp_pos_no.'/'.$spec_id.'/'.$version).'" target="_blank">ADDITIONAL RM</a> ' : '');
