...
<?php if(isset($note)){ echo "<p class='alert alert-success'>$note</p>";}?>
	<div class="record_inner_design">
			<table class="ui fixed red sortable celled table">
				<thead>
				<tr>
					<th>Id</th>
					<th>Date</th>
					...
					$i=1;
							foreach($followup_rejected as $followup_rejected_row){
								...
								echo "<tr>
									...
									<td>".$followup_rejected_row->record_no."</td>
									<td>".($followup_rejected_row->status==1 ? 'PENDING' : '')."</td>
									<td>";
									foreach($formrights as $formrights_row){ 
										echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/sales_order_book/view/'.$followup_rejected_row->record_no).'" target="_blank"><i class="print icon"></i></a> ' : '');
