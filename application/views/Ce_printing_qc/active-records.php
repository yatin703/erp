...
<h4>Active Records</h4>
   <div class="record_inner_design" style="overflow: scroll;">
         ...
         <?php 
            $sum_bm_wip_qty=0;
            if($coex_extrusion_printing_qc==FALSE){
            echo "<tr><td colspan='11'>No Active Records Found</td></tr>";
            }else{
            	$i=1;
            	foreach($coex_extrusion_printing_qc as $row){
            
            		echo "<tr>
            				<td>".$i."</td>
                        <td>".($row->flag==1 ? '<span class="" style="font-size: 10px;color: #fff;background: #00b5ad;padding: 4px;border-radius: 5px;font-weight: bold;"><i class="check circle outline icon"></i> QC Detach</span>' :'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/create/'.$row->prt_qc_id).'" style="font-size: 10px;color: #fff;background: #4183c4;padding: 4px;border-radius: 5px;font-weight: bold;"><i class="plus circle icon"></i><span>QC Release</span></a>')."</td>
                             ";
                            echo "</td>
