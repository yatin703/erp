...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/save_job_card');?>" method="POST" >
				...
				<table class="form_table_inner">
						<?php foreach($order_details as $order_details_row):?>
							...
							<tr>
								...
								<td class="label">Dispatch Tolerance % : </td>
									<td><input type="text" name="dispatch_tolerance" id="dispatch_tolerance" value="<?php echo $dispatch_tolerance;?>" disabled />
										<input type="hidden" name="dispatch_tolerance_1" id="dispatch_tolerance_1" value="<?php echo $dispatch_tolerance_1;?>">
								</td>
							</tr>
							<?php if($order_flag=='0'){?>
							<tr>
										<td class="label">Tolerance Qty <span style="color: red;"><b>*</b></span> :</td>
										<td><input type="text" name="tolerance_qty" value="<?php echo round(($this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])/$dispatch_tolerance_1));?>" disabled></td>
							</tr>

							<tr>
										<td class="label">Total Dispatch Allowed Upto <span style="color: red;"><b>*</b></span> :</td>
										<td><input type="text" name="dispatch_qty" value="<?php echo round($this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])+($this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])/$dispatch_tolerance_1));?>" disabled></td>

							</tr>

							<tr>
										<td class="label">In House R% Allowed Upto <span style="color: red;"><b>*</b></span> :</td>
										<td><?php
										if($order_flag=='0'){
											$order_matrix_result=$this->common_model->select_one_active_record('coex_jobcard_per_maxtrix',$this->session->userdata['logged_in']['company_id'],'archive','0');
											//echo $this->db->last_query();
											$rpc=0;

											if($order_matrix_result==TRUE){
												//echo "1";
												foreach($order_matrix_result as $order_matrix_row){
													if(($order_matrix_row->min<=$this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])) && ($this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])<=$order_matrix_row->max) ){
														echo "<input type='text' name='r_allowed_upto' value='$order_matrix_row->per%'>";
														$rpc=$order_matrix_row->per;
													}
												}
											}else{
							...
							<tr>
								<td class="label">Jobcard Allowed Upto <span style="color: red;"><b>*</b></span> :</td>
								<td><input type="text" name="jobcard_allowed_upto" value="<?php echo $this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])+($this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])/$dispatch_tolerance_1)+ (($this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])+($this->common_model->read_number($order_details_row->total_order_quantity,$this->session->userdata['logged_in']['company_id'])/$dispatch_tolerance_1))/100)*$rpc;?>"></td>		
							</tr>
						<?php } ?>
						...
						<?php }else{
								...
								if(substr($order_row->order_no,0,2)=='ST'){
									...
									$jobcard_perc=$dispatch_tolerance+$printing_rejection_perc+$bodymaking_rejection_perc;
									
									$extra_qty=($order_qty*$jobcard_perc/100);
						 ...
						 ?>
							...
							<tr>
								...
								<td><input type="text" name="job_card_quantity" id="job_card_quantity" value="<?php echo set_value('job_card_quantity',$jobcard_qty);?>"></td>
						...
						<tr><td class="label">Comment * :</td>
							<td colspan="3"><textarea name="comment" id="comment" value="<?php echo set_value('comment');?>"  required><?php echo set_value('comment');?></textarea>
						</tr>

						<!--<tr>
							<td class="label">Matrix</td>
							<td colspan="3">
								<table class="ui very basic collapsing celled table" style="font-size: 9px;">
								  <thead>
								    <tr>
								    	<th>Order Type</th>
								    	<th>Order Qty Range</th>
								    	<th>% Allowed</th>
								  	</tr></thead>
								  <tbody>
								    <tr>
	...
	<script type="text/javascript">
		...
		if($("#order_flag").val()==1){
			...
			$("#job_card_quantity").live('keyup',function(){			

			if($("#job_card_quantity").val()!=''){				
