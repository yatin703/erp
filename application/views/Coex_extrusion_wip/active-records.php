...
<div class="ui three column grid">
  <div class="row">
    <div class="column hold">
        <a class="nav-link active" href="<?php echo base_url(); ?>index.php/coex_extrusion_wip">WIP Hold</a>
    </div>
            ...
            <tbody>
               ...
               echo "<tr><td colspan='19' style='text-align: center !important;'>No Active Records Found</td></tr>";
               	...
               	foreach($coex_extrusion_wip as $row){                                         
                     ...
                     if($row->cost!=''){$cost = $row->cost;}else{$cost ='-';}
                     $total_cost = $row->ok_by_qc*$row->cost;
                     if($total_cost!=''){$to_cost = $total_cost;}else{$to_cost ='-';}

               		echo "<tr>
               				<td class='center aligned'>".$i."</td>
                           <td>";
                                 foreach($formrights as $formrights_row){ 
                                echo ($formrights_row->new==1 && $row->ok_by_qc !=0 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/wip_release/'.$row->cewip_id).'" title="WIP Release" target="_blank" style="font-size: 10px;color: #fff;background: #f2711c;padding: 4px;border-radius: 5px;font-weight: bold;"><i class="plus circle icon"></i><span> WIP Hold</span></a> ' : '');                                                         
                                 }
                           echo"
