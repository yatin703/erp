...
<button class="ui icon blue mini button" id="export_to_excel">
	...
	<div class="record_inner_design" style="overflow: scroll;white-space: nowrap;">
		<table class="record_table_design_without_fixed">
			<!--<tr>
				<th></th>
				<th colspan="22">Artwork Details</th>
						...
						foreach($artwork_springtube as $row){
							...
							echo "<tr>
								...
								foreach($formrights as $formrights_row){ 
										...
										echo ($formrights_row->copy==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/csv/'.$row->ad_id.'/'.$row->version_no).'" ><i class="file excel outline icon"></i></a> ' : '');
								...
								<td><a href='".base_url('index.php/'.$this->router->fetch_class().'/view/'.$row->ad_id.'/'.$row->version_no)."' target='_blank'><b>".$row->ad_id."</b></a>
								</td>

								<!--<td><b><a class='ui ".($row->final_approval_flag=='1' ? "green"  : "red")." circular label'> $row->version_no</td>-->

								<td style='color: ".($row->final_approval_flag==1? "green":"red").";font-size: 9pt;font-weight: 600;vertical-align: middle;'>$row->version_no
								</td>
								...
								<?php
									...
									<td>";
										...
										if($result_cold_foil_one==TRUE){
											...
											//echo'<br>';
											//echo ($cold_foil_1_length!=''? ($cold_foil_1_length*$cold_foil_1_width)/1000000:'');
									...
									<td>".strtoupper($this->common_model->get_user_name($row->user_id,$this->session->userdata['logged_in']['company_id']))."
									</td>
									...
									<!--<td>".($row->approved_by!='' ? "<a class='ui tiny label'><i class='checkmark box icon'></i>":'' )."".strtoupper($this->common_model->get_user_name($row->approved_by,$this->session->userdata['logged_in']['company_id']))."</td>
										<td>
									-->
									<td>".($row->approved_by!='' ? strtoupper($this->common_model->get_user_name($row->approved_by,$this->session->userdata['logged_in']['company_id'])):'')."</td>
