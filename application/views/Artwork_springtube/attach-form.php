...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/attach_update');?>" method="POST" autocomplete="off" enctype="multipart/form-data"  >
	...
	$cold_foil_2_area='';
		...
		// DEfault Foil Width-----------------
		$sleeve_diameter_master_result=$this->common_model->select_one_active_record('sleeve_diameter_master',$this->session->userdata['logged_in']['company_id'],'sleeve_diameter',$dia);

		$sleeve_id=0;
		foreach ($sleeve_diameter_master_result as  $sleeve_diameter_master) {
			$sleeve_id=$sleeve_diameter_master->sleeve_id;
		}

		$spring_width_calculation_result=$this->common_model->select_one_active_record('spring_width_calculation',$this->session->userdata['logged_in']['company_id'],'sleeve_dia_id',$sleeve_id);

		$std_foil_width='';
		foreach ($spring_width_calculation_result as  $spring_width_calculation_master) {
			$std_foil_width=$spring_width_calculation_master->std_foil_width;
		}

		if($cold_foil_1!=''){
			$cold_foil_1_width=$std_foil_width;
			
		}
		
		if($cold_foil_2!=''){
			$cold_foil_2_width=$std_foil_width;
				...
				<td>
					<table class="form_table_inner" >
						...
						<tr>
							...
							<td><select name="sleeve_type" required disabled>
