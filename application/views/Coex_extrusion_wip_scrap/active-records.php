			...
			<table class="ui very basic collapsing celled table"  style="font-size:10px;" id="tbl_data">
                ...
                $total_box=$this->common_model->get_total_box('coex_extrusion_wip_scrap',$row->jobcard_no,$this->session->userdata['logged_in']['company_id']);               
                  ...
                  echo "<tr>
                          ...
                          echo ($formrights_row->new==1 && $row->scrap_qty !=0 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/return_scrap/'.$row->wip_scrap_id).'" title="WIP Release" target="_blank"><i class="edit icon"></i></a> ' : '');                                                         
                           }
                          echo"
                        </td>  
               ...
               <td class='negative right aligned'><b>".$sum_box."</b> <i>BOX</i></td></tr>";
            }
					?>
		</tbody>			
		</table>
