					...
					echo "<tr><td colspan='9'>No Active Records Found</td></tr>";
							...
							foreach($digital_ink_price_master as $row){
								echo "<tr>
									...
									echo ($formrights_row->modify=='1' ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->dipm_id).'" title="Modify"><i class="edit icon"></i>|</a> ' : '');

									echo ($formrights_row->delete=='1' ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->dipm_id).'" title="Delete"><i class="trash icon"></i></a> ' : '');
