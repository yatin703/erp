...
<script>
	$(document).ready(function(){
		...
		$("#search").click(function(){
			...
			$.ajax({type: "POST",url: "<?php echo base_url('index.php/ajax/');?>",data: {from_date : $(".from_date").val(),to_date :$('.to_date').val(),sleeve_dia : $('.sleeve_dia:checked').serialize()},cache: false,success: function(html){
				...
				<td width="50%">
					<table class="form_table_inner">
									...
									<tr>
										...
										<td  width="25%"><input type="date" name="to_date" class="to_date" value="<?php echo set_value('to_date',date('Y-m-d',strtotime('+180 days')));?>"/></td>
									</tr>
									<?php endforeach;
										}?>
				...
				<span id="check">
					...
					if($print_type_wise_sales_order==FALSE){
						...
						echo '<table class="ui sortable selectable celled table" style="font-size:10px;">
					        	<thead>
								   <tr>
								    	<th colspan="29"><a class="ui orange label">SALES ORDER</a> <a class="ui green label">ON DELIVERY DATE</a>';
								    	if($account_periods_master==FALSE){
								    	}else{
								    		foreach ($account_periods_master as $account_periods_master_row ){
								    			echo '<a class="ui olive label"><i class="calendar icon"></i>'.$this->common_model->view_date($account_periods_master_row->fin_year_start,$this->session->userdata['logged_in']['company_id']).' TO '.$this->common_model->view_date(date('Y-m-d',strtotime('+180 days')),$this->session->userdata['logged_in']['company_id']).'</a><br/><br/>';
								    		}
								    	}
