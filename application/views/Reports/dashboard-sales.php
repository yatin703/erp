...
<script>
	...
	<!--
	<div class="ui red labels" style="text-align: center;">
      <div class="ui label">UNDER DEVELOPMENT</div>
    </div>
    <br/>-->
				...
				<td width="50%">
					<table class="form_table_inner">
									...
									<tr>
										...
										<td  width="25%"><input type="date" name="from_date" class="from_date"  value="<?php echo set_value('from_date',date('Y-m-01')/*$account_periods_master_row->fin_year_start*/);?>"/></td>
		    ...
		    <div class="column">
		    	...
		    	<div class="ui red segments">
		    		<div class="ui red segment">
					    <p><a  class="ui orange label">OVERVIEW</a><a class="ui olive label"><i class="calendar icon"></i>01 TO <?php echo date('d');?> OF LAST 4 MONTHS</a>
					    	<a class="ui black label">COEX & SPRING </a>
					    </p>
					</div>
						...
						<table class="ui very basic collapsing celled table"  style="font-size:10px;">
						  <thead>
						    <tr>
						    	<th>MONTH</th>
						    	<th>SALES VOLUME</th>
						    	<th>PRINTING VOLUME (COEX+SPRING)</th>
						    	<th>NET SALES</th>
						    	<th>AVG PRICE</th>
						    	<!--
						    	<th>CONTR</th>
						  ...
						  <tbody>
						    <tr>
						      <td><b><?php echo strtoupper(date('M y'));?></b></td>
						      <td><?php 
						      	$march=0;
							    ...
							    echo money_format('%!.0n',$total_total_print_type_sales_quantity);
						     ...
						     <td><?php if($printing==FALSE){ 
								 ...
								 $total_printing=0;
						      	...
						      	if($spring_printing==FALSE){ 
						      		$total_spring_printing=0;
						      	}else{
						      		$total_spring_printing=0;
						      		foreach($spring_printing as $row){
						      			$total_row_spring_printing=0;
										$total_row_spring_printing=$row->SPRING_PRINTING;
									
								    	$total_spring_printing+=$total_row_spring_printing;
						      		}
						      	}


						      	$total_p=$total_labeling+$total_printing+$total_spring_printing;

						      	echo money_format('%!.0n',$total_p);

						      } ?></td>
						    <td><?php echo money_format('%.0n',$total_total_print_type_sales_value);?></td>
						   ...
						   <!-- <td><?php /*
						    if($contribution==FALSE){
						    	...
						    	$total_costing=0;
						    	$total_net_amount=0;
						    	foreach($contribution as $contribution_row){
						    		$total_quantity+=$contribution_row->dispatch_quantity;
						    		$total_costing+=$contribution_row->total_costing;
						    		$total_net_amount+=$contribution_row->dispatch_quantity*$contribution_row->unit_rate;
						    	}
						    	$total_sales_avg_unit_price=($total_net_amount<>0 ? ($total_net_amount/$total_quantity) : '0');
						    	$total_cost_avg_unit_price=($total_costing<> 0 ? ($total_costing/$total_quantity) : '0');
								$total_contr_avg_unit_price=($total_sales_avg_unit_price-$total_cost_avg_unit_price);
						    	$total_contribution=$total_net_amount-$total_costing;
						    }

						    echo money_format('%.0n',$total_contribution);
						    ?></td>
						    <td><?php echo number_format($total_contr_avg_unit_price,2);?></td>
						    <td><?php echo number_format($total_contr_avg_unit_price<> 0 ? ($total_contr_avg_unit_price/$total_sales_avg_unit_price)*100 : '0');?>%</td>
						    <td><?php echo $this->costsheet_model->active_record_count_where_pkey('costsheet_master',$this->session->userdata['logged_in']['company_id'],'status_flag','0',date('Y-m-01'),date('Y-m-d'));

						    */?></td>-->
						    </tr>

						    <tr>
						      <td><b><?php echo strtoupper(date('M y',strtotime("".date("Y-m-01")." -1 month")));?></b></td>
						      <td><?php $feb=0;
						      	$total_total_print_type_sales_quantity=0;
							    $total_total_print_type_sales_value=0;
							    $total_total_print_type_sales_avg=0;
						     ...
						     <td><?php 
						     $total_p_last_month=0;
						     if($printing_last_month==FALSE){
						     	$total_printing=0;
						      }else{
						      	 $total_moss_offset=0;
								 $total_bonmac_offset=0;
						      ...
						      if($labeling_last_month==FALSE){ 
						      		$total_labeling=0;
						      	}else{
						      		$total_labeling=0;
						      		foreach($labeling_last_month as $row){
						      			$total_row_labeling=0;
										$total_row_labeling=$row->TECHNOSHEEL_LABEL_2+$row->TECHNOSHELL_HALF_LABEL+$row->TECHNOSHELL_LABEL_3;
						      	...
						      	if($spring_printing_last_month==FALSE){ 
						      		$total_spring_printing=0;
						      	}else{
						      		$total_spring_printing=0;
						      		foreach($spring_printing_last_month as $row){
						      			$total_row_spring_printing=0;
										$total_row_spring_printing=$row->SPRING_PRINTING;
						      ...
						      $total_p_last_month=$total_printing+$total_labeling+$total_spring_printing;

						      echo money_format('%!.0n',$total_p_last_month);
						      ?></td>
						    <td><?php echo money_format('%.0n',$total_total_print_type_sales_value);?></td>

							<td><?php echo "&#8377;".number_format($total_total_print_type_sales_avg,2,'.','');?></td>
							<!--<td><?php /*
						    if($contribution_last_month==FALSE){
						    	$total_quantity=0;
						    	$total_costing=0;
						    	$total_net_amount=0;
						    	$total_contribution=0;
						    	...
						    	$total_costing=0;
						    	$total_net_amount=0;
						    	foreach($contribution_last_month as $contribution_row){
						    		$total_quantity+=$contribution_row->dispatch_quantity;
						    		$total_costing+=$contribution_row->total_costing;
						    		$total_net_amount+=$contribution_row->dispatch_quantity*$contribution_row->unit_rate;
						    	}
						    ...
						    ?></td>
						    <td><?php echo number_format($total_contr_avg_unit_price,2);?></td>
						    <td><?php echo ($total_contr_avg_unit_price<> 0 ? number_format(($total_contr_avg_unit_price/$total_sales_avg_unit_price)*100) : 0);?>%</td>
						    <td><?php echo $this->costsheet_model->active_record_count_where_pkey('costsheet_master',$this->session->userdata['logged_in']['company_id'],'status_flag','0',date("Y-m-01", strtotime("-1 month")),date("Y-m-d", strtotime("-1 month")));
						    */?></td>-->
							</tr>


							<tr>
						      <td><b><?php echo strtoupper(date('M y',strtotime("-2 month")));?></b></td>
						      <td><?php $jan=0;
						      	$total_total_print_type_sales_quantity=0;
							    $total_total_print_type_sales_value=0;
							    $total_total_print_type_sales_avg=0;
						     ...
						     <td><?php 
						     $total_p_2ndlast_month=0;

						     if($printing_2ndlast_month==FALSE){
						      ...
						      if($labeling_2ndlast_month==FALSE){ 
						      		$total_labeling=0;
						      	}else{
						      		$total_labeling=0;
						      		foreach($labeling_2ndlast_month as $row){
						      			$total_row_labeling=0;
										$total_row_labeling=$row->TECHNOSHEEL_LABEL_2+$row->TECHNOSHELL_HALF_LABEL+$row->TECHNOSHELL_LABEL_3;
						      	...
						      	if($spring_printing_2ndlast_month==FALSE){ 
						      		$total_spring_printing=0;
						      	}else{
						      		$total_spring_printing=0;
						      		foreach($spring_printing_2ndlast_month as $row){
						      			$total_row_spring_printing=0;
										$total_row_spring_printing=$row->SPRING_PRINTING;
						      ...
						      $total_p_2ndlast_month=$total_printing+$total_labeling+$total_spring_printing;

						      echo money_format('%!.0n',$total_p_2ndlast_month);
							...
							<tr>
						      <td><b><?php echo strtoupper(date('M y',strtotime("-3 month")));?></b></td>
						      <td><?php $jan=0;
						      	$total_total_print_type_sales_quantity=0;
							    $total_total_print_type_sales_value=0;
							    $total_total_print_type_sales_avg=0;
						     ...
						     <td><?php 
						     $total_p_3rdlast_month=0;
						     if($printing_3rdlast_month==FALSE){
						     	$total_printing=0;
						      }else{
						      	 $total_moss_offset=0;
								 $total_bonmac_offset=0;
						      ...
						      if($labeling_3rdlast_month==FALSE){ 
						      		$total_labeling=0;
						      	}else{
						      		$total_labeling=0;
						      		foreach($labeling_3rdlast_month as $row){
						      			$total_row_labeling=0;
										$total_row_labeling=$row->TECHNOSHEEL_LABEL_2+$row->TECHNOSHELL_HALF_LABEL+$row->TECHNOSHELL_LABEL_3;
						      	...
						      	if($spring_printing_3rdlast_month==FALSE){ 
						      		$total_spring_printing=0;
						      	}else{
						      		$total_spring_printing=0;
						      		foreach($spring_printing_3rdlast_month as $row){
						      			$total_row_spring_printing=0;
										$total_row_spring_printing=$row->SPRING_PRINTING;
						      ...
						      $total_p_3rdlast_month=$total_printing+$total_labeling+$total_spring_printing;

						      echo money_format('%!.0n',$total_p_3rdlast_month);
							...
							<tr>
						     ...
						     if($printing_total_year==FALSE){
						      ...
						      if($labeling_total_year==FALSE){ 
						      		$total_labeling=0;
						      	}else{
						      		$total_labeling=0;
						      		foreach($labeling_total_year as $row){
						      			$total_row_labeling=0;
										$total_row_labeling=$row->TECHNOSHEEL_LABEL_2+$row->TECHNOSHELL_HALF_LABEL+$row->TECHNOSHELL_LABEL_3;
						      	...
						      	if($spring_printing_total_year==FALSE){ 
						      		$total_spring_printing=0;
						      	}else{
						      		$total_spring_printing=0;
						      		foreach($spring_printing_total_year as $row){
						      			$total_row_spring_printing=0;
										$total_row_spring_printing=$row->SPRING_PRINTING;
						      ...
						      $total_p_year=$total_printing+$total_labeling+$total_spring_printing;

						      echo money_format('%!.0n',$total_p_year);
		    ...
		    <div class="column">
		    	...
		    	<div class="ui segments">
		    		<div class="ui blue segment">
					    <p><a  class="ui orange label">PENDING ORDER</a><a class="ui olive label"><i class="calendar icon"></i>01 TO <?php echo strtoupper(date('d M Y'));?></a></p>
					</div>
					<div class="ui segment">
						<?php
					...
					if($pending_sales_order_opening==FALSE){
					 ...
					 if($pending_sales_order_opening==FALSE){
					 	echo "<tr><td colspan='10'>NO OPENING ORDERS</td></tr>";
					 }else{
					foreach($pending_sales_order_opening as $coex_order_row){
					    ...
					    echo "<tr>
							    	...
							    	<td class='negative right aligned'>".money_format('%!.0n',$total_total_coex_pending_order_quantity)."</td>
							    	<td class='negative right aligned'>".money_format('%.0n',$total_total_coex_pending_order_value)."</td>
							    	<td class='negative right aligned'>&#8377;".number_format($total_total_coex_pending_avg_price,2,"."," ")."</td>
							    	<td class='positive right aligned'>".money_format('%!.0n',$total_digital_pending_order_quantity)."</td>
							    	<td class='positiveright aligned'>".money_format('%.0n',$total_digital_pending_order_value)."</td>
							    	<td class='positive right aligned'>&#8377;".number_format($total_digital_pending_order_avg_price,2,"."," ")."</td>
							    	<td class='active right aligned'>".money_format('%!.0n',$total_total_pending_order_quantity)."</td>
							    	<td class='active right aligned'>".money_format('%.0n',$total_total_pending_order_value)."</td>
							    	<td class='active right aligned'>&#8377;".number_format($total_total_pending_avg_price,2,"."," ")."</t>
					...
					foreach($pending_sales_order as $coex_order_row){
					    ...
					    echo "<tbody>
							    <tr>
							    	<td><b>NEW ORDER</b></td>
							    	<td class='negative right aligned'>".money_format('%!.0n',$total_total_coex_pending_quantity)."</td>
							    	<td class='negative right aligned'>".money_format('%.0n',$total_total_coex_pending_value)."</td>
							    	<td class='negative right aligned'>&#8377;".round($total_total_coex_pending_avg_price,2)."</td>
							    	<td class='positive right aligned'>".money_format('%!.0n',$total_digital_pending_quantity)."</td>
							    	<td class='positive right aligned'>".money_format('%.0n',$total_digital_pending_value)."</td>
							    	<td class='positive right aligned'>&#8377;".round($total_digital_pending_avg_price,2)."</td>
							    	<td class='active right aligned'>".money_format('%!.0n',$total_total_pending_quantity)."</td>
							    	<td class='active right aligned'>".money_format('%.0n',$total_total_pending_value)."</td>
					    	...
					    	echo "<tr>
							    	...
							    	<td class='negative right aligned'>".money_format('%!.0n',$total_prin_type_coex_quantity)."</td>
							    	<td class='negative right aligned'>".money_format('%.0n',$total_print_type_coex_value)."</td>
							    	<td class='negative right aligned'>&#8377;".round($total_print_type_coex_avg_price,2)."</td>
							    	<td class='positive right aligned'>".money_format('%!.0n',$row_coex->DIGITAL)."</td>
									<td class='positive right aligned'>".money_format('%.0n',$row_coex->DIGITAL_VALUE)."</td>
									<td class='positive right aligned'>&#8377;".round($digital_avg_price,2)."</td>
							    	<td class='active right aligned'>".money_format('%!.0n',$total_prin_type_quantity)."</td>
							...
							echo "<tr>
							    	<td><b>BALANCE ORDER</b></td>
							    	<td class='negative right aligned'><b>".money_format('%!.0n',$total_balace_coex_pending_quantity)."</td>
							    	<td class='negative right aligned'><b>".money_format('%.0n',$total_balace_coex_pending_value)."</td>
							    	<td class='negative right aligned'><b>&#8377;".round($total_balace_coex_pending_avg,2)."</td>
							    	<td class='positive right aligned'><b>".money_format('%!.0n',$total_balance_spring_pending_order_quantity)."</td>
							    	<td class='positive right aligned'><b>".money_format('%.0n',$total_balance_spring_pending_order_value)."</td>
							    	<td class='positive right aligned'><b>&#8377;".round($total_balance_spring_pending_order_avg,2)."</td>
							    	<td class='active right aligned'><b>".money_format('%!.0n',$total_balace_pending_order_quantity)."</td>
							    	<td class='active right aligned'><b>".money_format('%.0n',$total_balace_pending_order_value)."</td>
							    	<td class='active right aligned'><b>&#8377;".round($total_balace_pending_order_avg,2)."</td>
							  		</tr>";
		    	...
		    	<div class="ui red segments">
		    		<div class="ui red segment">
					    <p><a  class="ui orange label">SALES</a>
					    	...
					    	<a class="ui olive label"><i class="calendar icon"></i>01 TO <?php echo date('d');?></a></p>
					</div>
		    		<div class="ui segment">
		    			<?php
						setlocale(LC_MONETARY, 'en_IN');
					...
					foreach($print_type_wise_sales_domestic as $row_coex){
					    ...
					    $total_total_print_type_spring_to_sarl_quantity=0;
					    ...
					    $total_dom_export_spring_avg=($total_dom_export_spring_quantity<>0 ? ($total_dom_export_spring_value/$total_dom_export_spring_quantity) : "0");
					    ...
					    echo "<tr>
					    		<td colspan='2'><b>TOTAL</b></td>
					    			<td class='active right aligned'>".money_format('%!.0n',$total_dom_export_coex_quantity)."</td>
					    			<td class='active right aligned'>".number_format($total_all_coex_pc)."%</td>
									<td class='active right aligned'>".money_format('%.0n',$total_dom_export_coex_value)."</td>
									<td class='active right aligned'>&#8377;".number_format($total_dom_export_coex_avg,2,'.','')."</td>

									<td class='negative right aligned'>".money_format('%!.0n',$total_dom_export_spring_quantity)."</td>
									<td class='negative right aligned'>".number_format($total_all_spring_pc)."%</td>
									<td class='negative right aligned'>".money_format('%.0n',$total_dom_export_spring_value)."</td>
									<td class='negative right aligned'>&#8377;".number_format($total_dom_export_spring_avg,2,'.','')."</td>

									<td class='active warning right aligned'>".money_format('%!.0n',$total_dom_export_coex_spring_quantity)."</td>
									<td class='active warning right aligned'>".money_format('%!.0n',$total_dom_export_coex_spring_value)."</td>
		...
		<div class="ui equal width grid">
		  <div class="equal width row">
		    <div class="column">
		    	...
		    	<div class="ui segments">
		    		<div class="ui blue segment">
					    <p><a  class="ui orange label">SALES</a><a  class="ui blue label"> PRINT TYPE WISE</a><a  class="ui green label"> DIA WISE</a> 
					    	<a class="ui olive label"><i class="calendar icon"></i>01 TO <?php echo strtoupper(date('d M Y'));?></a></p>
					</div>
					<div class="ui segment">
						<?php
						setlocale(LC_MONETARY, 'en_IN');
					...
					foreach($print_type_wise_sales_coex as $row_coex){
						...
						echo "<tr>
								...
								<td class='warning right aligned'>".money_format('%!.0n',$row_coex->LABEL)."</td>
								<td class='warning right aligned'>".money_format('%.0n',$row_coex->LABEL_VALUE)."</td>
		...
		<div class="ui equal width grid">
		  <div class="equal width row">
		    <div class="column">
		    	...
		    	<div class="ui segments">
		    		<div class="ui blue segment">
					    <p><a  class="ui orange label">SALES</a><a  class="ui blue label"> TOP CUSTOMER</a> 
					    	<a class="ui olive label"><i class="calendar icon"></i>01 TO <?php echo strtoupper(date('d M Y'));?></a></p>
					</div>
					<div class="ui segment">
						<?php
		...
		<div class="ui equal width grid">
		  <div class="equal width row">
		    ...
		    <div class="column">
		    	...
		    	<div class="ui red segments">
		    		<div class="ui red segment">
					    <p><a  class="ui orange label">OTIF STATISTICS</a>
					    	<a class="ui olive label"><i class="calendar icon"></i>01 TO <?php echo date('d');?></a></p>
					</div>
		    		<div class="ui segment">
						...
						if($order_dispatched_count==FALSE){}else{
							foreach($order_dispatched_count as $order_dispatched_count_row){
								...
								<td>".($total_order_dispatched_count<> 0 ? (($total_order_dispatched_count/$total_order_dispatched_count)*100) : 0)."%</td></tr>";
						...
						if($order_completed_dispatched_count==FALSE){}else{
							foreach($order_completed_dispatched_count as $order_completed_dispatched_count_row){
								...
								echo "<tr><td>Number of Orders Completed</td><td>".$order_completed_dispatched_count_row->order_completed."</td><td>".
								($total_order_completed_dispatched_count<> 0 ? round((($total_order_completed_dispatched_count/$total_order_dispatched_count)*100),0) : 0)
								."%</td></tr>";
						...
						if($order_short_completed_dispatched_count==FALSE){}else{
							foreach($order_short_completed_dispatched_count as $order_short_completed_dispatched_count_row){
								...
								<td>".($total_order_short_completed_dispatched_count<>0 ? round((($total_order_short_completed_dispatched_count/$total_order_dispatched_count)*100),0) : 0)."%</td>
						...
						if($open_order_count==FALSE){}else{
							foreach($open_order_count as $open_order_count_row){
								...
								<td>".($total_open_order_count<>0 ? round((($total_open_order_count/$total_order_dispatched_count)*100),0) : 0)."%</td></tr>";
						...
						if($print_type_wise_sales_coex==FALSE){}else{
							...
							echo "<tr><td>Total Sales Dispatched Volume</td><td>".money_format('%!.0n',$total_total_print_type_sales_quantity)."</td><td>".($total_total_print_type_sales_quantity<>0 ? round((($total_total_print_type_sales_quantity/$total_total_print_type_sales_quantity)*100),0) : 0)."%</td></tr>";
							}
						...
						if($order_completed_dispatched_volume==FALSE){}else{
							foreach($order_completed_dispatched_volume as $order_completed_dispatched_volume_row){
								...
								echo "<tr><td>Completed Orders Volume</td><td>".money_format('%!.0n', $order_completed_dispatched_volume_row->full_dispatch_volume)."</td><td>".($total_order_completed_dispatched_volume<>0 ? round((($total_order_completed_dispatched_volume/$total_total_print_type_sales_quantity)*100),0) : 0)."%</td></tr>";
						...
						if($order_short_completed_dispatched_volume==FALSE){}else{
							foreach($order_short_completed_dispatched_volume as $order_short_completed_dispatched_volume_row){
								...
								echo "<tr><td>Short Close Orders Volume</td><td>".money_format('%!.0n', $order_short_completed_dispatched_volume_row->short_tubes)."</td><td>".($total_order_short_completed_dispatched_volume<>0 ? round((($total_order_short_completed_dispatched_volume/$total_total_print_type_sales_quantity)*100),0) : 0)."%</td></tr>";
						...
						if($order_open_dispatched_volume==FALSE){}else{
							foreach($order_open_dispatched_volume as $order_open_dispatched_volume_row){
								...
								echo "<tr><td>Open Orders Volume</td><td>".money_format('%!.0n', $order_open_dispatched_volume_row->open_order_dispatch_volume)."</td><td>".($total_order_open_dispatched_volume<>0 ? round((($total_order_open_dispatched_volume/$total_total_print_type_sales_quantity)*100),0) : 0)."%</td></tr>";
						...
						echo "<tr><td>--------------------</td><td>--------------------</td><td>---------------</td></tr>
					      	<tr><td><b>STATISTICS</td><td><b>VALUE</td><td><b>%</td></tr>";

						if($total_total_print_type_sales_value<>0){
							echo "<tr><td>Total Net Sales</td><td>".money_format('%.0n',$total_total_print_type_sales_value)."</td></tr>";
						}
						...
						if($sales_total_order_completed_net==FALSE){}else{
							foreach($sales_total_order_completed_net as $sales_total_order_completed_net_row){
								echo "<tr><td>Completed Orders Net Sales</td><td>".money_format('%.0n', $sales_total_order_completed_net_row->sales_values)."</td></tr>";
						...
						if($sales_total_order_short_completed_net==FALSE){}else{
							foreach($sales_total_order_short_completed_net as $sales_total_order_short_completed_net_row){
								echo "<tr><td>Sales in Short</td><td>".money_format('%!.0n', $sales_total_order_short_completed_net_row->short_tubes_sales)."</td></tr>";
						...
						if($sales_total_order_short_completed_net==FALSE){}else{
							foreach($sales_total_order_short_completed_net as $sales_total_order_short_completed_net_row){
								echo "<tr><td>Loss Due to Short Sales</td><td>".money_format('%.0n', $sales_total_order_short_completed_net_row->short_tubes_sales)."</td></tr>";
								//$total_order_short_completed_dispatched_volume=$order_short_completed_dispatched_volume_row->short_tubes;
					    	}
						}
