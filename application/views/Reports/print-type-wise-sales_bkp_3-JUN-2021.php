...
<script>
	$(document).ready(function(){
		...
		$("#search").click(function(){
			...
			$.ajax({type: "POST",url: "<?php echo base_url('index.php/ajax/print_type_wise_sales');?>",data: {from_date : $(".from_date").val(),to_date :$('.to_date').val(),convert:$('.convert').val(),for_export :$('#for_export').val(),customer_no : $('.customer_no:checked').serialize(),sleeve_dia : $('.sleeve_dia:checked').serialize(),inv_type : $('.inv_type:checked').serialize(),customer_category :$('#customer_category').val()},cache: false,success: function(html){
