				...
				<span id="check">
					...
					if($total_order_received_by_customer==FALSE){
						...
						echo '<table class="ui sortable selectable celled table" style="font-size:9px;">
					        	<thead>
								   <tr>
								    	<th colspan="29"><a class="ui orange label">TOTAL APPROVED ORDER RECEIVED FROM CUSTOMERS</a> <a class="ui green label">ORDER DATE</a>';
								    	if($account_periods_master==FALSE){
								    	}else{
								    		foreach ($account_periods_master as $account_periods_master_row ){
					...
					foreach($total_order_received_by_customer as $row){
					   ...
					   echo "<thead>
								...
								<th class='positive right aligned'>".money_format('%!.0n',$total_ten_unapproved_order_quantity)."</th>
								<th class='positive right aligned'>".money_format('%.0n',$total_ten_unapproved_order_value)."</th>		
								<th class='positive right aligned'>".money_format('%.0n',$total_ten_unapproved_order_avg)."</th>
					   ...
					   $total_other_screen_flexo_avg=($total_other_screen_flexo_value!=0 ? $total_other_screen_flexo_value/$total_other_screen_flexo_quantity : '0');
					    ...
					    echo "</tbody>
					    	...
					    	<tr>
								...
								<td class='active right aligned'>".money_format('%!.0n',$total_other_unapproved_order_quantity)."</td>
								<td class='active right aligned'>".money_format('%.0n',$total_other_unapproved_order_value)."</td>
								<td class='active right aligned'>&#8377;".round($total_other_unapproved_order_avg,2)."</td>
