...
<script>
	$(document).ready(function(){
		...
		$("#search").click(function(){
			...
			$.ajax({type: "POST",url: "<?php echo base_url('index.php/ajax/pending_sales_order_by_cap');?>",data: {from_date : $(".from_date").val(),to_date :$('.to_date').val(),cap_code:$('#cap_code').val()},cache: false,success: function(html){
		...
		<table class="form_table_design">
			<tr>
				...
				<td>
					<table class="form_table_inner">
						<tr>
							<td class="label" width="25%">Cap Code :</td>
				...
				<span id="check">
					...
					foreach($pending_sales_order as $coex_order_row){
						...
						<?php 
							...
							$customer_wise=$this->sales_order_book_model->pending_sales_order_by_customer_group_with_cap($new_from_date,date('Y-m-d'),$coex_order_row->cap_code);
							$d=0;
							$total_d=0;
