...
<script>
	$(document).ready(function(){
		...
		$("#search").click(function(){
			...
			$.ajax({type: "POST",url: "<?php echo base_url('index.php/ajax/pending_sales_order_on_oc_date');?>",data: {from_date : $(".from_date").val(),to_date :$('.to_date').val(),convert:$('.convert').val(),for_export :$('#for_export').val(),customer_no : $('.customer_no:checked').serialize(),sleeve_dia : $('.sleeve_dia:checked').serialize(),customer_category :$('#customer_category').val()},cache: false,success: function(html){
				...
				<span id="check">
					...
					if($pending_sales_order==FALSE){
						...
						echo '<table class="ui sortable selectable celled table" style="font-size:10px;">
					        	<thead>
								   <tr>
								    	<th colspan="20"><a class="ui orange label">PENDING ORDER BY OC DATE</a>
