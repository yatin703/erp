...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/wip_release_save');?>" method="POST" >
    ...
    <?php foreach($coex_extrusion_heading_wip as $row):?>
	  <table class="form_table_design">
	    <tr>
		  <td>
			<table class="form_table_inner">
            <input type="hidden" name="heading_id" value="<?php echo set_value('heading_id',$row->heading_id);?>">
			<input type="hidden" name="qc_id" value="<?php echo set_value('qc_id',$row->qc_id);?>">
			<input type="hidden" name="wip_id" value="<?php echo set_value('wip_id',$row->hwip_id);?>">
			...
			<tr>
				...
				<td><select name="to_process" id="to_process">				
					...
					<option value="5" <?php echo set_select('heading_scrap','5');?>>Scrap</option>
					<option value="3" <?php echo set_select('printing','3');?>>Printing</option>
					<option value="6" <?php echo set_select('capping','6');?>>Capping</option>
					<option value="7" <?php echo set_select('release_qc','7');?>>Return QC</option>
				</select>
				</td>
			</tr>
