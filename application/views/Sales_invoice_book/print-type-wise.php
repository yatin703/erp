...
<script>
	$(document).ready(function(){
		...
		$("#search").click(function(){
			...
			$.ajax({type: "POST",url: "<?php echo base_url('index.php/ajax/print_type_wise_sales');?>",data: {from_date : $(".from_date").val(),to_date :$('.to_date').val(),for_export :$('#for_export').val(),customer_no : $('.customer_no:checked').serialize(),sleeve_dia : $('.sleeve_dia:checked').serialize(),customer_category :$('#customer_category').val()},cache: false,success: function(html){
			...
			<div class="column">
				<span id="check">
					...
					if($print_type_wise_sales_coex==FALSE){
						...
						echo '<table class="ui sortable selectable celled table" style="font-size:12px;">
					        	<thead>
								   <tr>
								    	<th colspan="13"><a class="ui orange label">COEX SALES</a></th>
								  </tr>
								  <tr>
					        			<th></th>
				...
				<?php
					...
					if($print_type_wise_sales_spring==FALSE){
						...
						<br/>
						<table class="ui sortable selectable celled table" style="font-size:12px;">
					        	<thead>
								   <tr>
								    	<th colspan="13"><a class="ui yellow label">SPRING SALES</a></th>
								  </tr>
