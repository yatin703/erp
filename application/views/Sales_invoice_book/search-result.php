					    ...
					    if($rowspan>0){
							...
							if($mrow->consin_adr_company_id!=''){
								...
								foreach ($result_consignee as $row_consignee){
									...
									$ship_to=strtoupper($this->common_model->get_state_name($row_consignee->zip_code,$this->session->userdata['logged_in']['company_id']));
								...
								$ship_to=strtoupper($this->common_model->get_state_name($mrow->zip_code,$this->session->userdata['logged_in']['company_id']));
							}
							...
							foreach ($result as $drow){
								...
								if(!empty($ad_id)){
									...
									// echo "<br/>";
									// echo "<br/>";
									if($artwork_result==TRUE){
									foreach ($artwork_result as $artwork_row) {
										$print_type_artwork=$artwork_row->print_type;
										$tube_foil=$artwork_row->hot_foil;
									}
									}else{

										$springtube_artwork_result=$ci->artwork_springtube_model->active_record_search('springtube_artwork_devel_master',$artwork,$search,$from,$to,$this->session->userdata['logged_in']['company_id']);

										if($springtube_artwork_result==TRUE){
										foreach ($springtube_artwork_result as $artwork_row) {
											$print_type_artwork=$artwork_row->print_type;
											$tube_foil=$artwork_row->cold_foil_1;
										}
								...
								if($mrow->for_export=='1'){
									...
									<td>".$po_no."</td>
									<td>".($po_no!=''?$po_date:'')."</td>
									<td>".$cust_product_no." ".$this->common_model->get_parent_name($drow->article_no,$this->session->userdata['logged_in']['company_id'])."</td>
									...
									<td>".$cap_orifice."</td>
									<td>".$this->common_model->get_article_name($tube_foil,$this->session->userdata['logged_in']['company_id'])."</td>
									";
