...
<div class="record_form_design">
		<div class="record_inner_design" style="overflow: scroll;">
				...
				<table class="ui sortable selectable celled table" style="font-size:10px;white-space: nowrap;" id="table-1">
					<thead>
						<tr>
							<th>Sr. No.</th>
							<th>Invoice Date</th>
				...
				else 
				{
					...
					foreach($ar_invoice_master as $mrow){
					    ...
					    if($rowspan>0){
							...
							<!--<td rowspan='".$rowspan."'><a href='".base_url('index.php/Sales_invoice_book/View/'.$mrow->ar_invoice_no.'')."' target='_blank'>".$mrow->ar_invoice_no."</a></td>-->
							...
							foreach ($result as $drow){
								...
								foreach ($order_master_result as $order_master_row) {
									...
									$po_date=$ci->common_model->view_date(($order_master_row->cust_order_date!='0000-00-00'?$order_master_row->cust_order_date:""),$this->session->userdata['logged_in']['company_id']);
								...
								foreach ($so_result as $so_row) {
									...
									$total_order_quantity=$so_row->total_order_quantity;
								}
								...
								if($mrow->for_export=='1'){
									...
									<td>".($drow->ref_ord_no<>'' ? number_format($this->common_model->read_number($total_order_quantity,$this->session->userdata['logged_in']['company_id']),0,'.',',') : '-')."</td>
									<td>".($drow->ref_ord_no<>'' ? $this->sales_order_book_model->get_order_status($drow->ref_ord_no) : '-')."</td>
									<td>".$oc_date."</td>
