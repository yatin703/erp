...
<div class="record_form_design">
	...
	<div class="record_inner_design">
			<table class="record_table_design_without_fixed">
				...
				<?php if($other_cost_master==FALSE){
					echo "<tr><td colspan='11'>No Active Records Found</td></tr>";
				}else{
							$i=($this->uri->segment(3)==''? 1: $this->uri->segment(3)+1);
							foreach($other_cost_master as $row){
								echo "<tr>
									<td>".$i++."</td>
									...
									<td>".($row->order_flag==0 ? "COEX" : ($row->order_flag==1 ? "SPRING" : "ON BOTH"))."</td>
									<td>$row->sleeve_dia</td>
									<td>$row->cap_type</td>
									<td>$row->cap_finish</td>
									<td>$row->other_cost</td>
									
									<td>&#8377; ".number_format($row->other_cost_value,'2','.',',')."</td>
									<td>".($row->sale_of_tubes<>0 ? number_format($row->sale_of_tubes,'0','.',',') : '0')." NOS</td>
								...
								foreach($formrights as $formrights_row){
									...
									echo ($formrights_row->modify==1 ?'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->ocsm_id).'" title="Modify" target="_blank"><i class="edit icon"></i></a> ' : '');	

									echo ($formrights_row->delete==1 ?'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->ocsm_id).'" title="Delete" target="_blank"><i class="trash icon"></i></a> ' : '');

									echo ($formrights_row->copy==1 ?'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/copy/'.$row->ocsm_id).'" title="Copy" target="_blank"><i class="copy icon"></i></a> ' : '');			
