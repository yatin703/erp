					...
					echo "<tr><td colspan='9'>No Active Records Found</td></tr>";
							...
							foreach($packing_material_consumption_master as $row){
								echo "<tr>
									...
									<td>".($row->archive==1 ? 'Inactive' :'Active')."</td><td>";

									foreach($formrights as $formrights_row){

									echo ($formrights_row->modify==1 ?'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->pmcm_id).'" title="Modify" target="_blank"><i class="edit icon"></i></a> ' : '');	

									echo ($formrights_row->delete==1 ?'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->pmcm_id).'" title="Delete" target="_blank"><i class="trash icon"></i></a> ' : '');

									echo ($formrights_row->copy==1 ?'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/copy/'.$row->pmcm_id).'" title="Copy" target="_blank"><i class="copy icon"></i></a> ' : '');			
									...
									//echo"<td><a href=".base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->pmcm_id.'').">Modify</a><a href=".base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->pmcm_id.'')."> | Delete</a>";
