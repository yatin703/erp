<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/create').'" class="ui blue tag label" ><i class="add circle icon"></i>New</a> : ' : '');

    echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/existing').'" class="ui blue tag label" ><i class="add circle icon"></i>Existing</a> : ' : '');
