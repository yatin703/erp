...
$(document).ready(function() {
   $(".release_qty").live("change", function() {
	   var ok_qty      = Number($('input[name=shortfall_qty]').val());
	   var release_qty = Number($("#release_qty").val());
      

      if(release_qty > ok_qty){
       	alert('Ok Qty Greater Than Release Qty');
		   Number($('#release_qty').removeAttr('value'));
		   location.reload();
      }else{
       	$("#shortfall_qty").val(ok_qty - release_qty);
...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/wip_release_save/'.$row->aprint_wip_id);?>" method="POST" >
				...
				<td>
					<table class="form_table_inner">
						...
						<tr>
							...
							<td ><input type="text" name="release_qty" id="release_qty" class="release_qty" size="20" value="" /></td>
						</tr>
						<tr>
