         ...
         <table class="ui very basic collapsing celled table"  style="font-size:10px;">
            ...
            <tbody>
               ...
               echo "<tr><td colspan='12'>No Active Records Found</td></tr>";
               	...
               	$i=1;
               	foreach($coex_extrusion_printing as $row){
               		...
               		echo "<tr>
                           ...
                           <td style='text-align:right;background:#fff;' ".($row->printing_qty==0? "class='td_wip_cost'":"")."> <span style='color:#2c662d!important;'><b> ".number_format($row->printing_qty,0,'.',',')." </b><i> No</i></span></td>
                           <td>".($row->flag==1 ? '<i style="color:#06c806" class="check circle icon"></i>' :'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/create/'.$row->printing_id).'"><i class="hand paper icon"></i></a>')."</td>
                           <td>".$row->form_process."</td>
                          
               				</tr>";
               		$i++;
               	 $sum_bm_wip_qty+=$row->printing_qty;
               }
