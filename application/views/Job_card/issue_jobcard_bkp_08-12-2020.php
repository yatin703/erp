		...
		<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/save_issue_jobcard');?>" method="POST">
					...
					foreach ($job_card as $job_card_row):
						...
						$calculated_purchase_price="";
							...
							echo "<tr>
										...
										-->
										<td class='label'><input type='text' name='quantity_$job_card_row->mm_id' value='".($job_card_row->work_proc_no=='11' ? '0' : $this->common_model->read_number($job_card_row->demand_qty,$this->session->userdata['logged_in']['company_id']))."' size='8'>".$uom."</td>
           ...
           //                              $result_reserved_qty_manu=$this->sales_order_item_parameterwise_model->get_total_issue_qty('reserved_quantity_manu',$this->session->userdata['logged_in']['company_id'],$data_reserved_quantity_manu);
			        ...
			        //                     } 
										...
										echo"</td>
										<td>".$uom."</td>
										<td>".$this->common_model->read_number($job_card_row->calculated_purchase_price,$this->session->userdata['logged_in']['company_id'])."
										</td>
										<td>".round($this->common_model->read_number($job_card_row->calculated_purchase_price,$this->session->userdata['logged_in']['company_id'])*$this->common_model->read_number($job_card_row->qty,$this->session->userdata['logged_in']['company_id']),2)."</td>
			...
			endforeach;?>
				...
				<tr><td colspan="6">TOTAL</td><td><?php echo $sum_qty?></td><td>KGS</td><td></td><td><?php echo $sum_amount?></td></tr>
				</table>
