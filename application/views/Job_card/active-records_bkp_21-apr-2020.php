							...
							foreach($job_card as $row){
								...
								echo "<tr>
									...
									foreach($formrights as $formrights_row){ 
										echo"<td>";	
										echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/issue_jobcard/'.$row->mp_pos_no.'/'.$spec_id.'/'.$version).'" target="_blank">ISSUE</a>' : '');
										echo"</td><td>";
										echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/sales_order_item_parameterwise/against_jobcard/'.$row->mp_pos_no.'/'.$spec_id.'/'.$version).'" target="_blank">AGAINST JOBCARDS </a>' : '');
										echo"</td><td>";
										...
										if($order_flag=='1' && $row->jobcard_status==0 && $row->jobcard_type==1){

											$total_qc_hold_meters=0;

											$dataa=array('jobcard_no'=>$row->mp_pos_no);
              								$table='springtube_extrusion_production_details';
          									$springtube_extrusion_production_details=$this->springtube_extrusion_production_model->active_details_records($table,$dataa);
          									foreach ($springtube_extrusion_production_details as $springtube_extrusion_production_details_row) {
          										$total_qc_hold_meters+=$springtube_extrusion_production_details_row->total_qc_hold_meters;
          									}

          									if($total_qc_hold_meters==0){
											?>		
												<a id="jobcard_close" href="#" onclick="window.open('<?php echo base_url("/index.php/".$this->router->fetch_class()."/close_jobcard/$row->mp_pos_no/$spec_id/$version");?>','Close Jobcard','height=200,width=615')" title="Close Jobcard" ><i class="edit icon"></i>
												</a>
