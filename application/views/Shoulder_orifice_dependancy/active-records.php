					...
					echo "<tr><td colspan='12'>No Active Records Found</td></tr>";
							...
							foreach($shoulder_orifice_dependancy as $row){
								echo "<tr>
									...
									<td>".$this->common_model->read_number($row->cap_weight,$this->session->userdata['logged_in']['company_id'])."</td>
									<td>$row->cap_drawing</td>
									<td>".($row->archive==1 ? 'Inactive' :'Active')."</td>
									<td><a href=".base_url('index.php/'.$this->router->fetch_class().'/modify/'.$row->sod_id.'').">Modify</a><a href=".base_url('index.php/'.$this->router->fetch_class().'/delete/'.$row->sod_id.'')."> | Delete</a>
