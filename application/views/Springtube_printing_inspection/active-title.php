<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    $table='springtube_printing_inspection_master';
  	...
  	echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'').'" class="ui teal tag label">Film Printing Inspection </a> : ' : '');
     ...
     echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/create').'" class="ui blue tag label" ><i class="add circle icon"></i>Outsourced</a> : ' : '');
