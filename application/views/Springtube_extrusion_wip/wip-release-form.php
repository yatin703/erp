...
<script>
	...
	$(document).ready(function(){
		...
		$("#release_meters").bind('blur keyup', function(){
			...
			if($("#release_meters").val()!='' || $("#release_meters").val()!='0'){
				...
				if(parseInt($("#release_meters").val()) > parseInt($("#total_ok_meters").val())){
					...
					$.ajax({
				   		...
				   		data: {order_no : $('#release_to_order_no').val(), article_no : $("#release_article_no").val(),total_meters : $("#release_meters").val() },
				   		cache: false,
				   		success: function(html){
				    		setTimeout(function () {$("#loading").hide();$("#cover").hide();},200);
				    		//alert(html);
				    		if(html!=""){
				    			$("#expected_qty").val(html);
				    		}
		...
		$("#release_to_order_no").bind('blur',function() {
			...
			if(release_to_order_no.length>=13){
				...
				$.ajax({
				   	...
				   	type: "GET",
				   	url: "<?php echo base_url();?>index.php/ajax_springtube/spring_open_so_no",
				   	data: {q : $('#release_to_order_no').val()},
				   	cache: false,
				   	success: function(html){
				    		setTimeout(function () {$("#loading").hide();$("#cover").hide();},200);
				    		//alert(html);
				    	if(html=='No Record Found'){
				    		...
				    		alert('Jobcard is not allowed for Sales order ' + $('#release_to_order_no').val());
		...
		$("#release_article_no").bind('change',function() {
   		...
   		// $("#expected_qty").bind('keyup blur',function(){
					...
					<table class="form_table_inner">
						...
						<tr>
				            ...
				            <td><input type="text"  name="sleeve_diameter" id="sleeve_diameter" value="<?php echo $sleeve_diameter;?>" readonly>
				             &nbsp;Length : <input type="text"  name="sleeve_length" id="sleeve_length" value="<?php echo $sleeve_length.' MM';?>"  size="10"readonly>
				            </td>
				        </tr>
				        <tr>
						...
						<tr>
							...
							<input type="text" name="total_ok_meters" id="total_ok_meters"  size="20" value="<?php echo set_value('total_ok_meters',$row->total_ok_meters);?>" readonly/>
							&nbsp;Ok Reels  : <input type="number" name="total_ok_reels" id="total_ok_reels"  size="20" maxlength="5" min="1" max="1000" step="1" value="<?php echo set_value('total_ok_reels',round($row->total_ok_meters/$default_reel_length,2));?>" readonly/>
						</td>
						</tr>
						<!--<tr>
					...
					-->
						...
						<tr>
							...
							<td><select name="release_to" id="release_to">					
								...
								<!-- <option value="3" <?php echo set_select('release_to','3');?> >OUTSOURCE FOR PRINTING</option> -->
