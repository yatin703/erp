			   	...
			   	success: function(data){
			    		...
			    		//alert(data);
			       		...
			       		if(data.status){      			
			       			...
			       			$("#table_counter_1").append('<tr><th width="1%">#</th><th width="11%">Counter</th><th width="77%">Counter Stop Reason</th> </tr>');
			       			...
			       			for(var k=0;k<obj.length;k++){
			       				...
			       				var tr='<tr><td>'+j+'</td><td>'+obj[k].counter+'</td><td>'+obj[k].stop_reason+'</td> </tr>';
			...
			<table class="middle_form_table_design" id="table_counter_1" width="100%">
				...
				<?php
					...
					foreach($springtube_printing_production_details as $details_row){
						...
						echo'<tr><td>'.$j++.'</td><td>'.$details_row->counter.'</td><td>'.$details_row->stop_reason.'</td><td><a href="#" name="editButton" editId="'.$details_row->details_id.'" ><i class="edit icon" ></i></a></td></tr>';
...
<script>
	...
	$(document).ready(function(){
		...
		$("[href]").click(function(e) {
			...
			if(editId!=''){
			 	...
			 	$.ajax({
				   	...
				   	type: "POST",
				   	url: "<?php echo base_url();?>/index.php/springtube_printing_production/modify_counter",
				   	data: {production_id : $("#production_id").val(), editId : editId },
				   	cache: false,
				   	success: function(html){
				   				    		
				    		var obj=JSON.parse(html);
				    		//alert(obj.stop_reason);
				    		if(obj!=''){
				    		var string='<table class="ui very compact celled table"><thead><th>Counter</th><th>Counter Stop Reason</th><th></th></thead><tbody><td><input type="hidden" name="edit_details_id" id="edit_details_id" value="'+obj.details_id+'" /><input type="number" id="edit_counter" value="'+obj.counter+'" ></td><td><input type="text" id="edit_reason" value="'+obj.stop_reason+'" ></td><td><div class="ui buttons"><button class="ui positive button" id="btnEditCounter">Update Counter</button></div></td></tbody>';	
				    		$("#editCounter").html(string);
				    		$("#editCounterModal").show();
		...
		$("#btnEditCounter").click(function(e){
