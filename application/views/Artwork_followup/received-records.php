								...
								echo "<tr>
									...
									foreach($formrights as $formrights_row){ 
										...
										echo ($formrights_row->new==1 ? 
											'<a href="'.base_url('index.php/'.$this->router->fetch_class().'/approved/'.substr($followup_received_row->record_no,0,6).'/'.substr($followup_received_row->record_no,9)).'/'.$followup_received_row->transaction_no.'" onClick="return confirm('.$a.');"><i class="thumbs outline up icon"></i> 
											&nbsp; 

											<a href="'.base_url('index.php/'.$this->router->fetch_class().'/notapproved/'.substr($followup_received_row->record_no,0,6).'/'.substr($followup_received_row->record_no,9)).'/'.$followup_received_row->transaction_no.'" onClick="return confirm('.$a.');"><i class="thumbs outline down icon"></i>'
											 : 
											 '');
