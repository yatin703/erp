      ...
      <div class="ui label">
        ...
        <table cellpadding="5" cellspacing="0" style="border:1px solid #D9d9d9;">
            ...
            <tr class="item last">
                ...
                <td>PO DATE</td>
                <td></td>
                <td><?php echo $this->common_model->view_date($order_master_row->cust_order_date,$this->session->userdata['logged_in']['company_id']); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>OC Date</b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($order_master_row->oc_date!='0000-00-00' ? $this->common_model->view_date($order_master_row->oc_date,$this->session->userdata['logged_in']['company_id']) : '-');?></td>
            </tr>
        </table>
        <table cellpadding="5" cellspacing="0" style="border:1px solid #D9d9d9;">
            <tr class="heading">
            ...
            <tr class="item">
                <td>TOLERANCE %</td>
                <td></td>
                <td style="border-right:1px solid #D9d9d9;"><?php echo ($order_master_row->dispatch_tolerance<>0 ? "<b>".$order_master_row->dispatch_tolerance." % </b>" : "0 %");?></td>
                <td>REF ORDER NO</td>
                <td></td>
                <td><?php echo $order_master_row->ref_order_no;?><?php //echo ($order_master_row->po_file!='' ? '<a href="'.base_url('assets/'.$this->session->userdata['logged_in']['company_id'].'/po_file/'.$order_master_row->po_file.'').'" target="_blank"><i class="file pdf outline icon"></i></a> ' : '-');?></td>
            </tr>
              ...
              foreach ($order_close_reasons_master_result as $key => $order_close_reasons_master_row) {
                ...
                echo'<table cellpadding="5" cellspacing="0" style="border:1px solid #D9d9d9;">
                    ...
                    <tr class="item">
                        ...
                        <td width="5%"></td>
                        <td width="35%" style="border-right:1px solid #D9d9d9;">'.strtoupper($this->sales_order_book_model->get_order_status($order_master_row->order_no)).'</td>
                        <td width="10%">DATE</td>
                        <td width="5%"></td>
                        <td width="35%" style="border-right:1px solid #D9d9d9;">'.$this->common_model->view_date($order_master_row->trans_closed_date,$this->session->userdata['logged_in']['company_id']).'</td>

                    </tr>
                    <tr class="item">
                        <td width="10%">REMARK</td>
                        <td width="5%"></td>
                        <td colspan="4">'.strtoupper($reasons.','.$order_master_row->trans_closed_remarks).'</td>
                    </tr>
                </table>';
...
if(!empty($bill_of_material)){  // IF BILL OF MATERIAL EXISTS------------------------------------
    ...
    <table cellpadding="0" cellspacing="0" style="border:1px solid #D9d9d9;">
        ...
        <!-- <tr class="item">
            <td><b>COMMENT</b></td>
            <td></td>
            <td colspan=" "><?php echo strtoupper($bill_of_material_row->comment); ?></td>
 ...
 -->
    ...
    <tr class="item">
        ...
        <td colspan=""><?php echo strtoupper($bill_of_material_row->comment); ?></td>
        <?php endforeach;?>
        <?php foreach ($bom_guage_total as $bom_guage_row):
            $a=$bom_guage_row->GUAGE_Layer_1;
            $b=$bom_guage_row->GUAGE_Layer_2;
            $c=$bom_guage_row->GUAGE_Layer_3;
            $d=$bom_guage_row->GUAGE_Layer_4;
            $e=$bom_guage_row->GUAGE_Layer_5;
            $f=$bom_guage_row->GUAGE_Layer_6;
            $g=$bom_guage_row->GUAGE_Layer_7;
            $h=$a+$b+$c+$d+$e+$f+$g;
            ?>

        <td><b>TOTAL THICKNESS</b></td>
        <td></td>
        <td ><?php echo $h ; ?> micron</td>
        <?php endforeach;?>
    </tr>
