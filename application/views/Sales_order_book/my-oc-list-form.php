						            	...
						            	<td>".$this->sales_order_book_model->get_order_status($mrow->order_no)."</td>
											...
											foreach($formrights as $formrights_row){
												if($formrights_row->view==1){
													...
													echo "<td>".$ci->common_model->view_date($mrow->oc_date,$this->session->userdata['logged_in']['company_id'])."</td>

													<td>".$mrow->oc_sent_flag."</td>";

													echo "<td>". ($formrights_row->view==1 && $mrow->oc_date!='0000-00-00' && $mrow->oc_sent_flag<>1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/send_oc/'.$mrow->order_no.'').'" target="_blank"><i class="envelope icon"></i></a>' : '')."</td>";
