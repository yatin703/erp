...
<div class="record_form_design">
	...
	<div class="record_inner_design" >
		...
		<table class="ui very compact sortable selectable celled table" style="font-size:10px;">
			<thead>
			<tr>
				<th>Sr. No.</th>
				<th>Hold/Unhold</th>
			...
			foreach($order_master as $mrow){
				...
				?>
					...
					<script>
						...
						$("#oc_<?php echo $mrow->order_no;?>").live('click',function(){
							//alert("We are working on OC Mail Notification To the Customer");
							$('#mail_<?php echo $mrow->order_no;?>').show();
						});
						});
						</script>
	            ...
	            if(!empty($this->input->post('customer_category'))){
	            	...
	            	$details_data['adr_category_id']=$arr_categary[1];
	            }
			    ...
			    if($rowspan>0){
					...
					echo "<div id='#mail_".$mrow->order_no."' style='display: none;'>
				  		HI
						</div><tr ".($mrow->hold_flag == 1 ? "style='background-color:#fff6f6;color:#9f3a38;'" : "").">
					<td rowspan='".$rowspan."'>".$n."</td>
					<td rowspan='".$rowspan."'>".($mrow->order_closed==1 ? "<a class='ui blue label'>COMPLETE</a>" : ($mrow->trans_closed<>1 ? ($mrow->hold_flag == 1  ? "<a class='ui red label'>HOLD</a>" : "") : "<a class='ui blue label'>CLOSED</a>"))."</td>
					...
					foreach($formrights as $formrights_row){ 
						...
						echo ($formrights_row->view==1 && $mrow->order_flag==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/view_spring_track/'.$mrow->order_no.'').'" target="_blank" title="Track My Order"><i class="print icon"></i></a> ' : '');

						echo ($formrights_row->view==1 && $mrow->po_file!='' ? '<a href="'.base_url('assets/'.$this->session->userdata['logged_in']['company_id'].'/po_file/'.$mrow->po_file.'').'" target="_blank"><i class="file pdf outline icon"></i></a> ' : '');

						echo ($formrights_row->view==1 && $mrow->oc_date!='0000-00-00'? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/view_oc/'.$mrow->order_no.'').'" target="_blank"><i class="clipboard outline icon"></i></a>' : '');

						echo ($formrights_row->view==1 && $mrow->oc_date!='0000-00-00' && $mrow->oc_sent_flag<>1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/send_oc/'.$mrow->order_no.'').'" target="_blank"><i class="envelope icon"></i></a>' : '');

						echo ($formrights_row->modify==1 && $mrow->final_approval_flag==1 && $mrow->trans_closed<>1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify_delivery_date/'.$mrow->order_no.'').'" alt"Modify Delievry Date" target="_blank"><i class="calendar icon"></i></a>' : '');	
					...
					<td rowspan='".$rowspan."'>".($mrow->final_approval_flag==1 ? "<i class='check circle ".($mrow->oc_date!='0000-00-00' ? 'green' : '')." icon'></i>" : "")." <a href=".base_url('index.php/'.$this->router->fetch_class().'/view/'.$mrow->order_no)." target='_blank'>".$mrow->order_no."</a></td>
				...
				<!--<td rowspan='".$rowspan."'>".$currency."</td>
					...
					foreach ($result as $drow){
									...
									if($specs_master_result){
										...
										if($specs_result){
											foreach($specs_result as $specs_row){
												...
												$length=$specs_row->SLEEVE_LENGTH;
												$sleeve_mb=$specs_row->SLEEVE_MASTER_BATCH;	
												$sleeve_mb2=$specs_row->SLEEVE_MASTER_BATCH_2;										
						...
						//---NEW STATUS ADDED ON DEMAND OF PRAJAKTA----------------------------------------------
							...
							if($mrow->trans_closed==1){
								...
								elseif($total_order_quantity>$supplyqty && $supplyqty<>0){
									...
									$pending_qty=$total_order_quantity- $supplyqty;
									$status.=number_format($total_order_quantity- $supplyqty,0,'.',',');
								}
								else{
									
									$status="Pending";
									$pending_qty=$total_order_quantity;
								}

								$sum_pending_qty+=$pending_qty;
						...
						//---------------------------------------------------------------------------------------
							...
							<td class='positive right aligned'>".$mrow->currency_id." ".$unit_rate."</td>
							<td class='positive right aligned'>".$mrow->currency_id." ".number_format($net_amount,2,'.',',')."</td>
							<td>";
									...
									if(!empty($drow->spec_id)){
											...
											if(substr($drow->spec_id,0,1)=="S"){
												...
												echo "<a href='".base_url()."index.php/cap_specification/view_cap_shoulder/".$rowcap->spec_id."/".$rowcap->spec_version_no."/".$shoulder_spe."/".$shoulder_spe_v." ' target='blank'>".$bom_row->cap_code."</a>";
													}
												}
							...
							<td>".($sleeve_mb<>'' ? $this->common_model->get_article_name($sleeve_mb,$this->session->userdata['logged_in']['company_id']) : $this->common_model->get_article_name($sleeve_mb2,$this->session->userdata['logged_in']['company_id']))."</td>	
							<td>".$shoulder_type."</td>
			    			...
			    			if($r==0){
						      ...
						      <td  rowspan='".$rowspan."'>".$ci->common_model->view_date($mrow->approval_date,$this->session->userdata['logged_in']['company_id'])."</td>
									...
									foreach($formrights as $formrights_row){ 
										...
										echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/view/'.$mrow->order_no.'').'" target="_blank"><i class="print icon"></i></a> ' : '');
										echo ($formrights_row->modify==1 && $mrow->final_approval_flag<>1 && $mrow->pending_flag<>1 && $mrow->trans_closed<>1  ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$mrow->order_no.'').'" target="_blank"><i class="edit icon"></i></a> ' : '');
										echo ($formrights_row->modify==1 && $mrow->trans_closed<>1 && $this->session->userdata['logged_in']['admin']==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify/'.$mrow->order_no.'').'"><i class="edit icon"></i></a> ' : '');
