...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/update_delivery_date');?>" method="POST" enctype="multipart/form-data" >
		...
		<table class="form_table_design">
            ...
            echo "<option ".$selected." value='".$ship_to_row->related_company_id."' ".set_select('consin_adr_company_id',''.$ship_to_row->related_company_id.'').">".$ship_to_row->relate."//".$ship_to_row->related_company_id."//".$ship_to_row->lang_property_name."</option>";
									...
									<tr>
										...
										<td><input type="text" name="cust_product_no" size="30" value="<?php echo set_value('cust_product_no',$order_row->cust_product_no);?>" disabled/></td>
									</tr>
			...
			<td>
				<table>
					...
					<tr>
						...
						<td><input type="checkbox" name="for_stock" <?php echo ($order_row->for_stock==1? 'value="1" checked' : 'value="0"');?> disabled/></td>
					</tr>
					<tr>
					...
					if($data['order_comment']==FALSE){
							...
							foreach($data['order_comment'] as $order_comment_row){
								...
								<td><textarea name="comment" disabled value="'.set_value('comment',$order_comment_row->lang_addi_info).'">'.set_value('comment',$order_comment_row->lang_addi_info).'</textarea></td>
								</tr>';
							}
					}
					?>
						<?php if(!empty($order_row->po_file)){
							echo '<tr>
														<td class="label">PO File  <input type="hidden" name="po_file" value="'.$order_row->po_file.'" readonly> </td>

														<td><a href="'.base_url('assets/'.$this->session->userdata['logged_in']['company_id'].'/po_file/'.$order_row->po_file.'').'" ><i class="file pdf outline icon"></i></a></td>
												</tr>';
							}?>
					...
					<div class="middle_form_inner_design">
						...
						if($this->input->post('sr_no')){
							...
							<tr id="tr_<?php echo $i;?>">
								...
								<td><input type="text" name="amount_<?php echo $i;?>" id="amount_<?php echo $i;?>" value="<?php echo set_value('amount_'.$i.'');?>" maxlength="15" size="10" class="amount" readonly disabled/></td>
