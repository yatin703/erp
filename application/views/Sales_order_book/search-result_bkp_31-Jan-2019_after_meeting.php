<div class="record_form_design">
	...
	<a target="_blank" href="<?php echo base_url('/index.php/'.$this->router->fetch_class().'/export_to_excel?from_date=').''.$this->input->post('from_date').'&to_date='.$this->input->post('to_date').'&adr_company_id='.$this->input->post('adr_company_id').'&consin_adr_company_id='.$this->input->post('consin_adr_company_id').'&article_no='.$this->input->post('article_no').'&order_no='.$this->input->post('order_no').'&for_export='.$this->input->post('for_export').'&for_sampling='.$this->input->post('for_sampling').'&final_approval_flag='.$this->input->post('final_approval_flag').'&order_closed='.$this->input->post('order_closed').'&trans_closed='.$this->input->post('trans_closed').'&cust_order_no='.$this->input->post('cust_order_no').'&cust_order_date='.$this->input->post('cust_order_date').'&user_id='.$this->input->post('user_id');?>" >
	...
	<div class="record_inner_design" style="overflow: scroll;">		
			    ...
			    if($rowspan>0){
					...
					foreach ($result as $drow){
									...
									if($specs_master_result){
										...
										if($cap_specs_result){
											foreach($cap_specs_result as $cap_specs_row){
												...
												$cap_foil=$this->common_model->get_article_name($cap_specs_row->CAP_FOIL_COLOR,$this->session->userdata['logged_in']['company_id']);
						...
						//---------------------------------------------------------------------------------------
							...
							<td>".$this->common_model->get_article_name($shoulder_foil,$this->session->userdata['logged_in']['company_id'])."</td>	
							<td>".$cap_dia."</td>
							<td>".$cap_type."</td>
							<td>".$cap_finish."</td>
							<td>".$cap_orifice."</td>
		...
		<?php  
			echo"<tr>
				<td colspan='33' style='text-align:right;'><b>TOTAL</b></td>
