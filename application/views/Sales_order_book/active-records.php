<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#year").change(function(event) {
			...
			$.ajax({
				type: "POST",
				url: "<?php echo base_url(); ?>" + "index.php/ajax/top_customer_order",
				data: {
					year: $('#year').val(),
					month: $('#month').val()
				},
...
<style>
	...
	[data-tooltip][data-position="bottom center"]:hover:after {
		-webkit-transform: translateX(-25%) scale(0.8) !important;
	}
			...
			<table class="ui sortable selectable celled table" style="font-size:10px;">
				...
				<tbody>
					...
					?>
							...
							<td rowspan='" . $rowspan . "'>" . ($mrow->order_closed == 1 ? "<a class='ui blue label'>COMPLETE</a>" : ($mrow->trans_closed <> 1 ? ($mrow->hold_flag == 1  ? "<a class='ui red label'><i>HOLD</i></a>" : "") : '')) . "</td>
							 <td rowspan='" . $rowspan . "'>" . $this->common_model->view_date($mrow->oc_date, $this->session->userdata['logged_in']['company_id']) . "</td>";
								echo "<td rowspan='" . $rowspan . "'>";
								foreach ($formrights as $formrights_row) {
									echo ($formrights_row->view == 1 ? '<div class="ui button view-clr" data-tooltip="View" data-position="bottom center"><a href="' . base_url('index.php/' . $this->router->fetch_class() . '/view/' . $mrow->order_no . '') . '" target="_blank"><i class="print icon"></i></a></div>' : '');

									//echo ($formrights_row->view==1 && $mrow->order_no!='0000-00-00'? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/view_oc/'.$mrow->order_no.'').'" target="_blank"><i class="clipboard outline icon"></i></a>' : '');

									if ($jobcard_flag == 0 &&  $mrow->user_id == $this->session->userdata['logged_in']['user_id']) {
										echo ($formrights_row->modify == 1 &&  $mrow->order_closed <> 1 && $mrow->trans_closed <> 1 || $jobcard_flag == 1 ? "<div class='ui button' data-tooltip='Hold/Unhold' data-position='bottom center'><a href='" . base_url('index.php/' . $this->router->fetch_class() . '/hold_modify/' . $mrow->order_no) . "' target='_blank'><i class='thumbs " . ($mrow->hold_flag == 1 ? 'down ' : 'up ') . " outline icon'></i></a></div> <input type='hidden' name='hold_flag_$n' value='1'  " . ($mrow->hold_flag == 1  ? "checked" : "unchecked") . ">
											<input type='hidden' name='ord_no[]' value='$mrow->order_no'>" : "");
									} else {
									}

									echo ($formrights_row->modify == 1 && $mrow->final_approval_flag <> 1 && $mrow->pending_flag <> 1 && $mrow->trans_closed <> 1 ? '<div class="ui button" data-tooltip="Modify" data-position="bottom center"><a href="' . base_url('index.php/' . $this->router->fetch_class() . '/modify/' . $mrow->order_no . '') . '"><i class="edit icon"></i></a></div>' : '');

									echo ($mrow->archive <> 1 && $formrights_row->delete == 1 ? '<div class="ui button" data-tooltip="Delete" data-position="bottom center"><a href="' . base_url('index.php/' . $this->router->fetch_class() . '/delete/' . $mrow->order_no . '') . '"><i class="trash icon"></i></a></div>' : '');



									echo ($formrights_row->view == 1 && $mrow->po_file != '' ? '<div class="ui button" data-tooltip="PO" data-position="bottom center"><a href="' . base_url('assets/' . $this->session->userdata['logged_in']['company_id'] . '/po_file/' . $mrow->po_file . '') . '" target="_blank"><i class="file pdf outline icon"></i></a></div> ' : '');


									echo ($formrights_row->modify == 1 && $mrow->final_approval_flag == 1 && $mrow->trans_closed <> 1 ? '<div class="ui button" data-tooltip="Modify Delievry Date" data-position="bottom center"><a href="' . base_url('index.php/' . $this->router->fetch_class() . '/modify_delivery_date/' . $mrow->order_no . '') . '" alt"Modify Delievry Date" target="_blank"><i class="calendar icon"></i></a></div> ' : '');
								}
								$n++;
								echo "</td>";
								echo "<td rowspan='" . $rowspan . "'>" . $ci->common_model->view_date($mrow->order_date, $this->session->userdata['logged_in']['company_id']) . "</td>
							<td rowspan='" . $rowspan . "'>" . ($mrow->final_approval_flag == 1 ? "<i class='check circle " . ($mrow->oc_date != '0000-00-00' ? 'green' : '') . " icon'></i>" : "") . " <div class='' data-tooltip='View' data-position='bottom center'><a class='so_underline' href=" . base_url('index.php/' . $this->router->fetch_class() . '/view/' . $mrow->order_no) . " target='_blank'>" . $mrow->order_no . "</a></div></td>
						...
						<!--<td rowspan='" . $rowspan . "'>" . $ship_to_gst . "</td>-->
							...
							<td rowspan='" . $rowspan . "'>" . $mrow->cust_product_no . "</td>
							...
							";
								...
								foreach ($result as $drow) {
									...
									<?php if ($sleeve_mb != '') { ?>
										<script type="text/javascript">
											$(document).ready(function() {
												$("#<?php echo "check_sleeve_mb_" . $mrow->order_no; ?>").change(function() {
													if ($(this).is(":checked")) {
														$("#loading").show();
														$("#cover").show();
														$('#loading').html('<img src="<?php echo base_url('assets/img/loading.gif'); ?>"> Loading...');
														...
														$.ajax({
															type: "POST",
															url: "<?php echo base_url('index.php/ajax/order_sleeve_mb'); ?>",
															data: {
																order_no: $("#<?php echo "checkk_sleeve_mb_" . $mrow->order_no; ?>").val(),
																value: 1
															},
															cache: false,
															success: function(html) {
										...
										<?php if ($shoulder_mb != '') { ?>
											<script type="text/javascript">
												$(document).ready(function() {
													$("#<?php echo "check_shoulder_mb" . $mrow->order_no; ?>").change(function() {
														if ($(this).is(":checked")) {
															$("#loading").show();
															$("#cover").show();
															$('#loading').html('<img src="<?php echo base_url('assets/img/loading.gif'); ?>"> Loading...');
															...
															$.ajax({
																type: "POST",
																url: "<?php echo base_url('index.php/ajax/order_shoulder_mb'); ?>",
																data: {
																	order_no: $("#<?php echo "checkk_shoulder_mb" . $mrow->order_no; ?>").val(),
																	value: 1
																},
																cache: false,
																success: function(html) {
											...
											<?php if ($cap_mb != '') { ?>
												<script type="text/javascript">
													$(document).ready(function() {
														$("#<?php echo "check_cap_code" . $mrow->order_no; ?>").change(function() {
															if ($(this).is(":checked")) {
																$("#loading").show();
																$("#cover").show();
																$('#loading').html('<img src="<?php echo base_url('assets/img/loading.gif'); ?>"> Loading...');
																...
																$.ajax({
																	type: "POST",
																	url: "<?php echo base_url('index.php/ajax/order_cap_code'); ?>",
																	data: {
																		order_no: $("#<?php echo "checkk_cap_code" . $mrow->order_no; ?>").val(),
																		value: 1
																	},
																	cache: false,
																	success: function(html) {
								...
								<!--<td>" . $ci->common_model->view_date(($drow->delivery_date != '0000-00-00' ? $drow->delivery_date : ""), $this->session->userdata['logged_in']['company_id']) . "</td>-->
									<td class='positive right aligned'>" . number_format($ci->common_model->read_number($drow->total_order_quantity, $this->session->userdata['logged_in']['company_id']), 0, '.', ',') . "</td>
									<td class='positive right aligned'>" . $unit_rate . "</td>
									<td class='positive right aligned'>" . number_format($net_amount, 2, '.', ',') . "</td>
									<td>";
									...
									if (!empty($drow->spec_id)) {
										...
										if (substr($drow->spec_id, 0, 1) == "S") {
											...
											foreach ($data['bom'] as $bom_row) {
												...
												$cap = array('article_no' => $bom_row->cap_code);
												$data['cap_specification'] = $this->common_model->select_active_records_where("specification_sheet", $this->session->userdata['logged_in']['company_id'], $cap);

												$shoulder = array('article_no' => $bom_row->shoulder_code);
												$dataaa['shoulder_specification'] = $this->common_model->select_active_records_where("specification_sheet", $this->session->userdata['logged_in']['company_id'], $shoulder);

												if ($data['cap_specification'] == FALSE) {
												} else {
													foreach ($data['cap_specification'] as $rowcap) {

														if ($dataaa['shoulder_specification'] == FALSE) {
															$shoulder_spe = "";
															$shoulder_spe_v = "";
														} else {
															foreach ($dataaa['shoulder_specification'] as $rowshoulder) {
																$shoulder_spe = $rowshoulder->spec_id;
																$shoulder_spe_v = $rowshoulder->spec_version_no;
															}
														}

														echo "<div class='' data-tooltip='Cap Specification View' data-position='bottom center'><a class='so_underline' href='" . base_url() . "index.php/cap_specification/view_cap_shoulder/" . $rowcap->spec_id . "/" . $rowcap->spec_version_no . "/" . $shoulder_spe . "/" . $shoulder_spe_v . " ' target='blank'>" . $bom_row->cap_code . "</a></div>";
													}
												}
											}
										}
									...
									if (!empty($drow->spec_id)) {
										...
										if (substr($drow->spec_id, 0, 1) == "S") {
											...
											foreach ($data['bom'] as $bom_row) {
												echo "<div class='' data-tooltip='BOM View' data-position='bottom center'><a class='so_underline' href='" . base_url() . "index.php/bill_of_material/view/" . $bom_row->bom_id . "' target='blank'>" . $drow->spec_id . "_" . $drow->spec_version_no . "</a></div>";
											}
										}
									}
									...
									if ($drow->ad_id != '' && substr($drow->ad_id, 0, 2) == 'AW') {
										echo "<div class='' data-tooltip='Artwork Coex Tube View' data-position='bottom center'><a class='so_underline' href='" . base_url() . "index.php/artwork_new/view/" . $drow->ad_id . "/" . $drow->version_no . " ' target='blank'>" . ($drow->ad_id != "" ? $drow->ad_id . "_R" . $drow->version_no : "") . "</a></div>";
									} else if ($drow->ad_id != '' && substr($drow->ad_id, 0, 3) == 'SAW') {

										echo "<div class='' data-tooltip='Artwork Spring Tube View' data-position='bottom center'><a class='so_underline' href='" . base_url() . "index.php/artwork_springtube/view/" . $drow->ad_id . "/" . $drow->version_no . " ' target='blank'>" . ($drow->ad_id != "" ? $drow->ad_id . "_R" . $drow->version_no : "") . "</a></div>";
									}
									...
									<td>" . $this->common_model->get_article_name($sleeve_mb, $this->session->userdata['logged_in']['company_id']) . "";
									foreach ($formrights as $formrights_row) {
										if ($formrights_row->view == 1) {
											echo ($sleeve_mb == '' ? "" : "<input type='hidden' name='checkk_sleeve_mb_" . $mrow->order_no . "' id='checkk_sleeve_mb_" . $mrow->order_no . "' value='" . $mrow->order_no . "'><input  readonly  type='checkbox' name='check_sleeve_mb_" . $mrow->order_no . "' id='check_sleeve_mb_" . $mrow->order_no . "'  " . ($drow->ordered_sleeve_mb == 1 ? 'value="1" checked="checked"' : 'value="0" ') . ">");
										}
									}
									echo "
									...
									<td>" . $this->common_model->get_article_name($shoulder_mb, $this->session->userdata['logged_in']['company_id']) . "" . ($shoulder_mb == '' ? "" : "<input type='hidden' name='checkk_shoulder_mb" . $mrow->order_no . "' id='checkk_shoulder_mb" . $mrow->order_no . "' value='" . $mrow->order_no . "'><input type='checkbox' name='check_shoulder_mb" . $mrow->order_no . "' id='check_shoulder_mb" . $mrow->order_no . "'  " . ($drow->ordered_shoulder_mb == 1 ? 'value="1" checked="checked"' : 'value="0" ') . ">") . "
									</td>
									<td>" . $shoulder_foil . "</td>
								<!--<td>" . $cap_dia . "</td>-->
									<td>" . $cap_type . "</td>
									<td>" . $cap_finish . "</td>
									<td>" . $cap_orifice . "</td>
									<td>" . $this->common_model->get_article_name($cap_mb, $this->session->userdata['logged_in']['company_id']) . "" . ($cap_mb == '' ? "" : "<input type='hidden' name='checkk_cap_mb" . $mrow->order_no . "' id='checkk_cap_mb" . $mrow->order_no . "' value='" . $mrow->order_no . "'><input type='checkbox' name='check_cap_mb" . $mrow->order_no . "' id='check_cap_mb" . $mrow->order_no . "'  " . ($drow->ordered_cap_mb == 1 ? 'value="1" checked="checked"' : 'value="0" ') . ">") . "
									</td>
									<td>" . $cap_foil . "</td>
								...
								<!--<td>" . $lacquer . "</td>
									...
									if ($r == 0) {
						            	...
						            	<td  rowspan='" . $rowspan . "'>" . ($mrow->for_sampling == 1 ? "SAMPLE" : "") . "</td>
					    					...
					    					<td class='ellipses' rowspan='" . $rowspan . "'><a style='background: #d8ea5c;'class='ui tiny label'><i class='user icon'></i>" . strtoupper($ci->common_model->get_user_name($mrow->user_id, $this->session->userdata['logged_in']['company_id'])) . "</a></td>
											<td class='ellipses' rowspan='" . $rowspan . "'>" . ($mrow->final_approval_flag == 1 ? "<a style='background: #d8ea5c;' class='ui tiny label'><i class='checkmark box icon'></i>" . substr(strtoupper($ci->common_model->get_user_name($mrow->approved_by, $this->session->userdata['logged_in']['company_id'])), 0, strpos($ci->common_model->get_user_name($mrow->approved_by, $this->session->userdata['logged_in']['company_id']), ' ')) . "</a>" : '') . "</td>
						...
						<thead>
						<tr>
						<th colspan='17' style='text-align:right;'><b>TOTAL</b></th>
						<th><b>" . number_format($sum_quantity, 0, '.', ',') . "</b></th>
						<th></th>
						<th><b>" . number_format($sum_net_amount, 2, '.', ',') . "</b></th>
						<th></th>
						<th></th>
						<th></th>
						<th></th>
						...
						<th></th>
						<th></th>
						<th><b>" . number_format($sum_supply_qty, 0, '.', ',') . "</b></th>
						<th></th>
						<th><b>" . number_format($sum_cancel_qty, 0, '.', ',') . "</b></th>
						<th><b>" . number_format($sum_total_tax, 2, '.', ',') . "</b></th>";
								$i = 0;
								foreach ($tax_header as $row) {
									echo '<th><b>' . number_format($total_array[$i], 2, '.', ',') . '</b></th>';
									$i++;
								}
								echo "<th><b>" . number_format($sum_gross_amount, 2, '.', ',') . "</b></th>
						 <th colspan='7'></th>
						 </thead>
					</tr>";
...
<div class="view">
  <div class="wrapper">
    <table class="ui very compact sortable selectable celled table">
      <thead>
        <tr>
