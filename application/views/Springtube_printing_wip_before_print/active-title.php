<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    $table='springtube_printing_wip_master_before';
  	...
  	echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'').'" class="ui teal tag label">Film printing WIP before print </a> : ' : '');
    ...
    echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/search_wip').'" class="ui tag label"><i class="search icon"></i>WIP Report</a> : ' : '');
     ...
     echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/pending_jobcards').'" class="ui tag label"><i class="search icon"></i>Pending Jobcards</a> : ' : '');
