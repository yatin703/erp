...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/qc_save');?>" method="POST" >
	<div class="form_design">
		...
		<?php foreach($coex_extrusion_printing as $coex_extrusion_row):?>
		<input type="hidden" name="printing_id" value="<?php echo $coex_extrusion_row->printing_id;?>">
				...
				<td width="45%">
					<table class="form_table_inner">
						<tr>
							...
							<td><input type="date" name="extrusion_date"  id="extrusion_date" size="10" value="<?php echo set_value('extrusion_date',$coex_extrusion_row->created_date);?>" readonly required/>
						...
						<tr>
							...
							<td><select name="machine" id="machine" readonly required/><option value=''>--Machine--</option>
						...
						<tr>
							<td class="label"><b>Operator</b></td>
							<td><input type="text" name="operator" size="10" value="<?php echo set_value('operator',$coex_extrusion_row->operator);?>" readonly required/>
						</tr>
					</table>			
				</td>							
			</tr>
	...
	<div class="middle_form_design" style="min-height:0px !important;">
		<div class="middle_form_inner_design">
         ...
         <th>Length</th>
			...
			<tbody>
		    <?php
			if($coex_extrusion_printing==FALSE){
				echo "<tr><td colspan='6'>No Record</td></tr>";
			}else{
				foreach ($coex_extrusion_printing as $coex_extrusion_row) {
			?>
			<tr id="tr_1">
        ...
        <td><input type="text" name="article_no" id="article_no" value="<?php echo set_value('article_no',$coex_extrusion_row->article_no);?>" placeholder="Product No" readonly required/ ></td>
			  ...
			  <td><input type="text" name="production_qty"  id="ok_qty_1" class="ok_qty" value="<?php echo set_value('production_qty',$coex_extrusion_row->printing_qty);?>" maxlength="15" size="10" readonly required/> No</td>
        <td><input type="text" name="ok_by_qc"  id="ok_by_qc" class="ok_by_qc" value="<?php echo set_value('ok_by_qc');?>" maxlength="15" size="10"/> NOS</td>
        <td><input type="text" name="hold_by_qc"  id="hold_by_qc" class="hold_by_qc" value="<?php echo set_value('hold_by_qc');?>" maxlength="15" size="10"/> NOS</td> 
			</tr>
			<?php 
				}
			}
