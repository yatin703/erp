	...
	$(document).ready(function(){
		...
		//VALIDATION STARTS--------------------------------------------------------
         ...
         $("#btnsubmit").click(function(e){
			...
			if(flag_pantone==0 || flag_screen_nature==0 || flag_screen_date==0 || flag_screen_label==0 || flag_screen_count==0|| flag_pantone==0){
				alert('Enter all the Scrren Details');
				return false;
			...
			<tr>
				<td width="50%">
					<table class="form_table_inner">
						...
						<td class="label">Screen Maker<span style="color:red;">*</span> :</td>
				...
				<table class="middle_form_table_design" id="tablePanton">
								...
								<?php
									...
									if(!empty($this->input->post('pantone[]'))){
												...
												<td><select name="artwork_para_id[]">					
													...
													if($graphics_plate_nature_master==FALSE){
														...
														foreach ($graphics_plate_nature_master as $row) {
															$selected=($row->artwork_para_id=='10'?'selected':'');
										...
										<td><input type="text" name="mesh[]" value="'.set_value('mesh[]').'"  placeholder="Mesh"></td>
										<td><input type="date" name="date_on_screen[]" value="'.set_value('date_on_plate[]').'" placeholder="Date On Screen"/></td>		
										<td><input type="text" name="label_on_screen[]" value="'.set_value('label_on_screen[]').'"  placeholder="Lable On Screen" ></td>
										<td><input type="number" name="screen_count[]" value="'.set_value('screen_count[]').'"  placeholder="No. Of Screen"></td>
