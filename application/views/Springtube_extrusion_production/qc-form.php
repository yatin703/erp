...
<script>
	...
	$(document).ready(function(){
		...
		$("#total_ok_meters").live('keyup',function(){
			...
			if($("#total_ok_meters").val()!=''){

				var total_meters_produced=parseInt($("#total_meters_produced").val());
				var total_ok_meters=parseInt($("#total_ok_meters").val());


				if(total_meters_produced>=total_ok_meters){

					var total_qc_hold_meters= total_meters_produced-total_ok_meters;
					$("#total_qc_hold_meters").val(total_qc_hold_meters);
				}else{
					$("#total_qc_hold_meters").val('');
					alert('Total Ok Meters should not be grater than Total Produced');
		...
		<div class="middle_form_inner_design">
		    ...
		    <?php  foreach ($springtube_extrusion_production_details as $detail_row):?>
					...
					<tr id="tr_<?php echo $detail_row->job_pos_no;?>">
						...
						<td>
							<input type="number" name="total_meters_produced"  id="total_meters_produced" class="quantity" value="<?php echo set_value('total_meters_produced',$detail_row->total_meters_produced);?>" min="1" max="20000" step="1" size="30" readonly/>
						</td>
						<td>
							<input type="number" name="total_ok_meters"  id="total_ok_meters" class="quantity" value="<?php echo set_value('total_ok_meters',$detail_row->total_ok_meters);?>" min="1" max="20000" step="1" size="30" required />
						</td>
						<td>
						...
						<td>
							<select name="qc_hold_reasons" id="qc_hold_reasons" ><option value=''>--Select QC Hold Reason--</option>
								<?php if($springtube_extrusion_reel_scrap_reasons==FALSE){
											echo "<option value=''>--Setup Required--</option>";}
								else{

									foreach($springtube_extrusion_reel_scrap_reasons as $reason_row){
										
										echo "<option value='".$reason_row->id."'  ".set_select('qc_hold_reasons',$reason_row->id).">".$reason_row->scrap_reason."</option>";
									}
								}?>
							</select>
						</td>
						<td>
							<textarea name="qc_remarks" id="qc_remarks"  value="<?php echo trim(set_value('qc_remarks',$detail_row->qc_remarks));?>" maxlength="500" style="height: 40px;width:250px">
								<?php echo trim(set_value('qc_remarks',$detail_row->qc_remarks));?>	
							</textarea>	
						</td>
						<td>
