...
<script>
	$(document).ready(function(){
		...
		$("#tbl_data .td_qc_check").each(function(){
			//alert($(this).html());
			if($(this).html()!=''){
				$(this).parent("tr").addClass("negative");
			}
...
<div class="record_form_design">
	...
	<div class="record_inner_design" style="white-space: nowrap;overflow: scroll;">
			<table class="ui sortable celled table"  style="font-size:10px;" id="tbl_data" >
				<thead>
					<tr style="text-align: center;">
						<th colspan="13" style="border-top: 1px solid rgba(34,36,38,.1); border-left: 1px solid rgba(34,36,38,.1);">Production Details</th>
						<th colspan="2" style="border-top: 1px solid rgba(34,36,38,.1);">Qc</th>
						<th colspan="3"  style="border-top: 1px solid rgba(34,36,38,.1);">Setup</th>
						<th colspan="4"  style="border-top: 1px solid rgba(34,36,38,.1);">Purging</th>
					</tr>
				<tr>
					<th style="text-align:center; border-left: 1px solid rgba(34,36,38,.1);">Sr.No.</th>
					<th>Action</th>					
					<th>Date</th>					
					<th>Shift</th>	
					<th>Customer</th>
					...
					<th>Job Weight (Kg)</th> -->
					<th>Trim Waste (Kgs)</th>
					<th>QC Status</th>
					<th>QC Hold</th>	

					<th>Setup Jobcard</th>					
					<th>Setup Weight (Kgs)</th>
					<!-- <th>Setup Meters</th> -->
					<th>Setup Reason</th>
					<th>Purging Jobcard</th>
					<th>Purging Weight (Kgs)</th>
					...
					echo "<tr><td colspan='22'>No Active Records Found</td></tr>";
						...
						foreach($springtube_extrusion_production_master as $master_row){
							...
							if(!empty($this->input->post('customer'))){
								...
								$details_data['customer']=$customer_arr[1];
							}
							if(!empty($this->input->post('sleeve_diameter'))){
								$details_data['sleeve_dia']=$this->input->post('sleeve_diameter');
							}

							if($this->input->post('qc_check')!=''){
								$details_data['qc_check']=$this->input->post('qc_check');
							}

							//$arr_data=array_filter($details_data);
					    	...
					    	if($rowspan>0){					    		
								...
								echo"<tr >
								<td style='border-left: 1px solid rgba(34,36,38,.1);text-align:center;' rowspan='".$rowspan."'>".$i."</td>
									<td rowspan='".$rowspan."'>";
									...
									foreach ($result as $details_row){
										...
										<td>".$details_row->total_microns."</td>
										<!--
										...
										<td class='positive right aligned'>".($details_row->qc_check ==1? $details_row->total_ok_meters."":"<i>Qc Pending</i>")."</td>
										
										<td class='positive right aligned'><b>".($details_row->total_ok_meters!=''? money_format('%!.0n',$details_row->total_ok_meters)."</b><i> MTRS</i>" :"<i>Qc Pending</i>")."</td>
										-->
										<td class='positive right aligned'>".($details_row->total_meters_produced!=0? number_format($details_row->total_meters_produced,2,'.',',')."</b>" :$details_row->total_meters_produced)."
										</td>
										<td class='positive right aligned'><b>";
										$total_ok_meters=0;
										...
										echo ($total_ok_meters!=0? number_format($total_ok_meters,2,'.',',')."</b>" :"<i>Qc Pending</i>");

										echo"
										</td>

										<td style='text-align:right' class='negative'><b>".($details_row->total_waste!=''? number_format($details_row->total_waste,2,'.',',')."</b>":"")."</td>
										<!--<td>".round($details_row->total_ok_meters/$reel_length,2)."</td>-->
										<td ".($details_row->qc_check =='0'? "class='td_qc_check'":"").">";
										foreach ($formrights as $formrights_row) {
											...
											if($formrights_row->new=='1' && $formrights_row->copy=='1' && $details_row->qc_check =='0'){ 	
												...
												if($formrights_row->modify=='1'){
													echo '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/modify_qc_inspection/'.$master_row->production_id.'/'.$details_row->details_id).'" title="Modify Qc Check" target="_blank"><i class="edit icon"></i></a>';
												}
											}else{
										...
										echo"</td>
										<td style='text-align:right' class='td_qc_hold warning'>";
										$qc_hold_meters=0;
										...
										echo($qc_hold_meters!=0?"<b>".$qc_hold_meters." </b><i>MTRS</i>":"");
										...
										<td style='text-align:right;' class='negative'><b>".($details_row->total_setup_weight!='' ? number_format($details_row->total_setup_weight,2,'.',',')." </b>":"")."</td>
										...
										<td style='text-align:right;' class='negative'><b>".($details_row->total_purging_weight!=''?number_format($details_row->total_purging_weight,2,'.',',')." </b>":"")."</td>
										<td>";
										...
										$sum_ok_reels+=round($total_ok_meters/$reel_length,2);
						...
						echo"
						<thead>
						<tr style='font-weight:bold;'><td colspan='11' style='text-align:right;'><b>TOTAL</b></th>
						<!--<th>".$sum_plan_meters."</th>
						<th></th>-->
						<th><b>".number_format($sum_total_meters_produced,2,'.',',')."</b><i> MTRS</i></th>						
						<th style='text-align:right;'><b>".number_format($sum_total_ok_meters,2,'.',',')."</b><i> MTRS</i></th>
						<th style='text-align:right;'><b>".number_format($sum_total_waste,2,'.',',')." </b><i> KGS</i></th>
						<!--<th>".$sum_ok_reels."</th>-->
						<th></th>

						<th style='text-align:right;'><b>".number_format($sum_total_qc_hold_meters,2,'.',',')."</b><i> MTRS</i></th>
						<!--<th>".$sum_qc_released."</th>
						<th>".$sum_qc_scrap."</th>
						
						<th>".$sum_jobcard_material_weight."</th>
						<th >".$sum_total_job_weight."</th>
						<th style='text-align:right;'><b>".number_format($sum_total_waste,2,'.',',')."</b><i> MTRS</i></th>
						-->
						<th></th>
						<th style='text-align:right;'><b>".number_format($sum_total_setup_weight,2,'.',',')."</b><i> KGS</i></th>
						<!--<th>".$sum_total_setup_meters."</th>-->
						<th></th>
						<th></th>
						<th style='text-align:right;'><b>".number_format($sum_total_purging_weight,2,'.',',')."</b><i> KGS</i></th>
						<th></th>
						<th></th>
						<!--
						<th></th>
