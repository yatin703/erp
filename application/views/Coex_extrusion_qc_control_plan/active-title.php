<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    $table='coex_extrusion_qc_control_plan';
  	...
  	echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'').'" class="ui teal tag label">Coex Extrusion QC Control Plan</a> : ' : '');
    ...
    //echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/attach_file').'" class="ui blue tag label" ><i class="add circle icon"></i>Attach File</a> : ' : '');
