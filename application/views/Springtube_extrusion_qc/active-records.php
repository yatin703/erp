...
<script>
	$(document).ready(function(){
		...
		$("#tbl_data .td_qc_scrap").each(function(){
			//alert($(this).html());
			if($(this).html()!=''){
...
<div class="record_form_design">
	...
	<div class="record_inner_design" style="overflow: scroll; white-space: nowrap;">
			...
			<tbody>	
				...
				$sum_total_qc_hold_qty=0;
				$sum_total_qc_release_meters=0;
				$sum_total_qc_release_qty=0;

				
				if($springtube_extrusion_qc_master==FALSE){
					echo "<tr><td colspan='14'>No Active Records Found</td></tr>";
						...
						foreach($springtube_extrusion_qc_master as $master_row){
							...
							$hold_qty=$this->common_model->jobcard_meters_to_qty($master_row->jobcard_no,$master_row->total_qc_hold_meters);



							echo"<tr>										
									<td style='text-align:right; border-left: 1px solid rgba(34,36,38,.1);'>".$i++."</td>
									<td>";
										...
										foreach ($formrights as $formrights_row) {
											...
											echo ($formrights_row->new==1 && $master_row->status==0 ? '<a class="ui green label" href="'.base_url('index.php/'.$this->router->fetch_class().'/qc_release/'.$master_row->qc_id.'').'" target="_blank" title="Release">QC Release</a> ' : '');

											//echo ($formrights_row->delete==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/delete/'.$master_row->qc_id.'').'" target="_blank" title="Delete"><i class="trash icon"></i></a> ' : '');
											
										}

										echo "</td>
									<td>".$this->common_model->view_date($master_row->qc_date,$this->session->userdata['logged_in']['company_id'])."
									</td>
									<td>";
									...
									<td class='negative'><b>".money_format('%!.0n',$master_row->total_qc_hold_meters)."</b><i> MTRS</i></td>
									...
									<td ".($master_row->next_process==11?"class='td_qc_scrap'":"")."><b>".($master_row->release_meters!=''?money_format('%!.0n',$master_row->release_meters)."</b><i> MTRS</i>":"")."</td>
									...
									<td>".$master_row->qc_remarks."</td>";
										...
										$sum_total_qc_hold_meters+=$master_row->total_qc_hold_meters;
										//$sum_total_qc_hold_qty+=$hold_qty;

										$sum_total_qc_release_meters+=$master_row->release_meters;
										//$sum_total_qc_release_qty+=$master_row->release_qty;


						}//master Foreach

						echo"<tr style='font-weight:bold;'><td colspan='13' style='text-align:right;'><b>TOTAL</b></td><td class='negative'><b>".money_format('%!.0n',$sum_total_qc_hold_meters)."</b><i> MTRS</i></td><td></td><td></td><td><b>".($sum_total_qc_release_meters>0?money_format('%!.0n',$sum_total_qc_release_meters)."</b><i> MTRS</i>":"")."</td> <td></td><td></td><td></td></tr>";	
