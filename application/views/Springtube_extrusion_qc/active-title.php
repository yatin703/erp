<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    $table='springtube_extrusion_wip_master';
  	...
  	echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'').'" class="ui teal tag label">Film Extrusion QC </a> : ' : '');
    ...
    echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/search').'" class="ui tag label"><i class="search icon"></i>Search</a> : ' : '');
     ...
     echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/search_qc_hold_monthly').'" class="ui tag label"><i class="search icon"></i>QC Hold Monthly</a> : ' : '');

    echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/scrap_search').'" class="ui tag label"><i class="search icon"></i>QC Scrap Search</a> : ' : '');
  	?> 
  <?php endforeach;?>
  </h2>
