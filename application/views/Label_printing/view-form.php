     ...
     <div style="margin:0.4cm;size:A4;padding:0;">
        	...
        	if($result==TRUE){
        		...
        		foreach($result as $row){
        			...
        			$customer_product_no=$row->cust_product_no;
        		}
        	}else{
        		$customer_po_no="";
        		$customer_po_date="";
        	...
        	for($i=1;$i<=$j;$i++){
        		...
        		<table width="99%" height="95%" style="font-size:10px;border:0.5px solid #000;padding:3px;margin:5px;border-radius:10px;">
        			...
        			<tr>
        				<td colspan="2" ><span style="font-size:11px;"><b>'.$this->common_model->get_article_name($this->input->post('article_no'),$this->session->userdata['logged_in']['company_id']).'&nbsp;'.($customer_product_no<>'' ? '('.$customer_product_no.')': '').'</span></b></td>
        			</tr>
