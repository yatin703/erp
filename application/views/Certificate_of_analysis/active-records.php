...
.inline_check{display: flex;align-items: center;}
</style>
            ...
            echo "<tr><td colspan='16'>No Active Records Found</td></tr>";
            	...
            	foreach($certificate_of_analysis as $row){
            		...
            		echo "<tr>
            				...
            				<td ><div class='inline_check'>".($row->final_approval_flag==1 ? "<i class='check circle icon'></i>" : "")." <a class='' href=".base_url('index.php/'.$this->router->fetch_class().'/view/'.$row->certificate_no.'/'.$row->so_no.'/'.$row->product_name.'')." target='_blank'>".$row->certificate_no."</a></div></td>
                        ...
                        <td>".$this->common_model->view_date($row->approval_date,$this->session->userdata['logged_in']['company_id'])."</td>
                           ...
                           echo ($formrights_row->view==1 ? '<div class="ui button view-clr" data-tooltip="View" data-position="top center"><a href="'.base_url('index.php/'.$this->router->fetch_class().'/view/'.$row->certificate_no.'/'.$row->so_no.'/'.$row->product_name.'').'" target="_blank"><i class="print icon" ></i></a></div>' : '');
