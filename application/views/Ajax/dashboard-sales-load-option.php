		    ...
		    <div class="column">
		    	...
		    	<div class="ui red segments">
		    		<div class="ui red segment">
					    <p><a  class="ui orange label">OVERVIEW</a><a class="ui olive label"><i class="calendar icon"></i><?php echo date('d M y',strtotime($from_date));?> TO <?php echo date('d M y',strtotime($to_date));?></a> 
					    	<a class="ui black label">COEX & SPRING </a></p>
					</div>
					<div class="ui segment">
						<table class="ui very basic collapsing celled table"  style="font-size:10px;">
						  ...
						  <!--  <td><?php /* 
						    ...
						    <td><?php echo $this->costsheet_model->active_record_count_where_pkey('costsheet_master',$this->session->userdata['logged_in']['company_id'],'status_flag','0',$from_date,$to_date);*/?></td>-->
						    </tr>
						...
						<!--	<td><?php /* 
						    ...
						    ?></td>
						    <td><?php echo number_format($total_contr_avg_unit_price,2);?></td>
						    <td><?php echo number_format(($total_contr_avg_unit_price/$total_sales_avg_unit_price)*100);?>%</td>
						    <td><?php echo $this->costsheet_model->active_record_count_where_pkey('costsheet_master',$this->session->userdata['logged_in']['company_id'],'status_flag','0',date("Y-m-d", strtotime("-1 month", strtotime($from_date))),date("Y-m-d", strtotime("-1 month", strtotime($to_date))));*/?></td>-->
							</tr>
					...
					foreach($pending_sales_order as $coex_order_row){
					    ...
					    echo "<tbody>
							...
							echo "<tr>
							    	...
							    	<td>".money_format('%!.0n',$total_balace_coex_pending_quantity)."</td>
							    	<td>".money_format('%.0n',$total_balace_coex_pending_value)."</td>
							    	<td>&#8377;".round($total_balace_coex_pending_avg,2)."</td>
							    	<td>".money_format('%!.0n',$total_balance_spring_pending_order_quantity)."</td>
							    	<td>".money_format('%.0n',$total_balance_spring_pending_order_value)."</td>
							    	<td>&#8377;".round($total_balance_spring_pending_order_avg,2)."</td>
							    	<td>".money_format('%!.0n',$total_balace_pending_order_quantity)."</td>
							    	<td>".money_format('%.0n',$total_balace_pending_order_value)."</td>
		  	...
		  	<div class="twelve wide column">
		    	...
		    	<div class="ui red segments">
					...
					foreach($print_type_wise_sales_domestic as $row_coex){
					    ...
					    $total_dom_export_spring_avg=($total_dom_export_spring_value<>0 ? $total_dom_export_spring_value/$total_dom_export_spring_quantity : '0');
		...
		<div class="ui equal width grid">
		  <div class="equal width row">
		    <div class="column">
		    	...
		    	<div class="ui segments">
		    		<div class="ui blue segment">
					    <p><a  class="ui orange label">SALES</a><a  class="ui blue label"> PRINT TYPE WISE</a><a  class="ui green label"> DIA TYPE WISE</a> 
					    	<a class="ui olive label"><i class="calendar icon"></i><?php echo date('d M y',strtotime($from_date));?> TO <?php echo date('d M y',strtotime($to_date));?></a></p>
					</div>
					<div class="ui segment">
						<?php
					...
					foreach($print_type_wise_sales_coex as $row_coex){
						...
						$total_print_type_avg_price=($total_print_type_value<>0 ? ($total_print_type_value/$total_prin_type_quantity) : 0);
						if($count==0){
							$fromm_date=strtoupper($row_coex->sales_month)." ".$row_coex->sales_year;
						}
						echo "<tr>
								<td>$i</td>
		    	...
		    	<div class="ui red segments">
		    		...
		    		<div class="ui segment">
						...
						if($print_type_wise_sales_coex==FALSE){}else{
							...
							echo "<tr><td>Total Sales Volume</td><td>".money_format('%!.0n',$total_total_print_type_sales_quantity)."</td><td>".round((($total_total_print_type_sales_quantity/$total_total_print_type_sales_quantity)*100),0)."%</td></tr>";
							}
						...
						if($order_completed_dispatched_volume==FALSE){}else{
							foreach($order_completed_dispatched_volume as $order_completed_dispatched_volume_row){
								...
								echo "<tr><td>Completed Orders Volume</td><td>".money_format('%!.0n', $order_completed_dispatched_volume_row->full_dispatch_volume)."</td><td>".round((($total_order_completed_dispatched_volume/$total_total_print_type_sales_quantity)*100),0)."%</td></tr>";
						...
						if($order_short_completed_dispatched_volume==FALSE){}else{
							foreach($order_short_completed_dispatched_volume as $order_short_completed_dispatched_volume_row){
								...
								echo "<tr><td>Short Close Orders Volume</td><td>".money_format('%!.0n', $order_short_completed_dispatched_volume_row->short_tubes)."</td><td>".round((($total_order_short_completed_dispatched_volume/$total_total_print_type_sales_quantity)*100),0)."%</td></tr>";
