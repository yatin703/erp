					...
					if($pending_sales_order==FALSE){
						...
						echo '<table class="ui sortable selectable celled table" style="font-size:12px;">
					        	<thead>
								   <tr>
								    	<tr>
								    	<th colspan="23"><a class="ui orange label">APPROVED & UNAPPROVED PENDING ORDER</a>'.($from_date!='' && $to_date!='' ? 
								    		' <a class="ui olive label"><i class="calendar icon"></i>'.$this->common_model->view_date($from_date,$this->session->userdata['logged_in']['company_id']).'  TO '.$this->common_model->view_date($to_date,$this->session->userdata['logged_in']['company_id']).'</a><br/><br/><a class="ui red label">EXCLUDING SAMPLE, DEVELOPMENT,CAP & STOCK SO</a>' : '');

                                        if(!empty($order_type_data)){
								    		
								    		$arr=explode(",",$order_type_data);
								    		foreach($arr as $order_type_data_row){
								    			$order_type_data_row=str_replace("'","",$order_type_data_row);
								    			echo '<a class="ui blue label">'.$order_type_data_row.'</a>';
								    		}
								    		echo "<br/>";
								    	}
