					...
					if($pending_sales_order==FALSE){
						...
						echo '<table class="ui sortable selectable celled table" style="font-size:10px;">
					        	<thead>
								   <tr>
								    	<th colspan="21"><a class="ui orange label">'.$customer_name.' PENDING ORDER BY OC DATE</a>'.($from_date!='' && $to_date!='' ? 
								    		' <a class="ui olive label"><i class="calendar icon"></i>'.$this->common_model->view_date($from_date,$this->session->userdata['logged_in']['company_id']).'  TO '.$this->common_model->view_date($to_date,$this->session->userdata['logged_in']['company_id']).'</a><br/><br/><a class="ui red label">EXCLUDING SAMPLE, DEVELOPMENT,CAP & STOCK SO</a>' : '').'</th>
								  </tr>
								  <tr>
					        			<th colspan="3"></th>
					...
					if($pending_sales_order_without_oc==FALSE){
					...
					foreach($pending_sales_order_without_oc as $coex_order_row){
					...
					foreach($pending_sales_order as $coex_order_row){
						...
						echo "<tr id='id_".$count."'>
					        ...
					        $pending_sales_order_customer=$this->sales_order_book_model->pending_sales_order_on_oc_date_by_customer('order_master',$first_date,$last_date,'');
