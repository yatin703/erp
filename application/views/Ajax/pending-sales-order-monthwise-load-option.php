					        	...
					        	<thead>
								   <tr>
								    	<th colspan="20"><a class="ui orange label">'.$customer_name.' PENDING ORDER</a>'.($from_date!='' && $to_date!='' ? 
								    		' <a class="ui olive label"><i class="calendar icon"></i>'.$this->common_model->view_date($from_date,$this->session->userdata['logged_in']['company_id']).'  TO '.$this->common_model->view_date($to_date,$this->session->userdata['logged_in']['company_id']).'</a><br/><br/>' : '').'<a class="ui red label">EXCLUDING CLOSED, DEVELOPMENT, CAP & STOCK SO</a></th>
								  </tr>
								  <tr>
					        			<th colspan="2"></th>
