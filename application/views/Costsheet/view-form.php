...
<?php 
       if(!empty($this->input->post('costheet_ide'))) {
...
<?php foreach ($ar_invoice_master as $ar_invoice_master_row):?>
    ...
    <div class="ui teal labels" style="text-align: center;">
      <div class="ui label">COST SHEET</div>
        ...
        <?php foreach($costsheet_master as $row):  ?>

        <?php echo $row->check_flag==1 ? '<span class="ui green right ribbon label"><i class="check circle icon"></i> Checked</span>' : '';?>
        ...
        /*
        $order_details=array('ref_ord_no'=>$order_no,'article_no'=>$article_no);
        ...
        */
        ...
        // Dispatch qty total added on 19 Aug 2021 -------------------eknath
        $order_details=array('ref_ord_no'=>$order_no,'article_no'=>$article_no);
        $result_dispatch=$this->sales_invoice_book_model->sum_supply_qty('ar_invoice_master',$order_details,$this->session->userdata['logged_in']['company_id']);

        $total_dispatch=0;
        foreach($result_dispatch as $row_total_dispatch){
            $total_dispatch=$this->common_model->read_number($row_total_dispatch->supply_qty,$this->session->userdata['logged_in']['company_id']);            
        }


        $order_master_data=array('order_no'=>$order_no);
        ...
        $order_master_data=array('order_no'=>$order_no,'article_no'=>$article_no);
        ...
        $supplyqty=0;
        $cancel_qty=0;
         $search_arr=array('ref_ord_no'=>$order_no,'article_no'=>$article_no);
        ...
        $order_data=array('order_no'=>$order_no,'article_no'=>$article_no);
    ...
    <?php if(isset($error)){ echo "<p class='alert alert-error'>$error</p>";}?>
        ...
        <table cellpadding="5" cellspacing="0" style="border:1px solid #D9d9d9;">
            ...
            <tr class="item">
                ...
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                <td width="35%" style="border-right:1px solid #D9d9d9;"><b><a href="<?php echo base_url('index.php/sales_order_book/view/'.$ar_invoice_details_row->ref_ord_no);?>" target='_blank'><?php echo $ar_invoice_details_row->ref_ord_no;?></a></b> <?php echo $this->sales_order_book_model->get_order_status($ar_invoice_details_row->ref_ord_no);?></td>
        ...
        <table cellpadding='5' cellspacing='0' style='border:1px solid #D9d9d9;'>
            ...
            foreach($result as $row_rm){
                ...
                $master_array= array('article_no' =>$article_no,'sales_ord_no'=>$order_no);
        ...
        <?php
            ...
            $data_jobcard= array('article_no' =>$article_no,'sales_ord_no'=>$order_no,'archive'=>0);
                    ...
                    if($order_flag!=1){
                        ...
                        foreach ($result_production_master as $key => $production_master_row) { 
                            ...
                            if($production_master_row->jobcard_type=='0'){
                                ...
                                echo'<a href="http://192.168.0.33:8181/3dtechnopack/projects/twerp/tracking-report.php?sono='.$order_no.'&psppsm_no='.$article_no.'&jobcardno='.$coex_jobcard_no.'" target="_blank">PRODUCTION</a>';
                                echo'&nbsp';

                                echo "//";

                                echo'<a href="'.base_url('index.php/costsheet/production_time/').$coex_jobcard_no.'" target="_blank">PRODUCTION TIME</a>';
                                echo'&nbsp';
        ...
        foreach($data2['job_card'] as $job_card_row){
            if($ar_invoice_details_row->order_flag==1 || $ar_invoice_details_row->order_flag==4){
...
<!-- Purging Start -->
        ...
        $heading_quantity=0;
        if($order_flag==1){
        $master_array= array('article_no' =>$article_no,'sales_ord_no'=>$order_no,'jobcard_type'=>'3');
        }else{
        $master_array= array('article_no' =>$article_no,'sales_ord_no'=>$order_no);
        }
        $data1=array_filter($master_array);
        ...
        <?php
        //echo strcmp($print_type_artwork, 'SCREEN');
        $total_ink_cost=0;
        if(strcmp($print_type_artwork, 'SCREEN') == 0 || strcmp($print_type_artwork, 'SCREEN ') == 0 || strcmp($print_type_artwork, 'SCREEN+UPTO NECK') == 0 || strcmp($print_type_artwork, 'SCREEN + LABEL') == 0 || strcmp($print_type_artwork, 'SCREEN+FLEXO') == 0 || strcmp($print_type_artwork, 'OFFSET SCREEN') == 0 || strcmp($print_type_artwork, 'FLEXO+SCREEN') == 0 || strcmp($print_type_artwork, 'Screen') == 0){
            ...
            foreach($result_screen_ink_value as $result_screen_ink_value_row){
                ...
                if($result_screen_ink_value==TRUE){
                   foreach($result_screen_ink_value as $result_screen_ink_value_row){
                        ...
                        $query=$this->db->query("SELECT * from coex_ink_consumption_master where article_no='".$article_no."' and artwork_no='$ad_id' and artwork_version_no='$version_no' and archive<>1 limit 0,1");
        ...
        $query=$this->db->query("SELECT * from digital_ink_price_master where ink_id='5' and apply_from_date<='$ar_invoice_master_row->invoice_date' and apply_to_date>='$ar_invoice_master_row->invoice_date' and archive<>1");
                                ...
                                if($result_digital_ink_pc_value==TRUE){
                                    ...
                                    if($result_digital_ink_pc<>0){
                                        $query_digi=$this->db->query("SELECT * FROM `springtube_printing_jobsetup_master` WHERE `order_no`='".$order_no."' AND article_no='".$article_no."'");
       ...
       /* echo "<tr class='heading'>
        ...
        <table cellpadding='5' cellspacing='0' style='border:1px solid #D9d9d9;'>
            ...
            $query=$this->db->query("SELECT sum(total_qty) as total_qty,calculated_purchase_price,article_no from reserved_quantity_manu where manu_order_no='$job_card_no' and article_no like 'RM-CAS%' AND calculated_purchase_price<>0");
            ...
            foreach ($data2['job_card'] as $job_card_row){
                $job_card_no=$job_card_row->mp_pos_no;
                    ...
                    $query=$this->db->query("SELECT sum(total_qty) as total_qty,calculated_purchase_price,article_no from reserved_quantity_manu where manu_order_no='$job_card_no' and article_no like '%CAME-%'");
                        ...
                        $query=$this->db->query("SELECT sum(total_qty) as total_qty,calculated_purchase_price,article_no from reserved_quantity_manu where manu_order_no='$job_card_no' and article_no like '%CAPS-000%' AND calculated_purchase_price<>0");
                        //echo $this->db->last_query();
                    }
...
<!--
    ...
    -->
        ...
        <table cellpadding='5' cellspacing='0' style='border:1px solid #D9d9d9;'>
            <tr class="heading">
                <td colspan="8" style='border-bottom:1px solid #D9d9d9;'><b>OTHER COST</td>
            </tr>
            <tr class="heading item">
                <td width="10%" ><b>GROUP</td>
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
        ...
        <?php
            ...
            $total_other_amc_cost=0;
            $total_other_amc_cost_amount=0;
            $query=$this->db->query("SELECT * from other_cost_master where company_id='000020' and archive<>1 and (order_flag=".$order_flag." or order_flag=3)  and sleeve_dia='' and apply_from_date<='$ar_invoice_master_row->invoice_date' and apply_to_date>='$ar_invoice_master_row->invoice_date' ");
            $result_other_amc_cost=$query->result();
            //echo $this->db->last_query();
            if($result_other_amc_cost==TRUE){
            foreach($result_other_amc_cost as $other_amc_cost_row){
                $total_other_amc_cost=$other_amc_cost_row->cost_per_tube*$this->common_model->read_number($ar_invoice_details_row->arid_qty,$this->session->userdata['logged_in']['company_id']);

                echo "<tr class='item'><td class='label' width='10%' >OTHER AMC COST</td>
                        <td class='label' width='5%' style='border-right:1px solid #D9d9d9;'></td>
                        <td class='label' width='35%' style='border-right:1px solid #D9d9d9;'>".$other_amc_cost_row->other_cost."</td>
                        <td class='label' width='10%' style='border-right:1px solid #D9d9d9;text-align:right'>".$this->common_model->read_number($ar_invoice_details_row->arid_qty,$this->session->userdata['logged_in']['company_id'])." NOS</td>
                        <td width='5%' style='border-right:1px solid #D9d9d9;text-align:right'> </td>
                        <td width='10%' style='border-right:1px solid #D9d9d9;text-align:right'>&#8377; ".round($total_other_amc_cost,2)."</td>
                        <td width='10%' style='border-right:1px solid #D9d9d9;text-align:right'>&#8377; ".round($other_amc_cost_row->cost_per_tube,2)."</td>
                        <td width='10%' style='text-align:right'></td>
                    </tr>";
                    $total_other_cost_amount+=$total_other_amc_cost;
                }
                }else{
                    echo "<tr><td colspan='8'>OTHER AMC COST IS NOT ENTERED</td></tr>";
                }

                // New Other cap cost-------------------------


                $total_other_cap_cost=0;
                $total_other_cap_cost_amount=0;
                $query=$this->db->query("SELECT * from other_cost_master where company_id='000020' and archive<>1 and (order_flag=".$order_flag." or order_flag=3)  and sleeve_dia='".$dia."' and cap_type='".$cap_type."' and (cap_finish = '".$cap_finish."' or cap_finish = '') and apply_from_date<='$ar_invoice_master_row->invoice_date' and apply_to_date>='$ar_invoice_master_row->invoice_date' ");
                $result_other_cap_cost=$query->result();
                //echo $this->db->last_query();
                
                if($result_other_cap_cost==TRUE){
                foreach($result_other_cap_cost as $other_cap_cost_row){
                    $total_other_cap_cost=$other_cap_cost_row->cost_per_tube*$this->common_model->read_number($ar_invoice_details_row->arid_qty,$this->session->userdata['logged_in']['company_id']);

                    echo "<tr class='item'><td class='label' width='10%' >OTHER CAP COST</td>
                            <td class='label' width='5%' style='border-right:1px solid #D9d9d9;'></td>
                            <td class='label' width='35%' style='border-right:1px solid #D9d9d9;'>".$other_cap_cost_row->other_cost."</td>
                            <td class='label' width='10%' style='border-right:1px solid #D9d9d9;text-align:right'>".$this->common_model->read_number($ar_invoice_details_row->arid_qty,$this->session->userdata['logged_in']['company_id'])." NOS</td>
                            <td width='5%' style='border-right:1px solid #D9d9d9;text-align:right'> </td>
                            <td width='10%' style='border-right:1px solid #D9d9d9;text-align:right'>&#8377; ".round($total_other_cap_cost,2)."</td>
                            <td width='10%' style='border-right:1px solid #D9d9d9;text-align:right'>&#8377; ".round($other_cap_cost_row->cost_per_tube,2)."</td>
                            <td width='10%' style='text-align:right'></td>
                        </tr>";
                        $total_other_cap_cost_amount+=$total_other_cap_cost;
                    }
                    }else{
                        echo "<tr><td colspan='8'>OTHER CAP COST IS NOT ENTERED</td></tr>";
                    }
                


                $total_other_cost_amount=$total_other_amc_cost+$total_other_cap_cost;
        ...
        <br/>
            ...
            <tr class="item">
                ...
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                <td width="35%" style="border-right:1px solid #D9d9d9;"><?php echo $dia."X".$length." | ".$ar_invoice_details_row->layer_no." LAYER";?>&nbsp;|&nbsp;<?php echo $this->common_model->get_article_name($sleeve_mb,$this->session->userdata['logged_in']['company_id']);?></td>
            ...
            <tr class="item">
                ...
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                <td width="35%" style="border-right:1px solid #D9d9d9;"><?php echo $shoulder_type;?>&nbsp;|&nbsp;<?php echo $this->common_model->get_article_name($shoulder_mb,$this->session->userdata['logged_in']['company_id']);?></td>
                <td width="10%" style='border-right:1px solid #D9d9d9;text-align:right'><b></td>
                <td width="5%" style='border-right:1px solid #D9d9d9;text-align:right'></td>
            ...
            <tr class="item">
                ...
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                <td width="35%" style="border-right:1px solid #D9d9d9;"><?php echo $print_type_artwork=($print_type_artwork=='' ? $print_type_bom : $print_type_artwork);?></td>
                <td width="10%" style='border-right:1px solid #D9d9d9;text-align:right'><b></td>
                <td width="5%" style='border-right:1px solid #D9d9d9;text-align:right'></td>
            ...
            <tr class="item">
                <td width="10%" >CAP + SHRINK SLEEVE</td>
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                <td width="35%" style="border-right:1px solid #D9d9d9;"><?php echo $cap_type." ".$cap_finish." ".$cap_dia;?></td>
                <td width="10%" style='border-right:1px solid #D9d9d9;text-align:right'><b></td>
                <td width="5%" style='border-right:1px solid #D9d9d9;text-align:right'></td>
            ...
            <tr class="item">
                <td width="10%" >OTHER COST</td>
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                <td width="35%" style="border-right:1px solid #D9d9d9;"><b></td>
                <td width="10%" style='border-right:1px solid #D9d9d9;text-align:right'><b></td>
                <td width="5%" style='border-right:1px solid #D9d9d9;text-align:right'></td>
                <td width="10%" style='border-right:1px solid #D9d9d9;text-align:right'>&#8377; <?php echo round($total_other_cost_amount,2);?></td>
                <td width="10%" style='border-right:1px solid #D9d9d9;text-align:right'>&#8377; <?php echo round($total_other_cost_amount/$this->common_model->read_number($ar_invoice_details_row->arid_qty,$this->session->userdata['logged_in']['company_id']),2);?></td>
                <td width="10%" style='text-align:right'></td>
            </tr>
        ...
        <table cellpadding="5" cellspacing="0" style="border:1px solid #D9d9d9;">
            ...
            <tr class="item heading">
                <td width="10%" >CHECKED<span style="color:red;">*</span></td>
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                <td width="35%" colspan="9"><b>
                    <input type="hidden" name="article_no" value="<?php echo $row->article_no;?>" readonly> 
                    <input type="hidden" name="order_no" value="<?php echo $row->order_no;?>" readonly> 
                    <input type="hidden" name="invoice_no" value="<?php echo $row->invoice_no;?>" readonly>    
                    <input type="hidden" name="costheet_ide" value="<?php echo $row->costheet_id;?>" readonly>
                    <input type="checkbox" id="check_flag" required name="check_flag"  value="1" <?php echo set_checkbox('check_flag','1');?> <?php echo ($row->check_flag==1 ? 'value="1" checked' : 'value="NULL"');?>/> 

                     <span style="color:red;"> <?php echo strtoupper($row->remark);?></span>   
                </td>
            ...
            <tr class="item heading">
                <td width="10%" >REMARK <span style="color:red;">*</span></td>
                <td width="5%" style="border-right:1px solid #D9d9d9;"></td>
                <td width="35%" colspan="9"> 
                         <select class="category related-post form-control" name="remark[]" multiple required>
                                <option value="">--Select Remark--</option>
                               <?php if($costsheet_remark==FALSE){
                                        echo "<option value=''>--Setup Required--</option>";
                                    }
                            else{
                                foreach($costsheet_remark as $costsheet_remark_row){
                                    $selected=($costsheet_remark_row->remark? 'selected' : '');
                                    echo "<option value='".$costsheet_remark_row->remark."'  ".set_select('remark',''.$costsheet_remark_row->remark.'').">".$costsheet_remark_row->remark."</option>";
                                }
                        }?>
