    ...
    echo money_format('%!.0n', $total_capping_s_cost);
        ...
        foreach($data2['job_card'] as $job_card_row){
           ...
           echo money_format('%!.0n', $total_additional_cost);
            ...
            $query=$this->db->query("SELECT * from other_cost_master where company_id='000020' and archive<>1 and (order_flag=".$order_flag.") and apply_from_date<='$mrow->invoice_date' and apply_to_date>='$mrow->invoice_date' and archive<>1");
            }else{

            $query=$this->db->query("SELECT * from other_cost_master where company_id='000020' and archive<>1 and (order_flag=".$order_flag." or order_flag=3) and apply_from_date<='$mrow->invoice_date' and apply_to_date>='$mrow->invoice_date' and archive<>1");
            }
			...
			if($result_other_cost==TRUE){
				foreach($result_other_cost as $other_cost_row){
					$total_other_cost=$other_cost_row->cost_per_tube*$this->common_model->read_number($drow->arid_qty,$this->session->userdata['logged_in']['company_id']);
			...
			echo "<td>";
				...
				$total_final_cost=$total_extrusion_cost+$total_purging_cost+$total_heading_cost+$total_printing_cost+$total_consumable+$total_labeling_cost+$total_foiling_cost+$total_shoulderfoil_cost+$total_capping_cost+$total_capping_s_cost+$total_total_packing_cost+$total_stores_spares_cost+$total_additional_cost+$total_freight_amount+$total_other_cost_amount;
