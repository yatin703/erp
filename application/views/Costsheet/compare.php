	...
	<div class="form_design">
			...
			<?php
				$count = 1;
					...
					$total_dispatch=0;
			   			...
			   			echo '<div class="four wide column">
								  ...
								  <div class="item">
								    ...
								    <div class="menu">
								      ...
								      <a class="item">Cap Moulding Cost</a>
								    </div>
								  </div>
            			...
            			$ar_invoice_details=$this->sales_invoice_book_model->active_details_records('ar_invoice_details',array('ar_invoice_no'=>$costsheet[0],'ref_ord_no'=>$costsheet[1],'article_no'=>$costsheet[2],'ar_pos_no'=>$costsheet[5]),$this->session->userdata['logged_in']['company_id']);
        ...
        foreach($data2['job_card'] as $job_card_row){
            ...
            if($data2['job_card']!=FALSE){
            	...
            	$job_card_no=$job_card_row->mp_pos_no;
            		...
            		if($cap_metalization!=''){
            			$query=$this->db->query("SELECT sum(total_qty) as total_qty,calculated_purchase_price,article_no from reserved_quantity_manu where manu_order_no='$job_card_no' and article_no like '%CAME-%' AND calculated_purchase_price<>0");
            		}else{
        ...
        foreach($data2['job_card'] as $job_card_row){
            ...
            foreach($result_freight_value as $result_freight_value_row){
	            ...
	            $query=$this->db->query("SELECT * from other_cost_master where company_id='000020' and archive<>1 and (order_flag=".$order_flag." or order_flag=3)  and sleeve_dia='' and cap_type='' and apply_from_date<='$ar_invoice_master_row->invoice_date' and apply_to_date>='$ar_invoice_master_row->invoice_date' and archive<>1");
				...
				if($result_other_amc_cost==TRUE){
					foreach($result_other_amc_cost as $other_amc_cost_row){
						...
						$total_other_amc_cost_amount+=$total_other_amc_cost;
				...
				// echo '<br/>';
				//  echo $total_other_amc_cost_amount;
				//  echo '<br/>';
				//  echo  $total_other_cap_cost_amount;
			
				$total_other_cost_amount=$total_other_amc_cost_amount;
				$total_cap_moulding_cost=$total_other_cap_cost_amount;
                

            
        
        	 $total_final_cost=$total_extrusion_cost+$total_purging_cost+$total_heading_cost+$total_total_printing_cost+$total_consumable+$total_labeling_cost+$total_foiling_cost+$total_shoulderfoil_cost+$total_capping_cost+$total_other_cap_cost_amount+$total_capping_s_cost+$total_capping_m_cost+$total_total_packing_cost+$total_stores_spares_cost+$total_additional_cost+$total_freight_amount+$total_other_cost_amount;
            ...
            $count=$this->common_model->active_record_count_where_costsheet('costsheet_master',$this->session->userdata['logged_in']['company_id'],'invoice_no',$ar_invoice_master_row->ar_invoice_no,'article_no',$costsheet[2],'archive','0','order_no',$costsheet[1]);

            $countt=$this->common_model->active_record_count_where_costsheet('costsheet_value_master',$this->session->userdata['logged_in']['company_id'],'invoice_no',$ar_invoice_master_row->ar_invoice_no,'article_no',$costsheet[2],'archive','0','order_no',$costsheet[1]);
            ...
            if($count==0){
            	$data=array(
					...
					'order_type'=>$order_flag,
						                ...
						                'dia'=>$dia,
						                'length'=>$length,
						                'layer'=>$ar_invoice_details_row->layer_no,
						                'print_type'=>$print_type_artwork,
						                'shoulder_type'=>$ar_invoice_details_row->shoulder_type,
						                'shoulder_foil'=>($ar_invoice_details_row->shoulder_foil<>'' ? 'YES':'NO'),
						                'cap_dia'=>$ar_invoice_details_row->cap_dia,
						                'cap_type'=>$ar_invoice_details_row->cap_type,
						                'cap_finish'=>$ar_invoice_details_row->cap_finish,
						                'cap_finish'=>$ar_invoice_details_row->cap_finish,
						                'cap_foil'=>($ar_invoice_details_row->cap_foil<>'' ? 'YES':'NO'),
						                'cap_shrink_sleeve'=>($ar_invoice_details_row->cap_shrink_sleeves<>'' ? 'YES':'NO'),
						                'cap_metalization'=>($ar_invoice_details_row->cap_metalization<>'' ? 'YES':'NO'),
						                'tube_foil_1'=>($ar_invoice_details_row->tube_foil_1<>'' ? 'YES':'NO'),
						                ...
						                'moulding_cost'=>$total_other_cap_cost_amount/$this->common_model->read_number($ar_invoice_details_row->arid_qty,$this->session->userdata['logged_in']['company_id']),
            ...
            if($countt==0){
            	$data=array(
					...
					'order_type'=>$order_flag,
						                ...
						                'unit_rate'=>$unit_rate_in_rupees,
						                'archive'=>'0',
						                'sleeve_cost'=>$total_extrusion_cost,
						                'purging_cost'=>$total_purging_cost,
						                'shoulder_cost'=>$total_heading_cost,
						                'printing_cost'=>$total_total_printing_cost,
						                'cosumable_cost'=>$total_consumable,
						                'label_cost'=>$total_labeling_cost,
						                'foil_cost'=>$total_foiling_cost,
						                'shoulder_foil_cost'=>$total_shoulderfoil_cost,
						                'capping_cost'=>$total_capping_cost,
						                'moulding_cost'=>$total_other_cap_cost_amount,
						                'shrink_sleeve_cost'=>$total_capping_s_cost,
						                'metalization_cost'=>$total_capping_m_cost,
						                'packaging_cost'=>$total_total_packing_cost,
						                'stores_spares_cost'=>$total_stores_spares_cost,
						                'additional_cost'=>$total_additional_cost,
						                'freight_cost'=>$total_freight_amount,
						                'other_cost'=>$total_other_cost_amount,
									 ...
									 $result=$this->common_model->save('costsheet_value_master',$data);
									}else{
										$data=array(
										'order_type'=>$order_flag,
						                'order_no'=>$costsheet[1],
										...
										$result=$this->common_model->update_one_active_record_where('costsheet_value_master',$data,'invoice_no',$ar_invoice_master_row->ar_invoice_no,'article_no',$costsheet[2],$this->session->userdata['logged_in']['company_id']);
									}
			   			...
			   			echo '<div class="four wide column">
								  ...
								  <div class="item">
								    ...
								    <div class="menu">
								      ...
								      <a class="item">'.round($total_other_cap_cost_amount/$this->common_model->read_number($ar_invoice_details_row->arid_qty,$this->session->userdata['logged_in']['company_id']),2).'</a>
