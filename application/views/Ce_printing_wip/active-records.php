   ...
   <div class="record_inner_design" style="overflow: scroll;">
         <table class="ui very basic collapsing celled table"  style="font-size:10px;" id="tbl_data">
            ...
            <tbody>
               <?php 
               $sum_bm_wip_qty=0;
               if($coex_extrusion_printing_wip==FALSE){
               echo "<tr><td colspan='12'>No Active Records Found</td></tr>";
               }else{
                  $i=1;
                  foreach($coex_extrusion_printing_wip as $row){
               
                     echo "<tr>
                           <td>".$i."</td>
                           <td>";
                                 foreach($formrights as $formrights_row){ 
                                echo ($formrights_row->new==1 && $row->ok_by_qc !=0 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/wip_release/'.$row->prt_wip_id).'" title="WIP Release" target="_blank"><i class="edit icon"></i></a> ' : '');                                                         
                                 }
                           echo"
