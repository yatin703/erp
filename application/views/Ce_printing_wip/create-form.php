...
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/wip_release_save');?>" method="POST" >
    ...
    <?php foreach($coex_extrusion_printing_wip as $row):?>
	  <table class="form_table_design">
	    <tr>
		  <td>
			<table class="form_table_inner">
            <input type="hidden" name="printing_id" value="<?php echo set_value('printing_id',$row->printing_id);?>">
			<input type="hidden" name="qc_id" value="<?php echo set_value('qc_id',$row->qc_id);?>">
			<input type="hidden" name="wip_id" value="<?php echo set_value('wip_id',$row->prt_wip_id);?>">
			
			<tr>
				<td class="label">Release Date <span style="color:red;">*</span> :</td>
				<td>
					<input type="date" name="extrusion_date"   value="<?php echo set_value('extrusion_date',date('Y-m-d'));?>" readonly />
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<select name="machine" id="machine" readonly required/>
			...
			<tr>
				...
				<td>
					<select name="shift" id="shift" readonly required/>
			...
			<tr>
				...
				<td><select name="to_process" id="to_process">				
					...
					<option value="8" <?php echo set_select('printing_scrap','8');?>>Scrap</option>
					<option value="2" <?php echo set_select('heading','2');?>>Heading</option>
					<option value="6" <?php echo set_select('capping','6');?>>Capping</option>
					<option value="9" <?php echo set_select('release_qc','9');?>>Return Printing QC</option>
				</select>
				</td>
			</tr>
