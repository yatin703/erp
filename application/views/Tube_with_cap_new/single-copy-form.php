...
<?php foreach($specification as $specification_row):?>
	...
	foreach($result_shoulder_foil as $shoulder_foil_row){ $shoulder_foil=$shoulder_foil_row->mat_article_no;}
	...
	if($result_cap_pp==FALSE){
		$cap_pp="";
		$cap_pp_per="";

		$result_cap_pp=$this->specification_model->select_details_record_where('specification_sheet_details',$this->session->userdata['logged_in']['company_id'],'spec_id',$specification_row->spec_id,'spec_version_no',$specification_row->spec_version_no,'srd_id','3_8_2','srd_id','asc');
		if($result_cap_pp==FALSE){
				$cap_pp="";
				$cap_pp_per="";
			}else{
	...
	$result_label=$this->specification_model->select_details_record_where('specification_sheet_details',$this->session->userdata['logged_in']['company_id'],'spec_id',$specification_row->spec_id,'spec_version_no',$specification_row->spec_version_no,'srd_id','3_12','srd_id','asc');
	if($result_label==FALSE){
		$label='';
	}else{
		
	foreach($result_label as $result_label_row){
		$this->load->model('article_model');
		$data['result_label_article']=$this->article_model->select_one_active_record('article',$this->session->userdata['logged_in']['company_id'],'article.article_no',$result_label_row->mat_article_no);
		if($data['result_label_article']==FALSE){
			$label='';
		}else{
				foreach ($data['result_label_article'] as $result_label_article_row) {
				$label=$result_label_article_row->article_name."//".$result_label_article_row->article_no;
				}
		}
			}
	}
	...
	?>
<form name="<?php echo $this->router->fetch_class();?>" action="<?php echo base_url('index.php/'.$this->router->fetch_class().'/save_single_layer_with_cap_copy');?>" method="POST" >
						          ...
						          if($data['artwork_final_version_no']==FALSE){
									...
									<tr>
										...
										foreach ($hot_foil as $hot_foil_row) {
											$selected=($hot_foil_row->article_no==$shoulder_foil ? 'selected' : '');
											echo "<option value='".$hot_foil_row->article_no."' ".set_select('shoulder_foil_tag',$hot_foil_row->article_no)." $selected>".$hot_foil_row->lang_article_description."</option>";
										}
										?>
										</select></td>
									</tr>
									...
									<tr><td class="label"><b>&nbsp;</b></td><td class="label"><b>&nbsp;</b></td></tr>
										...
										<td><input type="text" name="label_article_no" id="label" size="60" value="<?php echo set_value('label_article_no',$label);?>" /></td>
										</tr>
