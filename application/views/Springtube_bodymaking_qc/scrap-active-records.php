<div class="record_form_design">
	<h4>Scrap Active Records <?php echo ($this->input->post('from_date')!=''? 'From '.$this->input->post('from_date').' To '.$this->input->post('to_date'):'')?></h4>
	<div class="record_inner_design" style="overflow: scroll;">
			<table class="record_table_design_without_fixed">
				...
				if($springtube_bodymaking_scrap_master==FALSE){
					echo "<tr><td colspan='17'>No Active Records Found</td></tr>";
				}else{
						$i=($this->uri->segment(3)=="" ? 1 : $this->uri->segment(3)+1);
						
						$reel_length=$this->config->item('springtube_reel_length');

						foreach($springtube_bodymaking_scrap_master as $master_row){
							...
							echo"<tr>
									...
									<td>".$master_row->scrap_qty."</td>
									<td>";

										foreach ($formrights as $formrights_row) {


											echo ($formrights_row->new==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/qc_release/'.$master_row->scrap_id.'').'" target="_blank" title="Release"><i class="edit icon"></i></a> ' : '');
										...
										$sum_scrap_qty+=$master_row->scrap_qty;

						}//master Foreach

						echo"<tr><td colspan='15' style='text-align:right;'><b>TOTAL</b></td><td>".$sum_scrap_qty."</td><td></td></tr>";	
