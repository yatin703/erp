<article class="container_title">
  ...
  <?php foreach($formrights as $formrights_row):
    ...
    $table='springtube_bodymaking_qc_master';
  	...
  	echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'').'" class="ui teal tag label">Film Bodymaking QC</a> : ' : '');
    ...
    echo ($formrights_row->view==1 ? '<a href="'.base_url('index.php/'.$this->router->fetch_class().'/scrap_search').'" class="ui tag label"><i class="search icon"></i>Scrap Search</a> : ' : '');
  	?> 
  <?php endforeach;?>
  </h2>
